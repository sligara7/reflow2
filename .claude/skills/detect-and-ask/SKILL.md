...
3. Take the user's answer and write it back into the graph. There is a typed tool for every gap
   ...
   | `unallocated_component` | `propose_allocation` groups the capabilities by coupling and proposes which part should hold each — **put the proposal to the user, never apply it yourself**, then `allocate` what they accept and `evaluate_allocation` to score the result. This is the one gap whose answer is the deferred functional-allocation step: genesis stops before structure on purpose, and this is where that question comes back. If the boxes are namespaces rather than functional parts, `acknowledge_gap` with that reason |
