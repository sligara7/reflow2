...
| a number with a unit — "under 200ms", "no more than 40kg", "£3k" | **Constraint** → `add_constraint` with `quantity`/`unit`/`limit`/`direction`, then `constrains` each spender with its `contribution` **and its `unit`** — the rollup adds only numbers in the constraint's own unit and reports the rest, which is the Mars Climate Orbiter check made mechanical |
