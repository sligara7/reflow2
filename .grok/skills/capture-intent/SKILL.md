...
metadata: {composes: [STANDING, WRITES, MINTS], audience: anyone, summary: "Turn what you want into requirements and capabilities the design tracks."}
---
...
| "this is how we do X here" — operational know-how attached to a part or a place (a flag to pin, a file that must sit beside its store, the backup command that actually works) | **DesignRule** with `category: "runbook"` (the category is free text), then `governed_by` from the Environment or Component it concerns. Until 2026-09-18 this row said "nothing fits cleanly", and a droplet's whole run configuration went into a Decision's prose because of it (flo2, F5) — the vocabulary always fit; the row was wrong. `add_environment_rule` is NOT this: it is for what cannot be negotiated (a code, a law), and a flag default can |
| a sentence you could NOT place — the clause an extraction could not match, something they said that the design does not yet understand | **Fragment** → `create_node` with `node_type: Fragment`, `fragment_type: note`, `note_kind: author`. ⭐ These are the single best material for the next question to put to the person: they are, literally, the things said that the design cannot yet hold. Rendered once in a transcript they are gone (flo2 F25). ⚠️ There is **no typed constructor** — `create_node` is the only way today |
| anything no row above fits | ⚠️ **Nothing fits cleanly.** Say so rather than forcing it into a requirement's prose, and record the gap in reflow2's own design — the runbook row above sat here as exactly that admission for weeks before a field report costed it, which is how a table earns its next row |
