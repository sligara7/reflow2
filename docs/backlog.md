...
| **BL-29** | **`apply_heal` trusts the proposal; merge loses data silently** | **Done to a decision** — every reproducible hazard fixed (the chained-merge case reproduced and closed 2026-07-20); what remains is the survivor-choice rule, open by choice pending a decision. See below | ~~M~~ |
...
With that, every reproducible hazard on this item is fixed; the survivor rule is the one open
question, and it wants a decision rather than code.
