...
| **BL-181** | **`service.rs` is 6,356 lines and 139 tools in one file — and every structural detector reflow2 has says the design is fine, because none of them can see an artifact's mass** | *Anthony, 2026-08-02, mid-session and unprompted: "service.rs is about 5000 lines of code - it is becoming a monolith… one of the qualities of code is that we should avoid monoliths. Making things maintainable and modular is typically a much better approach… It also gets at the 'ilities' and MOSA." He named it as an example of breadboard → engineering mockup → prototype → production, not as a defect to fix now.* **Measured, and worse than the estimate: 6,356 lines, 139 `#[tool]`s and 122 request structs — 67% of the whole crate, and the entirety of the `ifc:mcp-tools` published boundary in one file.** **THE FINDING THAT MATTERS IS NOT THE FILE — IT IS THE SILENCE.** `detect_defects` reports no god-components and `graph_report` reports modularity **1.00**, both *correct*. **CORRECTION, 2026-08-02 — an earlier version of this row got the mechanism wrong and the real reason is blunter.** It said the detectors were quiet because `cmp:service` carries 8 capabilities, "an ordinary count". `god_components` does not count capabilities at all: it is *articulation points of the component-coupling graph* (`allocate.rs:416`), a routing-hub test with no notion of size in it. The honest statement is that **nothing in reflow2 measured granularity or mass, in the design or the build** — `coverage_report` counts whether a file is *claimed*, never how many distinctions the claim has swallowed. **ANSWERED BY [BL-182]**, on Anthony's constraint that the finding be reflow2's own rather than his eye's. So a human eye caught in one glance what the whole detector suite is structurally unable to say. That is [BL-179]'s thesis arriving as evidence: the instruments measure the design's shape and are blind to the artifact's, which is exactly the axis a maturity trajectory moves along. **The fix is mechanical, not research: router composition is ALREADY PROVEN IN THIS REPO** — `skills.rs` carries `#[tool_router(router = skills_router, vis = "pub")]` and `service.rs:2344` already sums them (`Self::tool_router() + Self::skills_router()`), so splitting into per-domain modules each with a named router is addition, not invention. **And it is the first real test case for [BL-180]**: same 139 tools, same function set, different packaging — a pure maturity restructuring, which `certify_preservation` should return `preserved` for. If it does not, that is a finding about the check. Size **L** | L |
...
| **BL-179** | **Designs normally start immature and morph to structure — reflow2 has no notion of that trajectory, so it cannot say where a design is on it** | *Anthony, 2026-08-02, closing the restructuring discussion: "probably all designs start immature (breadboard or engineering mockup) and then morph to a more refined structure. this allows reflow2 to build in how designs typically go where initially getting function right is the first objective, then you go towards getting the structure/packaging right. this is often iterative and organic."* **THIS REFRAMES EVERY STRUCTURAL MEASUREMENT reflow2 TAKES.** Its own design carries 8 functional systems, 3 Interfaces, **none between two systems**, and 51 cross-system `DEPENDS_ON` against 22 internal — and under this reading that is **not technical debt and not a criticism of any decision.** It is the expected state of a design that has correctly prioritised function first. The design is ON the normal trajectory, not off it. **So the thing to build is not a detector for bad structure — it is knowledge that the trajectory EXISTS and where a design sits along it**, which is `dec:readiness-is-an-observation-the-threshold-is-the-judgement` applied to the design's own structure rather than to enabling technologies. **The failure mode to avoid is precise:** a tool that reported undeclared seams as defects would punish correct early-phase work, which is the trap `cap:coverage` dodges by refusing to be a score, and bending the design to silence such a report is exactly `sharpening.md` §2's accommodation. **The bar before any vocabulary lands** (`dec:edge-orthogonality`): a maturity ladder earns its keep only if something COMPUTES position along it — labelling a design "engineering mockup" that nothing reads is a distinction with no computation behind it. Held under `dec:maturity-restructuring-delta` (proposed, brainstorm) along with the two first steps offered and not yet chosen: the function-preservation check, or fork-and-measure on the 14 seams. Size **M** | M |
