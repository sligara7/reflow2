...
| ID | Item | Why | Size |
|---|---|---|---|
| **BL-214** | **⭐ Cross-graph references are PROSE, so the loop cannot see them — a parent design naming a child's node ids in free text is invisible to `detect_gaps` and `propagate_change`** | *hxm_program field report, 2026-08-05, item 3.* The program-level graph references sim-graph node ids as plain strings (`"req:guided-tutorial"`, `"dec:panda-configurations in the sim graph"`) **because there is no cross-graph edge**. Consequence, in the reporter's words: "if the sim graph renames or drops a node, nothing here notices; blast-radius computations stop at the graph boundary even though the real design doesn't." **AND THE EXISTING TOOLS ADDRESS THE WRONG SHAPE:** `pair_designs` / `mirror_surface` / `compare_designs` / `merge_designs` handle PEER designs; the field need is a PARENT that CONTAINS children. `dec:nested-graphs` prescribed peer federation via published surfaces and never modelled hierarchy. Three suggestions from the reporter, none chosen: **(a)** graph-qualified refs (`8a9ce30fedd213e5:req:guided-tutorial`) usable as edge endpoints, with a dangling-cross-ref detector; **(b)** first-class subdesign containment, so `loop_status` at the parent can say "3 sub-designs, last checked when, N open items"; **(c)** failing both, a lint flagging id-shaped strings (`req:`, `dec:`, `cap:`) in node prose that resolve in a sibling graph — cheap, and it finds the informal links people actually make. Related and open: `dec:idea-one-owner-many-projects`, `dec:idea-intent-crosses-designs`. | L |
| **BL-215** | **The graph is machine-local and git-ignores itself, so every captured decision was ALSO hand-copied into markdown for teammates — double bookkeeping, and only the prose copy is shared** | *hxm_program field report, 2026-08-05, item 5.* `.reflow2` is git-ignored, so the design a session captures is invisible to the team; the reporter propagated it into `HXM-1288.F1.md` by hand. **This is `dec:the-transition-is-two-halves-and-reads-are-the-unscheduled-one` from the other side:** that node argues documents → graph → DERIVED documents, and here the derived half does not exist, so the graph became the private copy and markdown stayed the real one. Reporter's suggestions: a supported "commit this graph" story (deterministic, mergeable on-disk format), **or** a cheap tracked export (`graph_report_markdown` into a committed file) kept fresh by a session-end hook. **Note the tension to resolve rather than ignore:** reflow2 already HAS a committed deterministic export (`docs/design/reflow2.json`) and `req:persistence` says the design persists in-repo — so the gap is not the mechanism, it is that nothing tells a new project to commit its export or renders a READABLE one. | M |
| **BL-218** | **⭐ The content store has never stored a byte, and the verification that says it refuses to invent a directory PASSES AGAINST A STATE THE SHIPPED BINARY CANNOT REACH** | *Found by asking what the store actually holds, 2026-08-06 (Anthony: "are we still... what is it actively saving?").* **NOTHING IS SAVED AND NOTHING IS REFERENCED.** `content_manifest` → `entries: []`, `total_bytes: 0`, `missing: []`, `orphaned: []`, `store_root: "./reflow2-content"` — and **that directory does not exist anywhere on disk**. Of **136 Fragment nodes, ZERO carry a `content_ref`**, the field meant to point at stored bytes, so `missing: []` is honest rather than reassuring: nothing is referenced, so nothing can be reported absent. `ver:content-store` and `ver:content-manifest` are both still **`PLANNED`**. **⭐ THE FALSE GREEN.** `ver:content-surface` PASSES and claims *"A SERVER WITH NO `--content-path` CONFIGURED REFUSES BY NAME instead of inventing a directory — blobs are committed, so where they live is a decision about the consumer's repo."* But `crates/reflow2-mcp/src/main.rs:30` declares `#[arg(long, default_value = "./reflow2-content")]`, so `main.rs:809`/`:881` always pass `Some(default)` and **the refusal at `service.rs:2464` is unreachable from the real binary**. The test proving it — `a_server_with_no_content_store_says_so_instead_of_choosing_one` (`crates/reflow2-mcp/tests/tools.rs:2795`) — builds `ReflowService::in_memory()`, whose builder default is `content_path: None` (`service.rs:2425`): **a state the shipped server can never be in.** So the shipped server DOES invent a directory, which is precisely what the verification forbids, and the check is green. Same shape as the lesson `ver:content-surface`'s own header records (*"the only layer that catches what three agreeing home-grown clients once all missed"*) — the fix for which is to drive the REAL binary. **CONSEQUENCE, unexercised only because nobody has called `content_put`:** the first put creates `./reflow2-content` relative to **whatever cwd the server was launched in**, while `.gitattributes` already carries `reflow2-content/** binary` assuming repo root and `.gitignore` never mentions it — so blobs land untracked-but-not-ignored at a cwd-dependent path, and `dec:content-store-implementation` says blobs are meant to be COMMITTED. **Fix shape, not chosen:** make `content_path` an `Option<String>` with NO `default_value` so the refusal is reachable and the decision stays the consumer's (as the verification already claims), plus one case that drives the shipped binary rather than an in-process builder. **Bears on `dec:idea-human-newcomer-reader`:** a user pasting a brain dump has no file for an Artifact to hash, so the store IS the mechanism for keeping their source — `req:sources-are-kept-not-just-parsed` holds for reflow2's own graph only because its Fragments describe repo files that Artifacts already track. **⭐ FILED HERE UNDER PROTEST, AND IT IS EVIDENCE:** this is exactly category (i) of `dec:idea-backlog-belongs-in-the-graph` — a defect in a capability that is built, VERIFIED and wrong — the one kind of row that node measured as having **no home in the graph**. `Verification` says the check passes; `DriftEvent` says code and design agree, and here they agree *and are both wrong*. That absence is why this is prose. | M |
| **BL-217** | **⭐ The export-lineage guard cannot see through a PR that exports TWICE, so it FAILS a chain that is actually intact — and its failure message prescribes the one fix that does not help** | *Found by hitting it, 2026-08-06, on PR #87 (`feat/updating-doc`), which is STACKED on #86 (`feat/dependency-graph-id`).* A second export commit on a branch that already had one produced `FAIL LINEAGE ... records sha256:8b5471e9… where sha256:486ecbac… is expected`. **The chain was correct**: main `85b17257` → #86 `486ecbac` → #87's first export `8b5471e9` → the new one `2b536b9a`, every `prev_content_hash` matching its predecessor's `content_hash`, verified commit by commit. **The guard is what is wrong.** `_export_pair` takes `head == doc` to mean "in CI" and then reads the predecessor from `HEAD~1` — but `actions/checkout` on a `pull_request` checks out `refs/pull/N/merge`, whose FIRST PARENT is the base branch tip. So `HEAD~1` is not the previous export in the chain, it is **the base branch's export**, and that only coincides with the true predecessor when the PR carries exactly ONE export commit. **The message actively misleads:** it names two causes (exported elsewhere and copied in; exported more than once since the last commit) and prescribes "restore the committed file (`git checkout docs/design/reflow2.json`) and export straight onto it, once" — which is exactly what HAD been done. A third cause exists and is not listed, and following the advice cannot fix it. **Note where this sits:** BL-208 raised `fetch-depth` to 2 because at depth 1 this check silently SKIPPED and *"has therefore never once run in CI"* — so this is its newly-live state finding its own blind spot almost immediately, and the class of bug is the same one BL-107/BL-177 chase (two contexts, one predicate, contradictory answers about the same file). **Two candidate fixes, neither chosen:** **(a)** walk back to the last commit that actually CHANGED the export rather than assuming `HEAD~1` (needs more than depth 2, and `git log --follow -1` on the path is the natural form); **(b)** if the one-export-commit-per-PR convention is meant to be normative — the release procedure already says *"only ONE commit may export"* — then say so IN the failure message and make the guard name that rule, instead of diagnosing two causes that do not apply. Until then the workaround is the convention: one export commit per PR, folded. | M |
| **BL-216** | **Three graphs on one repo minted by three different reflow2 versions (0.17.0 / 0.22.1 / 0.23.0) and no stated cross-version policy** | *hxm_program field report, 2026-08-05, item 6.* No breakage observed. Filed because it becomes load-bearing the moment cross-graph linking ([BL-214]) lands: a parent on 0.24.0 reading a child minted at 0.17.0 needs a stated answer, and the schema stamp is the thing that would carry it. **Small now, and the cheapest time to decide.** | S |
| **BL-213** | **⭐ Auto-merge at 90 silently destroyed SIBLING components — one document's nine crates became five, and the survivors carried the WRONG NAMES — DONE 2026-08-05** | *Found by the FIRST REAL TRIAL of `cap:corpus-ingest`, six dev_storyflow architecture documents, 2026-08-05.* Measured against `dynograph_resolution::token_sort_ratio` at the 90 auto-merge threshold: **95** `dynograph-vector` vs `dynograph-core` (merged, wrong), **94** `dynograph-storage` vs `dynograph-core` (merged, wrong), **84** `Auth Service` vs `Authentication Service` (not merged, wrong). **The ordering is INVERTED for identifier-shaped names, so no threshold repairs it** — the scope doc's claim that "90 on a token-sorted ratio is near-identity" is false for the names real systems use. **NOT a corpus defect**: isolating one document reproduces it exactly, so `ingest` has done this to every single-document run since fuzzy dedup landed. Invisible to self-host because reflow2's own design uses prose names, not prefixed identifiers — **BL-199's shape a second time**. **FIXED** with a token discriminator (refuse a merge when either side has a whole word the other does not abbreviate; `core`/`storage` are not spellings of each other, `auth`/`authentication` are), a `distinguished_by` reason naming the word, and `CorpusReport.fuzzy_merges` carrying WHAT merged plus the document that caused it instead of a count. Mutation-checked: disabling the discriminator restores 5-of-9. | M |
| **BL-210** | **⭐ `adopt` REGISTERS documents and nothing READS them — so intent recovery is fed a list of filenames, and 1,124 documents produced 9 requirements** | *Anthony, 2026-08-04, from running it: "when I first ran the reflow2 adopt skill on dev_storyflow, it did a very lazy job of actually reviewing any of these documents. I think it just kind of acknowledged the files were there."* **It is not the agent being lazy — it is the skill working exactly as written.** Phase 0 says *"list what else exists, because intent lives outside the implementation… Record each source as a `Fragment`"*; Phase 3 says *"Requirements come **only** from Phase 0's non-code sources."* **So intent recovery is fed exclusively by documents, and the only operation the skill prescribes on a document is LISTING it.** There is no read step anywhere in adopt. **MEASURED, from the fleet's own baseline memory: `~124 nodes / 177 edges … 9 Requirements + 3 Constraints … 17 Fragments (Phase-0 provenance ledger)` — against 1,124 markdown files.** Their own verdict, unprompted: *"The adopt baseline captured what the system is **made of** (services, contracts, components); it captured almost none of what it is **for**. The intent lives in the docs."* **THE COST IS ALREADY PAID: a separate multi-session campaign (`project_reflow2_doc_intent_sweep`, on a user directive of 2026-07-25) had to read ~275 documents BY HAND to do what the skill should have done** — and that campaign's IN/OUT scope table is now the only human-validated benchmark we have for judging a mass-ingest strategy, so it is worth keeping. **The fix is not new prose in adopt; it is adopt CONSUMING [BL-186]'s mass ingest at Phase 0** — see [req:accreted-intent-becomes-a-design], which records that mass ingest sits UNDER adopt and genesis rather than beside them. **Note the trap this closes and the one it opens:** adopt's own granularity rule (*a directory artifact claims its subtree*) is what let 359 files read as registered while being individually invisible — the finding behind [BL-188] — so whatever adopt does with a doc corpus must mark what it did not descend into as `pending_expansion`, not leave it opaque. Size **M** | M |
| **BL-211** | **`genesis` assumes an opening BRIEF — one statement — and has no shape for an opening FOLDER** | *Anthony, 2026-08-04: "mass-ingest could be used on the genesis skill also because maybe somebody has laid out hundreds of documents about what they'd like to design and build, but never actually did anything further than layout in documents/drawings/other artifacts what it is they wanted to build."* The served skill's own words are *"bootstrap it from the user's opening brief"*, and every step downstream assumes one short statement to seed Requirements and Capabilities from. **A person arriving with 300 documents and no code is not a genesis case as written and not an adopt case either — adopt is for a system that ALREADY EXISTS and recovers intent by reverse-engineering it, and here there is nothing built to reverse-engineer.** That gap is now recorded as [req:accreted-intent-becomes-a-design] (`proposed`), which also names why it is not simply [req:corpus-ingest] in another hat: **contradiction is the EXPECTED state rather than a defect, there is nothing built to reconcile against, and the corpus is the only artifact so nothing outside it can settle a disagreement between its parts.** **DISTINCT FROM [BL-196] and they should not be merged:** BL-196 is genesis asking where artifacts WILL live, forward-looking and preventative; this is genesis accepting a corpus that ALREADY exists. Same skill, opposite time direction — and doing BL-196 first is what stops this case recurring. Size **M** | M |
| **BL-212** | **A folder of documents carries no recoverable chronology, so `EVOLVES_INTO` and `SUPERSEDES` — both directional — can never be drawn from a corpus** | *Found 2026-08-04 while capturing [req:accreted-intent-becomes-a-design].* The vocabulary for "someone changed their mind over years" is already there and is entirely unused for this: `CONTRADICTS` carries `alignment: opposing\|supporting` with confidence and evidence, `EVOLVES_INTO` is literally *"source is an earlier form that becomes target over time"*, `SUPERSEDES`/`OBSOLETES` retire on the record, and `TemporalFact` holds a time-bounded assertion with `VALID_FROM`/`VALID_TO`. **All of it needs to know which document came FIRST, and a folder cannot say.** File mtimes lie (checkout, copy, sync all rewrite them), a document corpus may have no git history at all, and the dates inside documents are prose. **This is the exact inverse of the bug fixed in this same increment:** there, arrival order wrongly decided the canonical NAME and the fix was to stop consulting order; here order genuinely carries meaning and there is no way to learn it. **So the honest default is that a corpus ingest draws `CONTRADICTS` (symmetric, needs no direction) and NEVER `EVOLVES_INTO`/`SUPERSEDES` (directional) unless a human or an explicit in-document date supplies the arrow** — which makes "you changed your mind here" answerable and "this replaced that" a question. Candidate signals worth measuring before choosing: an explicit date in frontmatter or filename, `git log --follow --diff-filter=A` first-commit date where a repo exists, and the corpus's own supersession language ("replaces", "superseded by"). **Do not infer the arrow from mtime.** Size **S** | S |
| **BL-209** | **⭐ `orphan_node` counts `AUTHORED_BY` as attachment, so 22 Decisions — three of them ACCEPTED — are invisible to the one detector that exists to find them** | *Found 2026-08-04 while recording the embeddings discussion: `propagate_from` on `dec:idea-embeddings-adoption` returned **0 impacted**, yet `detect_defects` did not flag it.* The Decision orphan check (`heal.rs:733`) is `outgoing(..).is_empty() || incoming(..).is_empty()` — **any edge of any type counts as attachment.** Every node authored through the tools carries an `AUTHORED_BY` edge to a Contributor, so a Decision needs only its own provenance to read as attached. **MEASURED on the committed export: 238 Decisions, 125 of them `decision:ack:` (excluded deliberately and correctly, `ver:acknowledgement-not-structure`), 14 with literally zero edges (the ones that DO fire), and 22 PROVENANCE-ONLY — invisible.** Three of the 22 are **`accepted`**: `dec:a-failing-check-says-against-what`, `dec:commitment-edges-survive-snapshots`, and `dec:export-once-per-pr`. **[BL-162] is the row this half-fixes:** *"An accepted Decision that governs nothing is invisible to every detector"* — the fix catches the zero-edge case and misses the provenance-only case, which is the larger one. **THE COST IS ALREADY PAID AND MEASURABLE: `dec:export-once-per-pr` is an accepted process rule that PR #70 violated, in a session that then wrote the same rule into [BL-208] as a discovery** — the decision could not be found by propagation, by the detector, or by anything that governs it. **The fix is in-house and one line of vocabulary:** [BL-176] already solved this exact shape for Artifacts with `ARTIFACT_BOOKKEEPING` — *"bookkeeping drawn by the machinery, not by anyone saying what the file is FOR … counting them as attachment would silence `orphan_node` everywhere"* — and `AUTHORED_BY` is precisely that, but the Decision path uses no exclusion list at all. Generalise the exclusion rather than duplicating it, and check the same question for every other type the orphan check covers. **WATCH THE BLAST RADIUS BEFORE SHIPPING: this will surface 22 decisions at once, and 19 are `proposed` brainstorm captures that may be correctly unattached — the row is not "attach all 22", it is "the detector must be able to SEE them".** Size **S** | S |
| **BL-208** | **⭐ The export-lineage guard has NEVER run in CI — `actions/checkout` is shallow, so it could not see `HEAD~1`, treated the question as unanswerable and skipped, silently, on every run since [BL-107] added it — CI HALF DONE 2026-08-04** | *Found 2026-08-04 because `main` went GREEN on a lineage break that the identical check fails on locally.* `check_export_chain` compares HEAD's export against HEAD~1's, and `_export_pair` returns `None` — **skip, never fail** — when `git show HEAD~1:…` cannot answer. `actions/checkout@v5` defaults to `fetch-depth: 1`, so HEAD~1 does not exist in any CI checkout and the guard has been inert since the day it landed. **PROVED BOTH WAYS with two clones of the same commit: `--depth 1` reports `OK — design and build agree`; `--depth 2` reports the LINEAGE failure.** Fixed by pinning `fetch-depth: 2` on both jobs. **The skip is correct behaviour in the wrong environment — "an unanswerable question is skipped, never guessed" is the right rule for a laptop outside a git tree, and it is exactly wrong for the one environment that is supposed to be authoritative. Any guard whose no-op path is silent will eventually run nowhere and say so to nobody: [BL-206] was the same sentence about a truncated sweep, one day and one gate apart.** **SECOND HALF, STILL OPEN, AND IT IS THE CAUSE OF THE BREAK THE GUARD SHOULD HAVE CAUGHT: squash-merging a PR that exports TWICE severs the chain.** #70 carried two export commits (`d6631e5` and `dfa6e04`); the squash collapsed them, so `main`'s export records `prev_content_hash: f231440…` — the content hash of an intermediate that now exists in **no commit on main** — where `7f49f27…` is expected. **THE HOLE IS PERMANENT SHORT OF REWRITING `main`, and it is documented here so a later reader does not mistake it for a tampered export.** It also self-conceals: the check only compares HEAD against HEAD~1, so the very next commit that leaves the export untouched makes the pair identical and the complaint stops — **buried, not repaired**. **THE RULE THIS FORCES, and nothing enforces it yet: in a squash-merge repo a PR may contain at most ONE commit that touches the export, and its `prev_content_hash` must be `main`'s committed export at merge time — so if `main` moves, re-export before merging.** A guard for that is the open work: it has to run on the PR's merge base, not on HEAD~1. **CORRECTED 2026-08-04 — I CLAIMED TO HAVE DERIVED A RULE THAT WAS ALREADY DECIDED, AND THEN BROKE IT.** `dec:export-once-per-pr` is **accepted** and states it verbatim: *"A branch may hold as many commits as it likes; exactly ONE of them may write docs/design/reflow2.json, and it should be the last."* Its rationale already contains this whole finding — that the project merges by SQUASH so only the last export's `prev_content_hash` survives, that PRs #18/#19 *"complied BY ACCIDENT"* because their first commit was the COORD claim, that a break is *"REPAIRED FORWARD, NOT BACKWARD"*, and that re-exporting an unchanged graph cannot paper one over because the chain deliberately does not advance. **It even names the enforcement gap as [BL-156] and the mechanism — `git log --name-only origin/main..HEAD` on the pull-request trigger, where it is still free.** So "nothing enforces it" was right; "the rule this forces" was not. **WHY IT WAS UNFINDABLE IS [BL-209], and that is the more useful half: the decision is PROVENANCE-ONLY — its single edge is `AUTHORED_BY` — so `propagate_from` returns 0 impacted and the orphan detector never fires. An accepted process rule, unreachable from everything it governs, violated by the session that then announced it as new.** Size **S** | S |
| **BL-207** | **⭐ A checksum accept now ASSERTS `granularity` and `volatility` on the author's behalf — the two properties whose whole purpose is to record what only the author can say** | *Found 2026-08-04 by watching the result of the eleven accepts in #70, which is the only place it is visible.* `set_artifact_checksum` returned artifacts carrying `granularity: atomic` and `volatility: stable` — **properties the nodes did not have and nobody set.** **Measured on the merged export: exactly the 11 artifacts accepted in #70 carry them; the other 133 carry neither.** **ROOT CAUSE, and it is one layer down and one repo over:** `dynograph-core/src/schema.rs:~505` mutates the property map on validate to apply the schema default for every missing property that declares one, so **every write through `create_node` materializes defaults for properties the node never had** — the hazard `graph.rs:240` already documents ("replacing re-materializes schema defaults over everything omitted", [BL-46]). The typed setters all rebuild the full property map and call `create_node`: **19 call sites across 8 modules share the shape**, so this is not a `set_artifact_checksum` bug. **WHY IT MATTERS HERE SPECIFICALLY AND NOT AS A GENERAL GRUMBLE:** `set_artifact_intent`'s own description says these are *"the two things only its author can say"*, and [BL-188] exists **because `opaque` and `pending_expansion` are opposite states that read identically** — a decision versus unfinished work. An artifact that ought to be either now gets stamped `atomic` the first time anyone accepts a checksum on it, which is **the exact false-green BL-188 was built to prevent, re-entering through a side door.** **THE RECORD IS WRONG BEFORE THE BEHAVIOUR IS — and that is why it is easy to miss:** the defaults match the pre-existing behaviour (`stable` = any change is drift), so nothing runs differently today; only the *claim* is false. **[BL-198] IS THIS, GENERALISED** (*"a serialized default is indistinguishable from a choice — 28 properties carry exactly one value, 22 of them the schema's own default"*), and **[BL-46]/[BL-183] are the same mechanism in its other direction** (defaults overwriting values that WERE set). **The precedent for taking it seriously is already in the graph:** `req:mode-is-chosen-and-changeable` exists because `mode` was *"a governance choice asserted by a default that nobody made and nobody could revisit"* — and its own text calls mode **"the one asserted default that is actually acted upon"**, which BL-191 has now falsified, because `volatility` is acted upon too. **FIX IS A DESIGN CALL, NOT A PATCH — three shapes, none obviously right:** (a) distinguish *required-with-default* (materialize, the case dynograph's comment says it was added for) from *optional-with-default* (leave absent, apply at read) — **correct, but it is a `dynograph-foundation` change behind a pinned `v0.12.0` tag, so it crosses a repo boundary that AGENTS.md rule 6 governs**; (b) reflow2-side, have setters write only what the caller supplied — blocked, since `create_node` is the only write path; (c) decide the defaults ARE the semantics and answer [BL-198] some other way, e.g. provenance per property value. **DO NOT "fix" by deleting the defaults from the schema — absence behaving as `stable` is load-bearing.** Size **M** | M |
| **BL-206** | **⭐ The CI design-coherence gate swept 124 of 144 artifacts and reported OK over the other 20 — the false-green detector was itself false-green — DONE 2026-08-04** | *Found 2026-08-04 by not believing a red gate's finding COUNT: eight drifts were reported, and a file I had just edited was not among them.* `tools/reflow2_check.py` called `scan_nodes {node_type: "Artifact"}` and iterated the result. **`scan_nodes` answers with as many nodes as FIT and says what it withheld — `total` 144, `returned` 124, `omitted` 20, `capped_by: "size"`, `next_offset: 124` — and the gate's JSON-RPC client unwraps the `{count, items}` envelope to the items, discarding every one of those fields.** So a capped page arrived indistinguishable from a complete set, **and the gate then passed `exhaustive: true` to `reconcile_artifacts` — asserting a complete sweep that was twenty artifacts short.** The detector was never wrong: asked directly about `art:tools-coherence` it returned `checksum_change` immediately. **NOT THEORETICAL — the blind spot was already hiding real drift: `art:tools-built` changed in `d6631e5` and the gate reported OK over it twice.** Paging the sweep took the run from 8 findings to 11. **The regression test is the load-bearing part and it was checked BOTH WAYS: with the single-page sweep restored it fails `0 != 1` — the gate exits GREEN with unaccepted drift present.** Fix: `Server.scan_all` pages on `next_offset` and dies if the collected count misses `total`, because paging that silently comes up short rebuilds the same bug one layer down. **The lesson generalises past this file: every consumer of `scan_nodes` that ignores `omitted` reads a truncated design as a whole one** — [BL-205] was the same envelope being too big to read, this is the same envelope quietly being too small. Size **S** | S |
| **BL-205** | **`loop_status` — "the one cheap call" the SessionStart hook and the server instructions both push you to run — returns 72,886 characters and cannot be read in a session at all — DONE 2026-08-04** | *Measured 2026-08-04, hit twice in one session on this repo's own graph.* The server's own instructions say *"`loop_status` is the one cheap call that says what the coherence loop is owed"*, and the Stop hook nudges when graph writes finish without one. **It returns 72,886 characters, over the response limit, so the call fails and the payload is spilled to a file the agent must then parse with a script — which is the opposite of cheap, and which no skill tells you to do.** **The distribution is the whole finding: `verifications` is 72,562 of those characters — 99.6% of the payload — while every number an agent actually acts on (`clean`, `next`, and eight counters) totals ~320 characters.** In this session the entire actionable answer was `{clean:false, next:["2 structural defect(s) outstanding — run check-health"], structural_defects:2}` and everything else was zero. **The consequence is exactly the failure this repo names elsewhere: a call that is expensive and awkward at the moment the loop is most likely to be skipped is a call that gets skipped** — [BL-192]'s finding about bookkeeping under operational load, arriving through the tool built to prevent it. **Fix shape: return the counters and `next` by default and put the 106-verification detail behind a flag or a separate call, the way `scan_nodes` already solves this with `brief: true` — the precedent is in-house and the tool's own contract already promises cheapness it does not deliver.** **Worth noting for the corpus ingest: this is size-driven, so it gets worse as the graph grows, and [BL-186]'s ~756 documents grow it by roughly 1.6×.** **DONE 2026-08-04 — and found a THIRD time before the row was noticed, which is its own evidence that the call is expensive enough to re-derive rather than look up.** `loop_status` now digests the roll at the MCP boundary: counts by status, `never_run` (a `passing` with no `last_run_at` is an assertion, not a measurement), and every NOT-passing check in full — the passing remainder counted in `omitted`, never dropped in silence, the contract `scan_nodes` already states. **On this graph: 74,239 → 1,123 bytes, and the 3 checks worth reading survive.** **Deviates from the fix shape prescribed above and the deviation is deliberate:** no `brief`-style flag, because `graph_report` ALREADY serves the full roll from the same `verification_recency` computation, so a flag would be a third surface onto one list rather than a cheaper first one — the digest names where the full list lives instead. Digested at the boundary and not in core, so `graph_report` and `loop_status` still cannot disagree about what the checks say. Size **S** | S |
| **BL-204** | **The served brainstorm skill prescribes a mandatory call and justifies it with behaviour that changed a fortnight ago** | *Found 2026-08-04 by following the skill, which is the only way this surfaces.* **brainstorm** step 3 says: *"`set_decision_status` to `proposed`. **This call is not optional**: `add_decision` defaults to `accepted`, which would assert a settlement that never happened."* **`add_decision` has not defaulted to `accepted` since 2026-07-25 — its own served description says so** (*"It lands `proposed` … BEHAVIOUR CHANGED 2026-07-25: this used to default to `accepted`"*), and recording two brainstorm Decisions today confirmed it: both landed `proposed` with no second call. **The instruction is now harmless-but-false, and it is the ONLY kind of skill error that self-lint cannot catch — `skill_lint` checks that served tool NAMES exist, not that the behaviour a skill ASSERTS about a tool is still true.** **This is [BL-197]'s shape from inside the kit rather than from the tool descriptions, and [BL-183]'s from the other direction** (there, the served revise-design skill prescribed the call that CAUSED the bug; here it prescribes a call that is merely redundant — same failure to re-read the skills when a tool's behaviour moves). **Fix has two halves and the second is the valuable one:** correct the sentence, and then ask what would have caught it — a check that every behavioural claim a skill makes about a tool is restated from that tool's own description, or at minimum a rule that changing a tool's default requires grepping the served skills for its name. **Cheap to fix, and worth more as the ONE case where the kit's own instructions drifted from the binary that serves them.** Size **S** | S |
| **BL-199** | **⭐ reflow2's own graph does not follow the skill reflow2 serves — and that is the mechanism by which [BL-176] stayed invisible for eleven releases — DONE 2026-08-04** *(**and doing it corrected this row's own claim.** The counts stand — 35 document/spec artifacts, all 35 carrying `REALIZES`, 4 `DOCUMENTS` and 0 `SPECIFIES` in the whole graph — but the implication that all 32 were WRONG did not survive judging them one at a time. **7 were genuinely wrong and are re-attached with `DOCUMENTS`:** three asserted a markdown file implements a **Decision**, four claimed a capability code elsewhere implements (`docs/collaborating.md` does not implement the merge driver — `merge.rs` does). **11 are arguable and deliberately left for Anthony** — the `schema/*.yaml` files, where the skill says a machine-readable contract is `SPECIFIES` but the yamls genuinely ARE the vocabulary. **The rest are correct**: a served `SKILL.md` really does realize its capability, and 9 legitimately realize `cmp:docs` — those GAINED the missing `DOCUMENTS` edge rather than losing the true one. **THE PROPERTY THAT MATTERS NOW HOLDS: 7 artifacts stand on a describing edge alone, so reverting [BL-176] produces 7 false positives HERE — this repo can finally catch that regression on itself.** Pinned by `tools/self_host_uses_documents.py` in CI, mutation-checked against 3 seeded regressions. **`impact-check` earned its keep: it showed every affected Capability keeps a real code realizer, so nothing was stranded — `delivered` held at 83/101 across the change.**)* | *Measured 2026-08-04 by [docs/built-right-review.md](built-right-review.md), the schema-vs-graph census.* **35 document/spec Artifacts in this graph. All 35 carry `REALIZES`. 32 carry `REALIZES` AND NOTHING ELSE. The whole graph holds 4 `DOCUMENTS` edges and ZERO `SPECIFIES` edges against 183 `REALIZES`.** The **link-artifacts** skill this project SERVES is explicit that a design doc, ADR, README, runbook or agent-instruction file *"**describes** the design instead: register it with `add_artifact` (artifact_type `document`) and link it with `documents`, **not** `realizes`"* — and `art:adopt-skill`, `art:brainstorm-skill`, `art:collaborating`, `art:fleet-lessons`, `art:kit-pointer` and 27 more do the opposite. **Now put that beside [BL-176]:** `orphan_node` fires *"realizes nothing"* on artifacts attached by `DOCUMENTS`/`SPECIFIES`; the field hit it at 26 documents (defects 13→39, false positives 46%→82%) and **stopped work, expressly refusing to add a bogus `REALIZES` because it would be a lie at 756× scale.** **This graph is that exact workaround, already applied, 32 times over — not to hide anything, but with precisely that effect: the highest-volume trial reflow2 has was STRUCTURALLY INCAPABLE of surfacing [BL-176], because it never once did the thing that fires it.** `0 gaps, 0 defects, loop clean` was true and uninformative throughout. **THE GENERAL LESSON, and it is a new generator for `sharpening.md`: self-host is blind wherever the self-host graph does not follow the practice the skills prescribe — both sides of the comparison were built by the same hand. The finding took ONE query (diff prescribed practice against actual practice) and explained a hard blocker that had been invisible for months.** **The work: re-attach the 32 with `DOCUMENTS` — AFTER [BL-176] lands, because today doing so is exactly what firing the false positive looks like. That re-attachment is also what makes the [BL-176] fix regression-testable in this repo at all.** Size **S** | S |
| **BL-198** | **A serialized default is indistinguishable from a choice — 28 properties carry exactly one value across every instance, 22 of them the schema's own default** | *Same census, 2026-08-04.* `Capability.tier = operational` on 112/112, `Component.kind = module` on 54/54, `Requirement.kind/concern/lineage/provenance/designation` each single-valued on 110/110, `Fragment.fragment_type/provenance/status` on 123/123. **None of these is a statement anybody made; each is the default, materialized into every node and every export.** **The consequence is not byte bloat, it is a doctrine hole: this project enforces the confirmed-vs-asserted distinction at requirement level — that is what `graph_report`'s "Requirement certainty" line is for, and `dec:certainty-derived` argues the distinction must be DERIVABLE rather than stored — and at property level it is neither derivable nor stored. Nothing in the export can tell you whether anyone ever chose `core`.** Fix shape: **do not serialize a value equal to the default**, so an absent property honestly means *"nobody said"* — which is already what it means in every case that matters. **Check before building: `import_graph` and the merge driver both compare property maps, so suppressing defaults changes what "changed" means to `compare_designs` and to `merge=reflow2`. That is the real cost of this row and it should be measured, not assumed.** Size **M** | M |
| **BL-200** | **`provenance` is `authored` on 402 of 402 nodes — the brownfield half of the product has never once been dogfooded** | *Same census, 2026-08-04.* Every Capability, Component, Fragment, Interface and Requirement in this graph entered it `authored`. **Not one `inferred`, `reconciled`, `healed` or `imported` node has ever existed here** — and `inferred` is the load-bearing one, written by the **adopt** path. **Five code branches key on it and none is exercised by the highest-volume real graph this project has:** `report.rs:70` (`certainty_of` returns `Recovered` only for `inferred`/`reconciled`/`healed`, **so the `recovered` bucket of the "Requirement certainty" line is structurally unreachable here** — the reading that exists to keep a summary honest has never shown its honest case), `report.rs:763` (a complete thread counted `inferred_only` rather than `delivered`), `detect.rs:1195`, `heal.rs:408`, `preserve.rs:226`. **Why it matters beyond coverage: per Anthony's own calibration greenfield and brownfield are BOTH first-class, and in his working life brownfield is closer to the norm.** reflow2's own graph was authored alongside its code, so it is a genesis design **by construction** — the trial that runs thousands of times exercises the half that already worked. **This is `sharpening.md` §5's standing bias one level deeper: not "the front half of the lifecycle" but "the front half of the PRODUCT".** **The fix is not a rewrite — keep ONE adopted graph in standing rotation** (any repo nobody here wrote, run through `adopt`, re-run each release) and every branch above gets a live case. **Pairs with [BL-199]: both are self-host blind spots, and both were invisible to every instrument.** Size **M** | M |
| **BL-201** | **`Requirement.concern` is an eleven-value ility axis that NO CODE READS — the quality axis was designed twice and the first one was never wired up** | *Same census, 2026-08-04.* `Requirement.concern` — `core / logistics / sustainment / safety / security / cost / schedule / manufacturability / usability / environmental / other`, indexed, present since the beginning, described in the schema as covering *"the ILS considerations an SME surfaces"* — **is `core` on all 110 requirements, and `grep '"concern"' crates/reflow2-core/src/*.rs` returns nothing.** Meanwhile **v0.23.0 shipped `ility_report` ([BL-184]), which computes nine quality axes by mapping existing findings onto them** — and correctly reports `performance`/`security`/`scalability`/`observability` as NOT INFORMED, because nothing computes over them. **The field that would let a user DECLARE those concerns has been sitting on `Requirement` the whole time.** So: either wire `concern` into `ility_report` as declared (as opposed to computed) evidence for an axis — **carefully, since [BL-184]'s load-bearing rule is that adverse is INHERITED and never re-judged, and a declared concern is not a finding** — or retire the property under **retire-from-design** and stop advertising it through `describe_schema`. **Deciding which IS the row.** Size **S** | S |
| **BL-202** | **The typed constructors every skill prescribes reach 44% of the declared properties — so the effective schema is the constructor signatures, not `schema/*.yaml`** | *Same census, 2026-08-04.* Across the 13 node types that have a typed constructor: **94 declared properties, 42 reachable through it. 67 of the 208 properties in the whole vocabulary are named by NO request struct anywhere in the 142-tool surface** — reachable only through the generic `create_node`, which takes an arbitrary schema-validated property map and **which no skill prescribes.** **Two known rows are symptoms of this single cause: [BL-129]** (`Interface.medium` unreachable from both tools that create and specify Interfaces) **and [BL-183]** (16 of 18 constructors silently reset every property they did not name, *and the served revise-design skill prescribed the call that did it*) — **that bug is only possible when a constructor's field list is a proper subset of its type's properties.** Fix shape: **one write path per node type taking the full property map, with the ergonomic arguments as sugar over it rather than as a second surface.** The typed constructors were an ergonomics decision that hardened into a lossy second schema; the generic `create_node` is the honest surface and it is the one nothing documents. **Size L and it is a surface change — it moves toolsnaps and wants its own increment.** Size **L** | L |
| **BL-203** | **Nine node types and thirty-five edge types have never been instantiated — and a computation written over an unused type reads an empty set in silence** | *Same census, 2026-08-04.* **29 node types and 60 edge types declared; 9 node types (31%) and 35 edge types (58%) have zero instances in this graph.** Domain by domain the shape is sharper than the percentage: **`dimensions` is entirely inert (both node types), and so are `environment`'s `EnvironmentRule`, `readiness`'s `ReadinessAssessment`, `temporal`'s `TemporalFact`, `verify`'s `QualityGate` and `functional`'s `Flow` — six of eleven domains with a dead centre.** **This is [BL-184] generalised: that row found `DimensionAssessment` count = ZERO and, with it, that `dimension_drifts` had been computing a slope over an empty set silently for eleven releases. The census says that was not a one-off — it is the expected condition of a third of the vocabulary, and any computation over an unused type inherits the same defect with nothing in the build to say so.** **THE HONEST COUNTERWEIGHT, and it must survive into whatever gets built: an unused type is NOT automatically a mistake.** `EnvironmentRule` is for regulated deployment-heavy designs, `Flow` for behavioural ones; reflow2 is neither, and a design tool whose vocabulary covered only its own shape would be useless. **The defect is that nothing distinguishes "unused because inapplicable here" from "unused because unreachable", and no computation over an empty population is ever challenged.** Actionable half: **make an unexercised type visible rather than silent** — a report or a gate that names every declared type with zero instances and every computation reading an empty population, so [BL-184]'s eleven-release silence cannot recur. **The other half — whether the vocabulary should have been grown on demand from real subjects rather than declared up front across eleven domains — is a standing judgement about how the schema evolves and is Anthony's, not a row.** Size **M** | M |
| **BL-186** | **`cap:corpus-ingest` is `planned` while both the requirement and the mechanism that specify it are `accepted` — the one row here that is a BUILD, not a design question — DONE 2026-08-05** *(`ingest_corpus_step` + the served `ingest-corpus` skill. One epoch per RUN not per document; identity converges across documents; the near-match band asked ONCE across the corpus; and the handshake batches so a corpus costs the ~3 rounds a single document costs rather than 3N — which needed no new mechanism, because a prompt id was already a content hash. Re-running SKIPS what landed before, so resume is derived from the graph rather than bookmarked. Every unreadable document is NAMED. Convergence stays LEXICAL and the check's evidence scope pins `corpus_size` and `vocabulary_overlap` to say so — the first VERIFIES edge in this repo to carry a scope at all, against 116 that never did.)* | *Field report from the `dev_storyflow` fleet's api-boss (`07426210`), 2026-08-04, asking "how do we mass-ingest hundreds of documents". The answer was already in this graph.* `req:corpus-ingest` **accepted** · `dec:corpus-ingest-mechanism` **accepted** (*"a folder of documents goes through the ingest handshake, not a skill"*) · `cap:ingest` (single text) **verified** · **`cap:corpus-ingest` `planned`, nothing built.** The design settled this and nobody built it. **AND THE RESUME STORY IS ALREADY ANSWERED, which is worth recording because it looked like the hard part:** Anthony asked how a 1000-document ingest resumes after a context reset. **There is no bookmark and there should not be one — the graph IS the resume point.** Each document becomes an Artifact with a checksum, so "what is done" is DERIVED, not recorded: sweep `git ls-files`, diff against registered Artifacts, the remainder is the work left — which is exactly `coverage_report`'s unclaimed regions. **Two things break that derivation and both are on this list: [BL-188]** (an opaque directory Artifact claims its whole subtree, so the sweep goes green over invisible files) **and [BL-183]** (before it shipped a re-registration ERASED `checksum` and `last_confirmed_at`, so resume was not idempotent — fixed at v0.23.0). **Order is therefore forced: [BL-176] to ingest at all → [BL-188] to know where you stopped.** Size **M** | M |
| **BL-187** | **The corpus records WHEN something was true and nothing computes whether it still is — two folders, 1,300 files, one hole, and EIA-649 names it: STATUS ACCOUNTING** | *Same field report, 2026-08-04, and the strongest cross-cutting finding in it — measured, not asserted.* Of 195 evidence writeups, **150 pin a commit SHA — they are honest about their anchor — across 175 distinct commits. Median pin is 117 commits behind main, oldest 1,885. Only 90 of 175 (51%) are still ancestors of `origin/main`**: squash-merge replaced the branch commits, so half this corpus names trees that can no longer be rebuilt. The evidence stays true about what it tested; **you can no longer reconstruct what it tested, and nothing anywhere computes the distance** — a reader sees a confident PASS with no signal it was proven 117 commits ago. **The same hole, twice more in this repo's own reading: 11 verifications sitting green on one batch run all stamped `2026-07-25T18:49:11Z`, and `maturity_report`'s assurance band counting `passing` while saying nothing about WHEN.** Candidate: an external anchor — `anchor_kind` (`vcs_commit` / `release` / `image_digest`) plus `anchor_ref` — on `TemporalFact` / `Verification` / `Artifact`, so a report can say *"passing, anchored to a commit 117 behind main"*. **reflow2's time axis is INTERNAL (`DesignEpoch`, which someone must create and remember to close) and this evidence anchors to EXTERNAL reality, which is more objective and which the graph cannot hold.** **Withdrawn on the way here and worth keeping on the record: the reporter proposed an `Attestation` node type off a three-file sample, read deeper, and killed it themselves — `TemporalFact` already expresses "a claim that held at a time".** **CAUTION FILED WITH THE ROW:** the same report claims reflow2 is *"already ~80% of the way there"* on ISO/IEC/IEEE 15289 — **the one place it drops its own evidence standard, a conformance claim asserted rather than measured. Compute the mapping before that is said anywhere public.** Size **M** | M |
| **BL-188** | **A directory Artifact makes a registration check report GREEN — DONE 2026-08-04** *(`granularity`: `atomic`/`opaque`/`pending_expansion` on Artifact, and `coverage_report` now returns `pending_expansion` and `opaque_claims` SEPARATELY — the whole point, since a settled archive and an untouched backlog are opposite states that read identically. Makes "53 artifacts, of which 3 stand in for the rest" producible from the graph at all. **No count stored on purpose** — reflow2 does no file IO, so a stored figure would be exactly the un-recomputable staleness [BL-187] names. Read from the node, never inferred from the path: no amount of looking at a tree recovers whether opacity was a decision.)* while 359 files under it are individually unreferenceable — and nothing distinguishes "deliberately opaque" from "nobody has got to it yet"** | *Same field report, 2026-08-04.* A check asserting every live doc is an Artifact returned **"✅ every live doc is registered"** — truthfully, because `docs/` is modelled as ONE Artifact with `location: docs/`, and the **adopt** skill's own rule is that a directory artifact claims everything beneath it. **The green is correct at directory granularity and completely wrong for the question actually being asked** ("can I find and reference the doc about X"). `coverage_report` has the identical shape — it counts a directory path as claiming its whole subtree, and the skill says so explicitly. **So the two instruments that exist to say what the design does not cover both go green on a subtree modelled as one opaque unit, and opposite states — settled archive vs untouched backlog — produce identical readings.** **This is [BL-170]'s shape returning (a hidden inclusion list making a region invisible), and `granularity_report` would NOT catch it:** it counts capabilities-per-artifact, so a `docs/` node realizing one capability looks perfectly ordinary. Candidate: let an Artifact declare its granularity intent — `opaque` (vendored mass or settled archive: subtree claimed, do not descend) vs `pending_expansion` (a placeholder standing in for N files that should each become a node) — so a report can honestly say *"53 artifacts, of which 3 are placeholders standing in for 756 files"*, **a sentence that cannot be produced from the graph at all today.** **This is the resume MECHANISM for [BL-186], not a nicety.** Size **M** | M |
| **BL-189** | **95 self-corrections against 7 retirements across 334 memory files — a corpus that corrects itself and never retires itself** | *Same field report, 2026-08-04, exact greps over all 334 non-satellite files.* 95 files (28%) carry a self-correction or supersession; **7 declare themselves resolved.** So a warning about a bug fixed in June still reads as live today with nothing to say otherwise — **the same shape as the evidence-OBE finding in [BL-187], in a different folder, which is what makes it a lifecycle gap rather than a filing habit.** reflow2 has the retirement half already — the **retire-from-design** skill and its one load-bearing question, *was this ever true?* — but **nothing prompts it, nothing measures the imbalance, and `loop_status` has no notion of a record that has outlived its subject.** **A second measurement in the same sample is the most actionable thing in the whole report and needs NO schema at all: 102 of 334 files (31%) declare themselves FUTURE / parked / unbuilt** — product intent sitting in markdown that reflow2 models perfectly today as Requirements and Capabilities at `proposed`/`planned`. **The biggest single category in that corpus is not a missing type; it is un-ingested design intent**, which feeds [BL-186] directly. Size **M** | M |
| **BL-190** | **"This role is ACCOUNTABLE for that component" is inexpressible — and the only edge that says ownership IS the decomposition spine** | *Same field report, 2026-08-03, modelling the fleet's own governance.* `describe_schema{from:Component,to:Component}` returns **`CONTAINS` as the only modelled fit**, hinted as *"Ownership/containment ... the axis-Y decomposition spine (see Component.level / matryoshka)"*. Using it for stewardship would make a boss **contain** a microservice and corrupt `hierarchy_issues` — one of the detectors the fleet most wants to trust. **Per AGENTS.md (*"if nothing models the relationship, leave the edge out rather than asserting one that is wrong"*) the reporter left it out, and that is the right call producing the wrong outcome: the ownership matrix, their most load-bearing governance artifact, is simply silent in the graph — and a relationship nobody records is indistinguishable from one that does not exist.** Consequence stated precisely: **reflow2 cannot answer "who owns this blast radius", which is the question a multi-agent team most needs of an impact report.** Candidate: an accountability/stewardship edge (Contributor or role → any node) explicitly NOT part of the decomposition spine and NOT a traceability edge — the way `AUTHORED_BY` and `CLAIMS` are already deliberately excluded from impact propagation, so the precedent exists in-schema. **Schema touch: worth landing with [BL-185]/[BL-143] if those go, since one schema change pulls the minor bump, the upgrade doc and the foundation-migration checklist and two do not cost twice.** Size **M** | M |
| **BL-191** | **Checksumming a live append-only file fires `checksum_change` forever — DONE 2026-08-04** *(`volatility`: `stable`/`append_only`/`living`. A content change on a volatile artifact reports as the new `expected_change` kind and is NOT written to the drift ledger, ending the forever-owed disposition. **Reported, never suppressed** — silence would trade a false positive for a false negative and let a wholesale replacement pass unmentioned, the trap [BL-176] avoided. **Absence still fires at full severity.** **SHRINK DETECTION DELIBERATELY EXCLUDED and this is the honest half:** a checksum cannot express a shrink, it needs a size baseline nothing records, and the reporting team's own compaction shrinks those files on purpose — so the heuristic needs the design's consent, not a bare rule. Filed as still-open in the row above rather than half-built.)*, and a SHRINK — the genuinely alarming event — cannot be expressed at all** | *Same field report, 2026-08-04.* Five coordination buses were modelled as Artifacts with checksums, exactly per the **adopt** skill's *"a checksum is what makes later drift detectable"*. **Within minutes `reconcile_artifacts` reported 5 `checksum_change` divergences — all correct, all meaningless**, because those files are appended to continuously by design. Disposed as `design_holds`, **a ritual owed again on every single reconcile forever, and the noise will bury a real drift when one appears** — the signal-trained-to-be-ignored failure, arrived at by following the skill correctly. Candidate: let an Artifact declare expected volatility — `stable` (a source file, any change is drift worth reporting) vs `append_only`/`living` (a log, a bus, a changelog, where only ABSENCE or a shrink is interesting). **The reporter's own caveat is the interesting half and should survive into the design: their bus compaction deliberately shrinks these files, so even "a shrink is alarming" needs the design's consent rather than a bare rule.** Size **S** | S |
| **BL-192** | **Recording ONE change event took six rejected calls — every rejection individually correct, not one of them predictive** | *Same field report, 2026-08-03, recording a container recreate against 3 nodes: a 3-minute bookkeeping task that cost ~8 round-trips.* In order: `get_node{node_id}` → *"unknown field node_id, expected node_type or id"* (**the field is `node_id` in every RESPONSE and `id` in the request**) · `set_verification_status{evidence}` not a field · `record_change{affected_ids}` not a field · then *"missing change_event_id"*, revealing that **`record_change` LINKS an existing event while `add_change_event` CREATES one — which neither its name nor its description said** · `{affected: ["cmp:embeddings"]}` → *"expected struct AffectedNodeReq"* **without saying what that struct is** · `{affected:[{node_type,id}]}` → *"unknown field id, expected node_type, node_id, action"*, so **the nested object wants `node_id` while the parent wants `id`, the exact inverse of `get_node`** · `{change_type:"modify"}` → *"unknown change type"* **with no list of legal values**. Only `describe_schema` ended it, returning ~8k tokens of edge documentation to answer *"what are the 10 legal `change_type` values"*. **The cost lands exactly where it hurts most: at the moment the loop is most likely to be skipped under operational load — the failure `PROTOCOL.md` names as "doing the bookkeeping without the loop".** Fixes, cheapest first: **(1) enum rejections list the legal values** — this alone kills three of the eight; **(2) field-name rejections name the accepted spelling FOR THAT POSITION**, the `id`/`node_id` inversion being the single biggest trap; (3) `record_change`'s description says it links an existing ChangeEvent and points at `add_change_event`; (4) a `describe_schema{properties_only:true}` that returns the property table without the full edge catalogue. Size **S** | S |
| **BL-193** | **An Interface CONSUMED across a component boundary with NO recorded spec makes the seam uncheckable — and no gap fires** | *Same field report, 2026-08-03, run on the `dynograph → foundation` seam before an engine bump. `detect_gaps` returned silence.* The graph held `cmp:dynograph —CONSUMES→ ifc:foundation_api` with a name, a description **and nothing else** — no operations, no payload surface, no error model — **so a release could change every signature behind that edge and the loop stays green.** The seam had to be captured by hand (`set_interface_spec`) plus an external shell script written to make it checkable at all. **This lands squarely on reflow2's own frontier and that is the strongest argument for it: `maturity_report` puts seams at 0% — 0 of 73 component couplings declared through a contract — and 23 releases have shipped over that reading.** Candidate: a gap for *"an interface is CONSUMED across a component boundary but carries no spec."* **The reporter's framing is the part to keep: *"a check reduced to 'an edge exists' can only ever detect the edge disappearing."*** Size **M** | M |
| **BL-194** | **`open_questions` returning 0 reads as "nothing owed" — and it is the orientation call the SessionStart hook tells every new session to run FIRST** | *Same field report, 2026-08-03, and reproduced in this repo while filing these rows.* In the field: `open_questions` at session start returned `{"count":0,"items":[]}`, and **`loop_status` in the very next call reported 11 structural defects, 3 capabilities claiming realized with no passing check, and 17 built capabilities never checked against reality.** **Reproduced here 2026-08-04 — `open_questions` returned 0 while `loop_status` reported 2 outstanding structural defects: the same shape at smaller scale, so this is not a property of one messy graph.** Both answers are true and they answer different questions; the problem is purely that **the call a session runs first is the one that looks like an all-clear**, and the hook's own wording points at it. Fix is small and needs no schema: have `open_questions` name the other outstanding counts when they are non-zero — even *"0 questions; `loop_status` reports 31 other items"* — so an empty result cannot be read as an empty loop. Size **S** | S |
| **BL-195** | **`disconnected_community` suggests `generate_bridge` at a requirement a RULING forbids anyone to satisfy — the detector proposes exactly the forgery the Decision exists to prevent** | *Same field report, 2026-08-04.* `req:stateless_scaling` is reported as a 4-node disconnected cluster. **It is disconnected on purpose:** a recorded user ruling states that no `SATISFIES` may be drawn to it, because a requirement backed out of its own implementation is satisfied by construction and can never contradict anything. **The suggested fix is `generate_bridge` — an agent working the defect list top-down would take it and forge the edge the ruling forbids.** The reporter closed the reachability half honestly instead, recording `VIOLATES` at the named break-point so the hole sits in that component's blast radius; **the cluster is still reported, presumably because risk edges are excluded from the structural view — which is itself worth confirming.** **This is [BL-179]'s punish-correct-work trap and [BL-176]'s shape from a third direction: declaring the truth in the vocabulary provided does not stop the alarm.** Candidate: let an acknowledgement or the governing Decision mark an intended-unsatisfied state, so the detector reports *"unsatisfied BY RULING (see `dec:…`)"* rather than a structural defect with a bridge suggestion. **The Decision that governs the unsatisfied-ness is already in the graph; the detector simply cannot see it.** Size **S** | S |
| **BL-196** | **`genesis` never asks where design artifacts will live — and after fifteen months that conversation is archaeology, not a decision** | *Same field report, 2026-08-04, and the user framing behind it: every large design effort accumulates a mountain of docs and test reports in a folder nobody can find again, especially on old projects where whole teams have come and gone.* **The case study is measured rather than asserted, which is what makes it usable: one 15-month project, one continuous team, a named librarian, a filing map, an archive convention and a generated catalog — and it still lost half its memory.** `memory/`: 333 files, 169 in the index, **164 unreachable (49%)**, 10 index lines pointing at nothing, and `MEMORY.md` at 20,446 bytes against a ~17KB read limit **so trailing entries drop silently while the file still looks complete**. `docs/`, which has **no central index at all** and is navigated by inbound links between documents, has **zero** unreferenced files. **The mechanism generalises and it is the argument for a graph over an index: a curated index has to be read IN FULL to be useful, so it has a hard ceiling; a queryable graph does not.** Proposal: `genesis` already captures deployment and platform context — it should also ask *"where will design artifacts live, and what kinds will there be?"* and record the answer as a Decision plus a small artifact policy. **Every project reaching for `adopt` because its docs are unfindable is a project that never had this conversation.** **The caution belongs in the row, not underneath it: "assign a dedicated librarian" is in every best-practice list and this project DID it — the role exists, is named in the ownership matrix, and has written doctrine. The sweep still rotted for 20 days and the index still overflowed. Judge any framework adopted here on what it makes MECHANICAL, never on what it assigns to a person.** Size **M** | M |
| **BL-197** | **Two served descriptions describe a world that has changed, and both mislead in the direction of "do it the old way"** | *Same field report, 2026-08-03/04.* **(a) `claim_region`'s own description still says the design *"lives as a file in each person's checkout with no shared server to hold a lock"*.** Since `--shared` shipped there IS a shared server and every session attaches to it, so the tool's description now argues against what the fleet's protocol binds; the worker who hit it knew better, **the next one will read it as current.** It should describe advisory claims under the shared server and treat the file plus merge-driver path as the cross-clone case rather than the default. **(b) `import_graph` warns honestly that a hand-authored document *"had no stamp"* and that provenance and the upgrade-direction check therefore cannot be run — but nothing says whether a hand-authored document is ALLOWED to claim one**, and the stamp's minimal shape is discoverable only by exporting a graph and reading the result. **Every `adopt` pass — the exact workflow that produces hand-authored documents — silently forfeits the check you most want on the day someone imports an old document into a newer binary.** Fix: either accept a minimal `{reflow2_version, schema_version}` stamp and say so in the warning, or state plainly that hand-authored documents cannot be stamped and the check is unavailable to them. **Related, known, still unactioned: this repo's own `.mcp.json` does not pass `--shared`** (it is `["--graph-path", "./.reflow2/graph"]` only) while every config `reflow2_init.py` generates does — **the sole reason the stale-server export replay is hand-rolled at every release cut.** Size **S** | S |
| **BL-184** | **What the graph computes now reaches the axis it informs — and reflow2's own dimension enum turned out to have NEVER been written to — DONE 2026-08-03** | *Anthony, correcting my claim that the maturity bands were "judgement rather than computation": **"I'm not sure a blanket yes or no can be applied here. I feel like there are some areas where the graph may inform or give a computed signal that indicates one of the 'ilities' are not sufficiently being met by the design."*** **HE WAS RIGHT, AND THE MEASUREMENT IS STARKER THAN THE ROW CLAIMED: `DimensionAssessment` count in reflow2's own design is ZERO.** Nine distinctions, no instances, and the sole computation over them (`dimension_drifts`, a slope over asserted scores) therefore reads nothing — `dec:edge-orthogonality` at its bluntest. Meanwhile modularity, articulation points, cycles, misplaced capabilities, hierarchy mismatches, surprising couplings, granularity and the trajectory bands were all being computed and connected to no axis at all. **`ility_report` connects them; it computes nothing new**, which was the point — the finding was never that reflow2 cannot measure these, but that it measures them and says nothing about what they are evidence of. **NEVER DERIVES A SCORE**, and the precedent is Anthony's own: `dec:readiness-is-an-observation-the-threshold-is-the-judgement` kept TRL out of that same float because *"TRL 7 as 0.78 asserts a precision the ladder does not have"*. **ADVERSE IS INHERITED, NEVER RE-JUDGED — the load-bearing rule:** a finding counts against an axis only where the computation that produced it already calls it a defect. A ratio, a trajectory position and a granularity observation are **context**, because relabelling them would overrule modules that deliberately refused to grade *and* smuggle back the thresholds they were built without. **NOT BLANKET, which was his actual point:** `performance`, `security`, `scalability` and `observability` report NOT INFORMED with the reason rather than reading clean. **THE ASYMMETRY IS DELIBERATE:** an asserted good score over adverse evidence is flagged; an asserted LOW score over a quiet axis is **not**, because "no detector fired" is an absence of evidence. **reflow2's own run: 0 adverse across every axis** — no cycles, no SPOFs, no misplaced capabilities, no hierarchy issues — with seams 0.0% and three granularity outliers correctly reported as context rather than charges. `tests/ility.rs` (11), mutation-checked against five regressions: **four caught, and the fifth was INERT and exposed a MIS-NAMED TEST rather than a hole** — deleting the `HealSeverity::Info` guard fails nothing because all four mapped categories are Warning-or-Critical by construction, so the test was renamed to what it actually proves and the guard's unreachability is stated at its site. **No schema change; the stamp does not move.** Size **M** | M |
| **BL-185** | **A test written from the requirement and a test written to match the code are indistinguishable in the graph — and this repo records the difference in PROSE** | *Anthony, 2026-08-03, on test-driven design: **"Developing tests as you design is extremely important. It shapes system architecture, forces clear thinking about edge cases early, and prevents the tight coupling that makes retrofitting tests painful."** His named risk is the one that bites here: **"Confirmation bias: Testing code you already wrote proves what it does, not necessarily what it should do."*** **MEASURED: `Verification` carries `criteria, kind, last_run_at, level, method, name, status` — no field for when the check was authored, and none for whether it preceded the code.** Yet **7 of reflow2's 105 verifications assert in their NAME that the test came first** (`ver:constructors-preserve`: *"WRITTEN BEFORE THE FIX: all 6 defect cases failed and all 3 counterweights passed"*), two of them written on 2026-08-03. **So the repo already practises test-first, already records it, and records it where NO COMPUTATION CAN READ IT** — [BL-143]'s shape (an opinion smuggled in as data) and `dec:edge-orthogonality`'s, from a new direction. **The consequence is precise: a `passing` check written from the requirement and a `passing` check written to match the implementation are the same node, same status, same everything.** reflow2 asserts `dec:passing-is-verified` — *verified means a check that passes* — while being unable to say what the check was evidence **of**. **THE GUARD RAIL IS ANTHONY'S OWN CAVEAT — *"may not apply to or be feasible for all project types."*** The **adopt** skill exists entirely for systems that already ship, where every recovered test is necessarily after the fact; a reading that scored that as failure would repeat exactly [BL-179]'s punish-correct-work trap. So: **record the fact, report the signal, never grade it.** **Fix shape, and it pairs with [BL-143]'s already-decided `criterion` field on the same node type** — one schema touch rather than two, which matters because a schema change pulls in the minor bump, the upgrade doc and the foundation-migration checklist. Candidates: an optional `authored_at` (order derivable against the Artifact's own dates), or an explicit provenance-of-evidence enum saying whether the check was written from the intent or from the implementation. **Deciding which is the row.** Size **M** | M |
| **BL-183** | **Sixteen of eighteen constructors were silently erasing the user's word — and the SERVED SKILL prescribed the call that did it — DONE 2026-08-02** | *Walked into while correcting `dec:bl83a-functional-decomposition`: `add_decision` on an existing id reset `status: accepted` → `proposed`. Caught only by reading the result back.* **THE AUDIT IS THE ROW, AND IT WAS WORSE THAN THE INSTANCE.** Mechanically comparing each `add_*` helper's named properties against its node type's schema: **16 of 18 constructors** name a subset and write create-or-**replace**, so re-calling one resets every unnamed property to its default. `add_interface` loses **13**, including `designation` — how the design says which contracts are the **published boundaries** that computations read (`req:key-interfaces`). `add_requirement` loses `status`, which is where certainty comes from (`dec:certainty-derived`), so rewording a requirement **un-confirms it and forges the user's signature in reverse**. `add_capability` un-builds a `verified` capability. `add_artifact` is **[BL-166] still reachable one call away** — that fix landed on `link_artifact` only. **WHY IT SURVIVED THREE ENCOUNTERS ([BL-46] → [BL-166] → here): it is invisible until a property has been moved OFF its default**, so on freshly-captured nodes the reset is a no-op. `describe_schema {required_only}` confirms only `name` is required on most types — everything else has a default to fall back to, silently. **IT VIOLATED `req:no-silent-fallback` — accepted, priority CRITICAL, *"Failures and skipped work are surfaced loudly, never swallowed"*** — which `propagate_change` named in the distance-1 ring rather than anyone recalling it. **THE FINDING WORTH MORE THAN THE FIX, and the blast radius is what surfaced it** (`cmp:skills` sits on the far side of `ifc:mcp-tools`): **the served revise-design skill told you to revise with the generic `create_node` and asserted "an existing id **merges**". It does not — it replaces.** Verified empirically, not read: following the skill literally downgrades an accepted requirement. So the instruction *caused* the defect it was documenting away, which is [BL-140]'s shape — declaring the truth in the vocabulary provided does not stop the alarm — inverted: following the guidance provided caused the fault. **FIXED:** all 18 constructors route through `upsert_node`; `create_node` and `import_graph` deliberately still replace, because an import document is a **complete** statement of a node and merging there would resurrect properties it meant to omit (this is also why `dec:import-merges-or-demands-whole-nodes` stays open and untouched); the skill and both mirrors now say use the typed constructor and warn off `create_node`; two tool descriptions state the merge contract. `tests/constructors_preserve.rs` (9) — **written before the fix, all 6 defect cases failed and all 3 counterweights passed**, the counterweights being that creation still writes defaults, a named property is still overwritten, and an explicit optional still wins, so "merge" never became "you may never change this back". Size **M** | M |
| **BL-182** | **reflow2 could not have found the monolith itself — now it can, and it would have said so at v0.21.0 — DONE 2026-08-02** | *Anthony, 2026-08-02, on being offered the `service.rs` split: **"I want this to be something that reflow2 natively discovered - not me pointing this out by my own observation."** Then, pressing harder and correctly: **"I'd imagine that it'd be something that the agent would have to look for a principle of good design practices - it is kind of a subjective ask."*** **HIS OBJECTION IS WHAT MADE THE ANSWER NARROW ENOUGH TO DEFEND.** "Avoid monoliths" *is* subjective, there is no design-derived answer to *how big is too big*, and reflow2 cannot read code — so none of that is what is measured. What is measured is a **disagreement between two records the design already holds**: the design asserts N distinct Capabilities, the build asserts one Artifact. **That is the same shape as the rest of the family** — `compare_designs` finds design-vs-design disagreement, `reconcile_artifacts` finds design-vs-disk, and this finds design-granularity vs build-granularity — and like all of them it **rules on neither side** (`dec:report-dont-judge`): N capabilities in one file may mean the file should be N files, or that the design over-decomposed, or that it is right for this phase. **NO SIZE THRESHOLD, BY CONSTRUCTION.** Artifacts are compared against *this design's own distribution*, which is what keeps it safe under [BL-179]: a breadboard design where everything lives in one file has no outlier and is told nothing, and `a_uniformly_coarse_design_is_not_punished` pins that as an executable guarantee. It speaks only once a design has decomposed elsewhere and left one place behind — a position on the trajectory, not a score. The precedent was already next door: `surprises` excludes `PROVIDES`/`CONSUMES` because every properly-modelled contract would read as a sole bridge, *the design discipline penalising itself*. **THE PROOF THAT IT IS DISCOVERY AND NOT A RULE FITTED TO A KNOWN ANSWER: run against the v0.21.0 export — weeks before anyone said anything — it names `art:service` alone, 9 capabilities, z=5.75.** On the committed design: 10 capabilities, z=6.46, `at_or_above=1`, out of a population of 55 whose median is 1. **A REAL STATISTICAL WEAKNESS FOUND WHILE WRITING THE TESTS, not after:** counts pile up at 1, the standard deviation collapses, and the distributional cutoff alone fires on an artifact holding *two* capabilities — true, trivial, and suppressed by a stated `MIN_DISTINCTIONS` floor on how many distinctions the DESIGN draws (never on size). z-masking is recorded as a known property too (`several_coarse_artifacts_mask_each_other`), so it reads as a prompt to look, never as a count of how many there are. **Wired into `graph_report_markdown`, because a tool nobody calls is still invisible** — which was the whole complaint. Silent about: unregistered artifacts, size of any kind, and which side is wrong. Size **M** | M |
| **BL-181** | **The tool surface is carved into the systems the design already named — and the whole loop closed on itself — DONE 2026-08-03** | *Anthony spotted `service.rs` by eye, then refused the offered fix: **"I want this to be something that reflow2 natively discovered."** [BL-182] built that discovery; this is the answer to it.* **THE SPLIT:** 141 items moved verbatim into **11 per-domain modules**, each declaring its own `tool_router`, summed in `ReflowService::new`. `service.rs` **6,356 → 2,658 lines**. The carving follows `dec:bl83a-functional-decomposition` — *"reflow2's systems are functional, not its file tree"* — because a file tree that disagrees with the design's own decomposition is precisely what this existed to fix. Router composition was already proven in-repo (`skills.rs`), so this was addition, not invention. **THE FULL LOOP, AND THIS IS THE POINT:** `granularity_report` **found** it (`art:service`, 10 capabilities, z=6.46, alone in the design, and it would have said so at v0.21.0); `toolsnap` proved the served surface **byte-identical — all 144 match**; `certify_preservation` returned **`PRESERVED`** — 0 function changes, 0 unclassified, 36 structural, 30 supporting; and `granularity_report` re-run shows **`art:service` gone from the report entirely.** Found → fixed → proved safe → confirmed resolved, by four instruments that did not exist yesterday morning. **A DOCUMENTED LIMITATION SHOWED UP AS REAL BEHAVIOUR, which is the best evidence the caveat was honest:** the report now names `art:temporal` (4 capabilities, z=4.54), `art:detect` and `art:graph` (3 each). `art:temporal` was previously **z=1.94, below the cutoff and invisible** — it surfaced only because removing service.rs's 10 stopped inflating the spread everything is measured against. That is exactly the z-masking pinned by `several_coarse_artifacts_mask_each_other` and printed in `not_observed_about`: *a prompt to look, never a count of how many there are.* **THOSE THREE ARE NOT THIS ROW** — they are the next lap, and deliberately left. **Mechanical cost of the split:** 28 free helpers and 5 methods in `service.rs` became `pub(crate)`, and the request structs stayed put (the wire types are a coherent set; moving them is a second increment worth doing separately). Size **L** | L |
| **BL-180** | **A restructuring is safe exactly when function is provably preserved — and nothing computes that, so today it can only be asserted** | *Anthony chose this, 2026-08-02, as the first step under [BL-179] / `dec:maturity-restructuring-delta` — of the two offered (this, or fork-and-measure on the 14 seams).* **The invariant, from the decision:** a maturity restructuring holds the function set INVARIANT and moves everything else, so it is safe exactly when **no Capability was added or removed, no Requirement was added or removed, and no `SATISFIES` edge changed** — only `ALLOCATED_TO` / `CONTAINS` / Interface edges moved. **Why it is buildable now rather than invention:** `compare_designs` already diffs two as-designed records at node and edge granularity, directionally and deterministically; what is missing is the classification that turns a diff into a **verdict**. **The move it makes is `dec:passing-is-verified`'s, applied to structure** — the difference between a refactor someone hopes is safe and one the graph checked. **Where it must not cheat, and this is the whole risk:** a classifier with a hidden inclusion list would call a divergence it does not recognise "structural" and certify a design it never examined — [BL-170]'s fourth quadrant exactly. So the default is **unclassified → `indeterminate`**, never a silent pass, and an exhaustive test over the schema's own vocabulary fails the build when a new node or edge type lands with no class. Property changes on function-bearing nodes are `indeterminate` too: a reworded description may be a rename or a scope change, and the graph cannot tell — `dec:three-party-checks` says it counts and the human judges. **What it deliberately does NOT claim:** it compares two design records, so it says nothing about whether the code preserved function. Size **M** | M |
| **BL-170** | **The sweep scope was a hidden inclusion list, so a region nobody named was silently out of scope — DONE 2026-08-02** | *[BL-165]'s root cause, generalised: `build_design_graph.py`'s sweep named two globs, so `schema/` was not excluded — it was never considered.* **The quadrant that hid it:** `coverage_report` compares what the caller swept against what the design claims, which answers three cases and is structurally blind to the fourth — **neither swept nor claimed** is mentioned by neither input. `unobserved_locations` looks like the field that catches "did I forget to sweep this?", and cannot: it only names regions the design *already* claims. **Fixed by inverting the default** — the sweep is now everything `git ls-files` tracks, minus four **named** exclusions echoed back with their rule, so a new region is in scope automatically and what is left out is stated (rule 6). The artifact-level inclusion list stays, deliberately, for a different job: those paths are expected to carry an Artifact so `reconcile_artifacts` can compare checksums. **First run: 123 of 430 tracked files claimed, 142 unclaimed regions** — ranked by mass with the display cap stated, not silent. It found [BL-171] immediately. **The general form is NOT built and is Anthony's call**, recorded as `dec:coverage-scope-is-declared` (proposed) with three shapes and their costs: a property on Project/Component, a first-class node recording a sweep and its scope (which would also close the persistence deferral in `cap:coverage`), or derive-from-VCS as done here — which assumes a repo and so says nothing for a design whose subject is a fleet, a document set, or hardware (`req:design-anything`). **The correction that belongs with it:** `docs/requirements-coverage.md` defers this with *"reflow2 cannot enumerate a filesystem it never reads"*, which is over-stated — `identity.rs` and `provenance.rs` in the core read and write files, as do `nudge.rs`, `latent.rs` and `main.rs`. The honest statement is narrower and still sound: the **analysis** never reads the subject system. Size **S** | S |
| **BL-176** | **`orphan_node` calls an Artifact that DOCUMENTS-but-does-not-REALIZE an orphan — penalising exactly what the link-artifacts skill prescribes for instruction files — DONE 2026-08-04** *(the rule is now the EXCLUSIONS — an Artifact is an orphan only when every edge it carries, either direction, is bookkeeping (`INCLUDES`/`CHANGED`/`YIELDED`/`AT_EPOCH`). Naming the attaching edges is what broke: an inclusion list has to be extended as the vocabulary grows, and [BL-170] was the same shape. **Deliberately NOT degree-zero** — a Release `INCLUDES` sits on almost every artifact in a mature graph and would silence the detector everywhere. `tests/orphan_attachment.rs`, 6 cases: **the 2 defect cases failed and all 4 counterweights passed before the fix**, which is what proves the counterweights were not passing by accident of the bug. **[BL-199] is the honest limit: this repo goes 0 → 0 across the change and CANNOT demonstrate its own fix**, because all 139 artifacts carry `REALIZES`. The rule shape is recorded for Anthony as `dec:orphan-attachment-is-an-exclusion-list`, `proposed`.)* | *Found 2026-08-02 doing [BL-174], by registering `getting-started/AGENTS.md` the way the served skill says to and watching a warning appear for it.* The **link-artifacts** skill is explicit: *"A design doc, ADR, README, runbook or agent-instruction file (AGENTS.md, CLAUDE.md) **describes** the design instead: register it with `add_artifact` (artifact_type `document`) and link it with `documents` (+ `doc_kind`), **not** `realizes`."* Follow that literally and `detect_defects` returns `orphan_node — Artifact 'x' realizes nothing`, **`severity: warning`, with a `DOCUMENTS` edge sitting right there.** The message is precise (it does realize nothing) but the CATEGORY is false: an orphan is a node connected to nothing, and this one is connected. **This is [BL-140]'s shape** — declaring the truth in the vocabulary provided does not stop the alarm. **The honest note, per `sharpening.md` §2:** the node that surfaced this had a *true* `REALIZES cap:skills-served` available and both its siblings (`art:kit-pointer`, `art:setup`) carry both edges, so adding it was completing the model rather than silencing the tool — and the general defect is unaffected, because a **pure runbook has no capability to realize** and the skill tells its author to use `DOCUMENTS` alone. **Fix shape:** the orphan rule should treat `DOCUMENTS` (and `SPECIFIES`) as connecting an Artifact, or report a distinct, non-warning observation for a document artifact that realizes nothing — deciding which is the real question, since "this doc describes nothing either" IS worth saying. Size **S** | S |
| **BL-179** | **A design can now be LOCATED on the function-to-structure trajectory — the position is computed, the destination is not stated — DONE 2026-08-03** | *Anthony's closing realisation, 2026-08-02: "probably all designs start immature… initially getting function right is the first objective, then you go towards getting the structure/packaging right. this is often iterative and organic."* **Seven bands, each a count over a population carrying the question it answers: intent, function, allocation, seams, realization, assurance, operation. The lowest-scoring measurable band is the FRONTIER.** **THE FRONTIER IS RELATIVE, AND THAT IS THE WHOLE SAFETY PROPERTY: there is no threshold anywhere in this reading** — nothing to default, argue with or quietly tune — which is *stronger* than the stated cutoffs `granularity` needed. It says where a design **is** and refuses to say where it **should** be, exactly as `dec:readiness-is-an-observation-the-threshold-is-the-judgement` refuses to default a TRL gate: a demonstrator may sit at function-first forever and be right; a fielded increment may not. **reflow2's own reading is the thesis as a measurement:** intent 83.7%, function 88.8%, allocation **100%**, **seams 0.0% ← frontier**, realization 50.9%, assurance 84.5%, operation 95.5%. **Three bands run AHEAD of the frontier and are reported as NORMAL, not as work out of order** — this design has shipped 22 releases with 84.5% passing assurance and *zero* of its 73 component couplings declared through a contract. A linear ladder would have misdescribed that. **NO STAGE LABEL IS EMITTED, on Anthony's call** — breadboard/EVT/production is how people talk about the profile, and `dec:edge-orthogonality` forbids a distinction no computation reads. **NO SCHEMA CHANGE: the stamp does not move**, because nothing new was declared — this only computes over edges already present. Other restraints, each a test: an empty population reads *unmeasured* with its reason rather than zero (scoring an absence invents a deficiency); an empty design reports as empty, not immature; a one-sided contract is not a declared seam; assurance counts a **passing** check (`dec:passing-is-verified`); a dropped requirement leaves the intent population, because settling something out is the user's word too. Surfaced in `graph_report_markdown` as well as on demand. `tests/maturity.rs` (10), **mutation-checked against five seeded regressions, all caught** — and the first one-sided-contract mutation was a no-op that had to be rewritten the way the bug would actually be written before it counted as evidence. Size **M** | M |
| **BL-178** | **The brainstorm skill instructs a call the tool made unnecessary — [BL-152]'s shape a second time** | *Found 2026-08-02 by FOLLOWING the skill: it says step 2 is not optional, the tool disagreed, and the tool was right.* `brainstorm/SKILL.md` step 2 reads *"`set_decision_status` to `proposed`. **This call is not optional**: `add_decision` defaults to `accepted`, which would assert a settlement that never happened."* `add_decision`'s own served description says the opposite and dates it: *"It lands `proposed` … BEHAVIOUR CHANGED 2026-07-25: this used to default to `accepted`."* Confirmed empirically — `dec:maturity-restructuring-delta` came back `proposed` with no `set_decision_status` call. **This is [BL-152] exactly** (*"a served skill instructs a call the served tool made unnecessary a week earlier — the two halves of the served surface disagree"*), in a second skill, and BL-152 is still open. **The wasted call is the small half; the misinformation is the large one** — the skill states a FALSE FACT about the tool's default in bold, so an agent that reads it and skips the redundant call still carries the belief that a bare `add_decision` asserts a settlement. **Worth fixing as the CLASS rather than the instance:** BL-152 should be re-read as "no skill may restate a tool's default", since a default is exactly the kind of fact that moves under a skill without touching it. Size **S** | S |
| **BL-177** | **`--check` promises a change the real run refuses, and the two give different advice** | *Found 2026-08-02 doing [BL-175].* `reflow2_init.py . --check` listed `update  REFLOW2.md`; the run then printed `REFLOW2.md  LEFT ALONE — differs from what the kit installed (your edits); delete the file to accept the kit copy` and changed nothing. **Both are individually correct and they disagree**, because `planned_changes()` tests only `filecmp.cmp(src, side)` — does the file differ from what the kit ships — while `place_kit_file()` additionally tests the **manifest hash** (*"the file is ours to refresh only when its current content matches what the kit last installed"*). A dry run exists to say what a run will do; this one over-promises, and a user who reads `--check`, runs it, and does not read the full output concludes the update landed. **It is worse than a plain mismatch because the remedies differ:** `--check` implies "run it", the run says "delete the file". **Fix shape:** `planned_changes()` applies the same manifest test and reports `LEFT ALONE (delete to accept)` where the run would — one predicate, shared, rather than two that drift. Size **S** | S |
| **BL-175** | **reflow2's own kit install was SEVEN RELEASES STALE, and the cause was hand-editing a file the installer owns — DONE 2026-08-02** | *Found 2026-08-02 doing [BL-174], by running `reflow2_init.py . --check` — which reported it in its first two lines and had been able to the whole time.* `installed from reflow2 0.15.0 (519457e)` / `now at reflow2 0.22.0 (b51d202)`. **`CLAUDE.md` directs every agent here to read `REFLOW2.md`**, and that file is a **286-line pre-`req:thin-install` copy** of the kit's working instructions frozen at v0.15.0, where the current install would place the **42-line pointer**. So this repo's own agents read superseded text — 29 lines differ from what `get_instructions` serves, including a **`Restoring a design` procedure that predates the shared server and omits `--stop-shared`**, and the whole missing section explaining that *the skills are served, not installed*. **This is the exact failure the link-artifacts skill names** — *"Two instruction files disagreeing about the build command is exactly the coherence failure this exists to catch"* — committed in reflow2's own repository. **It is also [BL-165]'s pattern a fifth time and §2b's thesis confirmed: the detector worked and nobody called it.** **DONE on Anthony's word.** `REFLOW2.md` is now the current 42-line pointer, which ends with the promise that makes this un-repeatable: *"Upgrading reflow2 should never produce a diff in this repository."* **THE ROOT CAUSE IS NOT STALENESS, IT IS HAND-EDITING AN INSTALLER-OWNED FILE.** `place_kit_file()` refreshes a kit file *only while its content still matches the manifest hash* — that is the guard which stops an installer destroying your edits. Three commits (`5eac9c8`, `5a128f0`, `939f342`) edited `REFLOW2.md` **in place** instead of editing `getting-started/` and re-running, so the hash diverged and the file was frozen from then on: **every subsequent install correctly refused to touch it, and the protection that keeps edits safe is the same mechanism that keeps a stale file stale — silently, forever, until someone reads `--check`.** Fixed by deleting the file and re-running, exactly as the tool's own message instructs. **A SECOND, SELF-INFLICTED LESSON WORTH MORE THAN THE ROW:** I first reported that the installer *"reported success and never applied the update"* — **wrong, and wrong because I read its output through `tail -20` and cut off the line that said so.** The tool was loud; I truncated it. That is `sharpening.md` §2b's own first habit — *read the result back* — failed by reading only part of it, and it is the exact shape of the false-green it warns about, inverted: a **false red** invented by a partial read. Also filed [BL-177] from the same run. Size **S** | S |
| **BL-174** | **The served working instructions told a consumer agent to report what FOUGHT it, and nothing about a tool that answers cheerfully and is wrong — DONE 2026-08-02** | *On Anthony's word, after he asked how a new project gets told how to use reflow2 and observed that the max-use guidance must not suppress an agent's own evaluation.* `getting-started/AGENTS.md` — the text `build.rs` embeds as `INSTRUCTIONS` and `get_instructions` serves to **every** consumer project — carried *"If reflow2 gets in your way, say so"*, whose every example is something that OBSTRUCTS you: a tool that fails without saying what would work, a gap firing on correct work, something you cannot record. **Nothing told a consumer agent that a successful response is a claim rather than a result.** That bites harder on the consumer side than here: a new project has no `docs/sharpening.md` and no prior sessions, so `0 gaps` reads as reassurance on a design that may cover a third of the system. Added a sibling section carrying the rule (`0 gaps` means nothing was DETECTED, never nothing is wrong) and four habits — read the result back, diff two things that ought to agree, ask why odd output is odd before filtering it, ask what the check could not have seen — framed as the other half of using reflow2 well rather than an argument for using it less. The consumer-facing twin of `docs/sharpening.md` §2b. **A second finding fell out: `getting-started/AGENTS.md` was NOT A REGISTERED ARTIFACT** while `POINTER.md` (`art:kit-pointer`) and `SETUP.md` (`art:setup`) beside it both were — the single highest-consequence text in the kit, compiled into the binary, invisible to the design. [BL-165]'s shape a fourth time. Registered as `art:kit-instructions`, `DOCUMENTS cap:skills-served` with `doc_kind: agent_instructions` per the link-artifacts skill's rule that an instruction file describes rather than realizes. Size **S** | S |
| **BL-172** | **Coverage scope is settled — derived from version control, on Anthony's word — and the objection recorded against it was mine, not his — DONE 2026-08-02** | `dec:coverage-scope-is-declared` moves proposed → **accepted**: option 3 of the three put to him. The sweep is everything version control tracks, minus NAMED exclusions. **No vocabulary change, so the schema stamp does not move** — which on this project is what locks out older binaries, and it is the cheapest of the three by a wide margin. **His reasoning corrected the weakness I had recorded against it**, and that correction is the durable part: the objection on file said deriving from a repo *"says nothing for a design whose subject is a fleet, a document set, or hardware"*, and that **conflated the subject system with the design's artifacts.** A sweep only ever covers the second. In his words: *"while a hardware system, for example, can't live within a git repo, its design artifacts can. obviously, for all non-code projects, this is the case."* So the derived scope is not a code-only convenience — it is the general case, and **more** clearly right for hardware, where the artifacts are all there is to sweep. **What is actually under version control, in his decomposition:** a NON-CODE project tracks TWO things — the reflow2 design graph and the design artifacts; a CODE project tracks THREE — graph, artifacts, **and the implemented code**. The first two are the constant and the implemented system is the only term that varies. **Code is the special case by holding MORE than the norm, not less** — *"code based projects are the system and can be inspected by the agent as well as the design"* — the one kind of subject an agent can inspect directly as well as through its design; and he expects that exception to be the majority of reflow2's use, which is exactly why it must not be mistaken for the rule. **The real residual limit, which is a different one:** an artifact outside version control (a PLM system, a wiki, a local CAD file) is invisible to a derived sweep — answered by the existing seam, since `coverage_report` takes caller-supplied observations, so derivation is the DEFAULT and not a ceiling. **Carried into the adopt skill**, whose Phase 4 told you to sweep and never said how to scope it — the hole [BL-165] fell through. The two rejected shapes stay on the record as overrides, per ADR discipline. Size **S** | S |
| **BL-171** | **`Verification.location` exists in the schema, is set on none of the 101, and the path lives in the node's NAME as prose** | *Found 2026-08-02 by the derived coverage sweep built in [BL-170], on its first run: test files dominated the unclaimed list even though the design models every one of them.* The BL-23 granularity doctrine is **one Verification per test file**, and the design honours it — `ver:detect` is `detect.rs`, `ver:heal` is `heal.rs`. But `coverage_report` counts nodes carrying a **`location`**, and **zero of 101 Verifications carry one**; the path is written into `name`, e.g. *"An acknowledgement is a statement ABOUT the design, not a part of it (crates/reflow2-core/tests/acknowledgement_not_structure.rs)"*. So the design's knowledge of where its own tests live is real, recorded, and **unqueryable** — ~71 test files report as regions nothing claims, forever. **This is [BL-165]'s shape a third time** (the fact exists somewhere the machinery does not look) and **[BL-129]'s shape exactly** (`Interface.medium` unreachable from both tools that create Interfaces): **`add_verification` does not accept `location`**, so the schema property is unreachable from the surface that creates the node — which is *why* the path went into the nearest writable field. **Not fixable by the build script alone:** only 26 of the 101 come from its `VERIFICATIONS` table; the other 75 were authored in live sessions, so a script-side patch would leave a confusing half-state. **Fix shape:** `add_verification` takes `location` (surface change, toolsnap), then backfill — the script's 26 from its own table, the rest by parsing the paths already sitting in their names. **Then `coverage_report` can finally see verification coverage**, which is the one class of design knowledge it is currently blind to. Size **S** | S |
| **BL-169** | **A round trip through the `import_graph` TOOL silently renamed the design — and nothing, including CI, could see it — DONE 2026-08-02** | *Found 2026-08-02 doing [BL-165], by walking into it: an export replayed through a temp graph came back with `graph_id: 05a6fbe860bf7a23` where the design has been `reflow2` since its first commit, and `compare_designs` was the only thing that said so.* The CLI `--import` path **adopts** the document's identity when the receiving store is empty, deliberately and with the reason in a comment — *"or the round trip would not come back byte-identical (graph_id is inside the content hash)"* — via `identity::adopt_on_import`. **The MCP `import_graph` tool does not.** It loads into the receiving graph and keeps the receiver's name, which is correct for merging a document into an existing design and silently wrong for a restore. **This is [BL-119]/[BL-138]'s finding — "`import_graph` never read `graph_id`" — with its other half still open: it was right to stop DEMANDING the field, and it is not right to keep IGNORING it.** **The live landmine:** `tools/build_design_graph.py`, the command AGENTS.md documents for rebuilding the design, builds in a temp graph and so writes exports stamped with a random id (`ab64af9035ed7a7c` when run 2026-08-02) — the only reason `main` still says `reflow2` is that recent exports came from live sessions instead. Committing one script-produced export would have renamed the design, and **`graph_id` is inside the content hash, so it also breaks the lineage chain**. **NOTHING CAUGHT IT, INCLUDING CI.** The renamed export was pushed and **GitHub Actions run `30745776837`'s predecessor went fully green on it** — both jobs, `reflow2_check.py` included. The chain was intact (the rename happened between two commits that each linked correctly), the content hash matched its own content, and every test passed; the design had simply stopped being called `reflow2`. The one signal in the whole system was `compare_designs` mentioning "different graph ids" inside a `provenance_note` — a field nothing gates on. **So the fix needs a second half: the gate should treat a `graph_id` change like the lineage break it resembles** — a design's identity moving is either deliberate or a bug, and it must not be able to happen silently. **DONE — both halves.** (1) **`import_graph` now adopts identity itself**, and the rule MOVED rather than being copied: it lived in `main.rs`, so the CLI adopted and the tool did not, which is the whole defect. `DesignGraph` gained a `store_path` so the operation can reach its own identity sidecar, and `main.rs` now only REPORTS what the core did — one predicate, every caller, including any future one. The adoption is returned as `ImportReport.adopted_identity` rather than done quietly, because a caller that cannot see a design was renamed cannot tell you. (2) **`reflow2_check.py` gained an `IDENTITY` check**, and the pair-resolution logic is now SHARED with the lineage check (`_export_pair`) instead of copied — [BL-177]'s lesson applied on the spot, since two predicates that drift is exactly what filed that row. **THE TEST THAT MATTERS PROVES WHY IT SHIPPED:** the rename case asserts `IDENTITY` fires *and that `INTEGRITY` does not*, because the hash matches its own content across a rename and the chain links across it perfectly — identity is the only check that can catch it. **A FINDING IN THE EXISTING SUITE:** the new counterweight failed on first run, and the cause was the fixture — `second_export` built into a SECOND store (`graph2`), so every "second export" was a different design with its own minted id, and the chain tests had been checking the lineage of two unrelated designs and passing. Fixture now reuses one store, which is what "a design replacing itself" actually means. **THE OTHER THING THE ROW MISSED:** the rule already HAD tests — `an_empty_store_takes_the_name_of_the_design_it_restores` passed the whole time. The predicate was tested; the operation did not use the predicate. 3 new gate cases, 2 new identity cases, mutation-checked in both directions. `import_graph`'s served description gained the identity contract and had to be TRIMMED to fit the 1500-char budget — rationale ("because you cannot learn it from an export of an empty graph") and a benefit restatement came out, which is the right thing to cut from a description an agent pays for every session. Size **S** | S |
| **BL-168** | **The two paths that author reflow2's own design disagree, and whichever ran last wins** | *Found 2026-08-02 doing [BL-165], by running `tools/build_design_graph.py` against a scratch export and diffing its output against the committed record.* The design is written two ways: **from a live session** through the MCP tools, and **by `build_design_graph.py`**, which derives structure from source. They do not produce the same graph. The script derives **10 `DEPENDS_ON` edges the committed export does not hold** (`cmp:temporal`→`cmp:detect`, `cmp:graph`→`cmp:temporal`, `cmp:drift`→`cmp:artifact`, …) — real call-graph facts, absent because the last several exports came from live sessions and nothing refreshes the derived layer. It also prints **`NOT MODELLED (no Component; DEPENDS_ON edges dropped)` for 14 modules** — `alternatives`, `bulk`, `claims`, `compose`, `content`, `coverage`, `depends`, `identity`, `merge`, `readiness`, `sanitize`, `scope`, `seam`, `sync` — every one of which HAS an Artifact realizing a Capability but no Component, so its dependencies are silently dropped from the structure the topology detectors read. **The stake:** `circular_dependencies`, `single_point_of_failure` and `evaluate_allocation` all run over `DEPENDS_ON`, so a design missing 10 real edges and blind to 14 modules is reporting structural health on a partial graph — and reporting it as `0 defects`. **The question first:** is the derived layer authoritative (the script refreshes it and a live export must not drop it), or advisory? Until that is answered the two paths keep overwriting each other. Size **M** | M |
| **BL-167** | **1,057 lines of shipped product have no Artifact — `proxy.rs` and `shared.rs` are invisible to the design** | *Found 2026-08-02 by the report line added in [BL-165], on its FIRST run.* `crates/reflow2-mcp/src/proxy.rs` (513 lines) and `crates/reflow2-mcp/src/shared.rs` (544 lines) arrived in `280397c` — the shared-mode feature delivering `req:sessions-share-a-graph` and `req:never-silently-absent` — and neither was ever registered. They were inside the sweep's scope the whole time and reported as `undocumented_addition`, but that phrasing reads as a nit; the explicit "swept file(s) with NO Artifact" line is what made them legible. **The consequence is the one [BL-165] names:** they are in no release manifest, `reconcile_artifacts` cannot see them drift, and a ChangeEvent touching shared mode has nowhere correct to point. **Fix shape:** they need Components first (`ARTIFACTS` is keyed component → path), which makes this [BL-168]'s 14-module problem in miniature — do them together. Size **S** | S |
| **BL-166** | **`link_artifact` silently erased every property it was not given — including the dated proof that someone checked the file — DONE 2026-08-02** | *Found 2026-08-02 doing [BL-165], by re-registering `schema/readiness.yaml` and watching `last_confirmed_at: 2026-08-02` vanish while the tool reported success.* `write_artifact_link` built its `Props` from the four fields it takes and called `create_node`, which is create-or-**REPLACE** and re-materializes schema defaults over everything omitted — and it names four of `Artifact`'s nine properties. **The evidence was sitting in the committed export:** of the 34 artifacts `build_design_graph.py` re-links on every run, **ZERO carried a `last_confirmed_at`**, while the only two in the entire design that did were the two registered by hand the day before and never re-linked since. So every rebuild of reflow2's own design had been erasing its dated confirmations, and a swept artifact was indistinguishable from one nobody ever looked at — the exact distinction `reconcile_artifacts(record_events: true)` exists to draw. **`status` was the subtler half:** it only ever LOOKED safe because its default (`realized`) happened to equal the stored value, so an Artifact at `verified` was being silently downgraded on every re-link. **This is [BL-46] at a second call site** — the fix, `upsert_node`, was written for that incident, documents this exact hazard, and `set_artifact_checksum` twenty lines below hand-rolls the same merge rather than calling it. 1 case, mutation-checked both directions (the counterweight asserts a re-link still MOVES what it does name, or preserving state would just be a way of ignoring the caller). Size **S** | S |
| **BL-165** | **Ten of the eleven schema domains are not registered artifacts, so a release ships vocabulary the design cannot see — DONE 2026-08-02** | **DONE — and the row's own fix shape was the smaller half.** All eleven domains are registered (`art:schema-{domain}`, `spec`, full sha256, `REALIZES cmp:schema`), each with a dated confirmation from a real observed sweep. **The shape question the row asked was answered "ten", per its own reasoning and `art:schema-readiness`'s precedent; they realize `cmp:schema` because that is PROVABLE — `src/schema.rs` embeds every one with `include_str!` — where "the component each domain defines" is a judgement, and making that judgement wrong is how this row got filed.** **The root cause was NOT ten missing calls.** `coverage_report` names all ten unclaimed on demand, so the detector was never the problem; what was missing is that **`build_design_graph.py`'s filesystem sweep — the probe written after the 2026-07-20 self-adopt found 15 of 33 files unregistered — had a hardcoded scope of the two `src/` trees**, so the one probe built to catch unregistered files could not see the directory AGENTS.md calls *"the foundation everything builds on"*. And `ARTIFACTS` is keyed component → ONE path, which makes a second file of an already-carried component **unrepresentable** — the reason a list was never going to hold this. Fixed by globbing `schema/*.yaml` (so domain twelve registers itself) and adding it to the sweep. **Two findings fell out, both worth more than the row:** [BL-166] — `link_artifact` was silently erasing `last_confirmed_at` and downgrading `verified` artifacts on every re-link — and [BL-167], found by a report line whose variable was collected and never printed. Verified by running the script against a scratch export and diffing: **all eleven artifacts byte-identical to the hand-written graph.** — *original entry follows* — *Found 2026-08-02 during the v0.22.0 cut, by chasing a FALSE CLAIM in my own ledger: `chg:bl68-readiness-gating` asserted that `art:temporal` (`crates/reflow2-core/src/temporal.rs`) had changed. It never did — the edit was to `schema/temporal.yaml`. Retracting the edge exposed why the mistake was easy: there is no artifact for `schema/temporal.yaml` to point at.* **`schema/readiness.yaml` is registered only because it was created this session and `link_artifact` was called on it; the other ten domains (`core`, `functional`, `structure`, `build`, `verify`, `operate`, `environment`, `temporal`, `inference`, `dimensions`) have no Artifact at all.** **THE STAKE IS NOT BOOKKEEPING.** AGENTS.md calls the schema *"the vocabulary — the node/edge names are load-bearing"* and *"the foundation everything builds on"*, and `release_includes_all` derives a release manifest from registered Artifacts — so **v0.22.0 ships ten vocabulary files its own manifest does not list**, and `reconcile_artifacts` cannot detect drift on any of them. A schema edit is the highest-consequence edit in this repo (it moves the stamp and locks out older binaries) and it is the one class of file the as-built machinery is blind to. **It also makes a whole category of ChangeEvent unsayable**, which is how the false claim happened: with nowhere correct to point, the nearest-looking node gets named. **Fix shape:** register all ten with `link_artifact`, `REALIZES` the components they define — the work is ten calls plus their checksums, and `create_nodes`/`create_edges` make it one round trip each. **The question worth asking first, because it decides the shape:** should each domain be its own Artifact, or is the schema ONE artifact with ten files? Ten is more honest — they change independently and `validate_schema.py` reports them separately — and it is what `art:schema-readiness` already implies. Size **S** | S |
| **BL-164** | **The schema's own size is hand-copied into at least NINE places, and adding one node type broke the build four separate times** | *Found 2026-08-02 while building [BL-68], by counting the fix cycles: one schema addition (`ReadinessAssessment` + two edge types) produced four independent red builds, each in a different layer, each the same one-line edit.* The merged schema knows it has 29 node types and 60 edge types. **Nine other places also claim to know**, by hand: `schema.rs`'s test, `vocabulary.rs`'s test, `reflow2-mcp/tests/tools.rs`'s test, `tools/smoke_mcp.py`'s schema-discovery check, and prose in `schema.rs`, `lib.rs`, `AGENTS.md` (twice), `README.md`, `docs/overview.md` and `docs/interaction-surfaces.md`. **THE PROSE HALF WAS ALREADY WRONG BEFORE THIS CHANGE TOUCHED IT, which is the actual finding:** `schema.rs`'s own doc comment said *53 edge types* while the test three screens below asserted **58**; `lib.rs` said *27 node types, 53 edge types*; `AGENTS.md`, `README.md` and `overview.md` all said *57*; `interaction-surfaces.md` said *55*. **Five documents disagreeing with the code and with each other, and nothing noticed** — because only the four TEST copies are enforced, and a test copy is exactly the one a builder updates without thinking. **This is [BL-159]'s class for the third time and [BL-152]'s for the fourth** — two records of one contract, kept by hand, drifting — and the pattern is now well enough evidenced to state plainly: *in this repo, a hand-copied fact about the code is not a fact, it is a decaying claim.* **Fix shape, and it is small because the machinery exists:** `skill_lint.py` already cross-checks AGENTS.md's gate block against `ci.yml` four ways, so teach it one more check — the counts asserted in prose must equal what `validate_schema.py` computes from `schema/*.yaml`, which already prints them. The TEST copies are a separate and easier call: they could read the count from `load_schema()` instead of restating it, though there is an argument that a test restating the number is the point (it fails when the vocabulary moves, which is a deliberate speed bump on a stamp-moving change). **So the recommendation splits: make the four test copies stay hand-written and LOUD, and make the five prose copies computed or linted, because prose has no such speed bump — it simply rots.** Size **S** | S |
| **BL-163** | **The trigger that exists to catch "bookkeeping is not the loop" measures exactly bookkeeping — it checks a ChangeEvent's PRESENCE where it means its PRECEDENCE — DONE 2026-08-01** | *Found 2026-08-01 by Anthony asking why an agent with reflow2 installed had spent a whole session barely using it, and the answer being reproducible in that same session.* `tools/loop_nudge.py:450` fires the impact-check trigger on `edits > 0 and state.get("changes", 0) == 0` — **only when the session recorded ZERO ChangeEvents.** The session that found this edited files across three backlog items and recorded three ChangeEvents, so `changes > 0` and the trigger stayed silent — **while every one of those ChangeEvents was written AFTER its edit, as bookkeeping.** The Stop hook's own message says *"Bookkeeping is not the loop"*; the trigger shipped beside it cannot tell the two orders apart. **THE MECHANISM IS NOT MISSING, IT IS MIS-KEYED** — `cap:skill-triggers` is built and carries the right counterweight (*a session that did the right thing gets NOTHING*), so this is a discriminator fix, not a new capability. **THE ROOT CAUSE IS VISIBLE IN ONE LINE:** `CHANGE_OPS = {record_change, add_change_event}` — **both are RECORDING ops, and no set counts `propagate_change` or `propagate_from` at all.** The nudge cannot separate recording from looking because it never counts looking. **Fix shape:** add `PROPAGATE_OPS = {propagate_change, propagate_from}` and key impact-check on *edited code, recorded a change, never propagated* — `add_change_event` is recording, `propagate_change` is the act of looking at the blast radius, and impact-check is both in that order. **Measured against the session that found it:** BL-160 propagated once and would correctly stay silent; BL-159 and BL-162 never propagated and would both have fired. Two true positives, one true negative, no false positives. **THE STANDING COUNTERWEIGHT MUST HOLD** — [BL-23] and [BL-42] both name firing on correct work as the failure this family exists to avoid, so a session that propagated before editing must get nothing, and that is the case to pin first. **The wider point the row is really about, and why it is worth more than its size:** reflow2 enforces the WRITE side and merely suggests the READ side. The gate fails the build on undeclared drift; the export chain fails on a broken link; the Stop hook fires on unchecked writes. **Nothing whatsoever fails when an agent designs without consulting the design** — `propagate_change`, `search_design` and `get_skill` have no forcing function, which is [BL-154]'s measured decay (*skills read once per 380 tool calls*) with no counterweight. `cap:tool-carries-convention` already reached this conclusion one layer down: *the convention rides the TOOL, because the tool description arrives with every call while the skill is read once per 380 calls.* **A session-opening primer is NOT the fix and that was tested by accident here:** the SessionStart hook fired twice telling the agent to orient with `where-am-i`, the cheap half was done (`loop_status`, `open_questions`) and the skill was skipped. What actually corrected the agent twice were INTERRUPTS tied to a situation — the Stop hook, and the user's question — not the primer. Size **S** — **BUILT 2026-08-01 exactly as the row's fix shape specified, plus one thing the row did not see and one it left implicit.** `PROPAGATE_OPS = {propagate_change, propagate_from}` exists, a `propagates` counter tallies it, and the impact-check shape splits in two: *nothing recorded* keeps its old sentence, and *recorded but never propagated* gets its own. **THE HALF THE ROW LEFT IMPLICIT IS THE LOAD-BEARING ONE, and it is a change in kind rather than degree: fixing the SHAPE alone would have caught nobody.** `match_shape` only ever refines a nudge the hook had already decided to send — `cap:skill-triggers` added no interruptions on purpose — and the session this row is about has **no unchecked writes** (`record_change` is a graph write, and `loop_status` clears the count) and **has touched reflow2**, so both existing branches read it as clean and there was no nudge for a better sentence to attach to. So the row's own measurement (*BL-159 and BL-162 would both have fired*) is only true if the trigger ARMS, and a third blocking branch is what was built. **That is the one interruption this capability had deliberately never added**, and the argument for it is the row's own wider point: reflow2 fails the build on undeclared drift, on a broken export chain and on unchecked writes, and nothing whatsoever failed when an agent designed without consulting the design. **THE COUNTERWEIGHT IS THE CONJUNCTION, and all three clauses are pinned separately:** `edits > 0` (a pure design session has no blast radius to compute), `changes > 0` (this session put a ChangeEvent on the record, which is what makes it design-relevant and what stops the branch becoming a second, thresholdless bypass nudge), `propagates == 0`. **The measurement that proves the middle clause is the mutation that drops it: SEVEN tests fail, including BL-90's ENTIRE bypass family** — without it this branch simply swallows the older one. **PRESENCE, NOT LITERAL ORDERING, and that is deliberate and stated in the code:** the honest key would be *propagated before the first edit*, but a session legitimately edits for one item while propagating for another, and a strict ordering rule would fire on correct work — the [BL-23]/[BL-42] failure this family exists to avoid. Presence cannot catch an agent who propagates late; that limit is written down rather than hidden. **[BL-161]'s PRECEDENT HELD WITHOUT BEING ASKED FOR:** *"you never propagated"* is a NEGATIVE claim, so a tally rebuilt after an unreadable one drops it exactly as *"the graph was never consulted"* does — the two branches now sit on the same side of one `reset` guard. **THE COUNTERWEIGHT TEST WAS CHANGED, and it is named here because it is the most dangerous edit in this repo:** `test_THE_COUNTERWEIGHT_a_session_that_did_it_right_gets_nothing` FAILED against the fix. Its fixture — edit → `record_change` → `link_artifact` → `add_requirement` → `detect_gaps` — never once asked what the change reached, so what moved is the definition of *did it right*, not the assertion, which is untouched. `propagate_change` was added to the fixture **and the old fixture is pinned by its own new test as the defect**, so nothing was quietly relaxed. **AND A FOURTH DEFECT THE ROW DID NOT SEE, found by the tests failing rather than by reading:** `update_state` re-serialises the tally from an **explicit key whitelist** — a THIRD hand-kept copy of the state's key set, beside `blank_state` and `parse_state` — so the new counter incremented in memory and was thrown away on every write, with no error. Two of three updated is a silent drop, and it is [BL-159]'s *two records of one contract, kept by hand* a third time **inside a single file**; the comment now says so at the site. 11 new cases (47 total), **mutation-checked seven ways**, each failing exactly the tests that name it — including *remove the branch entirely* (5 failures, i.e. ship the defect) and *disable the write branch* (14), which proves the write-nudge-still-takes-precedence case is not vacuous. Also tunable: `REFLOW2_LOOP_NUDGE_PROPAGATE_THRESHOLD`. | S |
| **BL-162** | **An accepted Decision that governs nothing is invisible to every detector — and `disconnected_community` structurally cannot see it — DONE 2026-08-01** | *Found 2026-08-01 by running the `check-health` and `detect-and-ask` skills on reflow2's own design at Anthony's request, getting a clean bill from every one of them — `detect_defects` 0, `hierarchy_issues` 0, `dimension_drifts` 0, `propose_heal` 0 operations, `detect_gaps` 0 — and then counting zero-degree nodes by hand.* **`dec:sanitize-spof-accepted` — an ACCEPTED single-point-of-failure disposition — had no edges at all**, the only one of five such dispositions not linked to what it disposes. `disconnected_community` cannot report it because it only fires on clusters of **≥2**, and a node joined to nothing is never a cluster; `orphan_node` cannot because since [BL-42] it holds exactly one rule (an Artifact realizing nothing). **THE STAKE IS NOT TIDINESS.** A zero-degree Decision is unreachable by propagation, so it never enters an impact analysis — and a *disposition* specifically **can never EXPIRE**, because expiry is computed from the affected set (`ver:reviewed-defects`). A conditional judgement silently becomes a permanent off switch. **THE RULE IS DEGREE-ZERO, AND MEASUREMENT CHOSE IT AGAINST MY OWN STATED RECOMMENDATION** — the finding worth carrying. I argued the safe narrow rule was *an accepted Decision with no incoming `GOVERNED_BY`* and that general degree-zero was the risky one. Measured on the real design: the `GOVERNED_BY` form fires on **six**, five of them with degree 1–3 — connected, merely not through that one edge type, which is **[BL-42]'s exact shape**, where this same detector reported a well-connected Capability missing one named link, became **20 of 31 defects**, and had to be cut back. Degree-zero fires on **one**, and is self-limiting in a way an edge-named rule is not: **any** edge silences it, so it can never grow into a per-convention nag. **Graded by status on Anthony's call:** Warning when `accepted` (it claims to shape the design and shapes nothing), Info otherwise (a parked decision point is a legitimate state, and firing on one is firing on correct work). **Review records are excluded, and that is a design choice not a convenience** — `structure.rs` already keeps `decision:ack:` ids out of the design network as judgements *about* the design rather than structure (`ver:acknowledgement-not-structure`), and all twelve of reflow2's own are `accepted` by construction, so including them would have been pure noise. **AN EARLIER READING OF THIS FINDING WAS WRONG AND IS CORRECTED HERE:** the first count was *18 zero-edge nodes*, reported as a blind spot. Reading `structure.rs` showed 15 of those 18 are excluded deliberately — 12 review records by id rule, 3 `DesignEpoch` by `NON_DESIGN_TYPES`. The real finding is **three nodes, one of them a defect.** 6 cases in `crates/reflow2-core/tests/unreachable_decision.rs`, mutation-checked three ways, each failing exactly the test that names it — including *key on missing `GOVERNED_BY`*, which reproduces BL-42 and fails precisely the counterweight the measurement demanded. **Fixed on our own design in the same change**: `cmp:ingest GOVERNED_BY dec:sanitize-spof-accepted`, matching the four sibling dispositions; the design now reports 2 Info and no Warning. Size **S** | S |
| **BL-161** | **The loop nudge corrupted the record it judges from — a session that consulted the graph constantly was told at Stop that it never had — DONE 2026-08-01** | *Fired on THREE separate stops of a session that made dozens of graph calls, and it is the second independent reproduction: the same thing happened on 2026-08-01 earlier and was deliberately left unfiled pending source verification, per [BL-133]'s rule.* **THE CAUSE IS THIS PROJECT'S OWN FIRST PRINCIPLE, VIOLATED INSIDE THE TOOL THAT ENFORCES IT.** `write_state` used `Path.write_text` — truncate, then write, **not atomic** — while `read_state` wrapped the parse in `except (OSError, ValueError, ...)` and returned an **all-zero, `touched: False`** tally. PostToolUse hooks run as SEPARATE CONCURRENT PROCESSES (two matchers are registered, `mcp__reflow2__.*` and `Edit|Write|...`) and parallel tool batches are ordinary — so one hook reading while another wrote got a truncated file, `json.loads` raised, the failure was swallowed into zeros, and that process **wrote the zeros back**, wiping `touched`, `artifacts`, `captures` and `gap_pass` for the rest of the session. AGENTS.md rule 4 and engineering principle 2 forbid exactly this — *"no catch-returns-default... a swallowed failure makes broken code report success"* — and the correct pattern already existed here: `ver:content-store` pins *"an interrupted write leaves no partial file, proven by writing to a temp path and renaming"*. **REPRODUCED, NOT INFERRED:** seed `{touched: true, artifacts: 4, writes: 4}`, fire 150 concurrent edit hooks, get back `{touched: false, artifacts: 0, writes: 0, edits: 6}` — 144 of 150 increments lost with every sticky field. A gentler 40-pair run loses ONE update, which is why it read as intermittent and why two prior sessions could not pin it. **FIXED THREE WAYS.** (1) The read-modify-write takes an exclusive `flock`, degrading to the old behaviour where `fcntl` is absent rather than failing — a hook must never break a session. (2) The write goes to a temp file and `os.replace`. (3) **An unreadable tally still RESTARTS the count — an existing test required that and it was right — but the restart is now MARKED**, and the Stop backstop drops its one NEGATIVE claim (*"the graph was never consulted"*) when the flag is set, because a tally rebuilt from nothing cannot prove nothing happened. The POSITIVE claim (*"N writes went unchecked"*) survives a restart honestly, which is what stops the flag being an off switch. **SECOND DEFECT FIXED IN THE SAME PASS: the op sets never learned [BL-153]'s bulk forms** — `ARTIFACT_OPS` held `set_artifact_checksum` and not `set_artifact_checksums`, `CAPTURE_OPS` had no `create_nodes`, the gap-pass reckoning had no `gaps_to_prompts` — so a session doing everything right THROUGH THE NEW TOOLS tallied as having done none of it. [BL-152]'s shape landing on the trigger that judges the loop, and it worsens exactly as the bulk forms succeed. **A HYPOTHESIS RECORDED IN THE HANDOFF WAS DISPROVEN AND SHOULD NOT BE CHASED:** that deferred tools loaded via `ToolSearch` are not counted. Calling a tool and re-reading the state 8 seconds later showed `touched` flip `false → true`. **AND THE MUTATION CHECK CAUGHT MY OWN TEST PASSING FOR FREE** — the first concurrency helper looped `communicate()`, which blocks until each child exits, so the children ran serially and the test stayed green against the UNFIXED script. Removing the lock and removing the atomic write both passed until the helper was corrected to feed every stdin before waiting on any process. 4 new cases, mutation-checked five ways. **STILL OPEN AND ANTHONY'S TO MAKE: the nudge fires TWICE**, reproducing [BL-111]'s broken *"fires once"* promise — the `Stop` hook is registered in THREE settings files under TWO distinct command spellings (`$CLAUDE_PROJECT_DIR/...` in the repo's `.claude/settings.json`, the absolute path in both `.claude/settings.local.json` and `~/.claude/settings.json`); identical strings dedupe and the two spellings do not. Two of those files are personal config, so the repo cannot fix it. Size **S** | S |
| **BL-160** | **A registered checksum's LENGTH is a dialect too, and only the Python gate speaks it — the core reports 100% false drift on a truncated baseline — DONE 2026-08-01** | *Found 2026-08-01 by writing the proof for [BL-158] and having it report **51 phantom drifts on a provably clean tree**.* reflow2's own design registers checksums at MIXED lengths — some the full 64 hex chars, some a 16-char prefix — and `reconcile_artifacts` compares **strings**. So an honest caller who hashes a file with `sha256sum` and passes the full digest is told an untouched file has drifted, for every artifact registered short. **`tools/reflow2_check.py` already knows this and compensates in Python** (`hash_file` truncates the observed digest to `len(registered)`), with a comment saying exactly why — which is how the gate reported `OK — design and build agree` on the same tree, in the same minute, that a direct sweep called 51-way drift. **THE COMPENSATION LIVES IN THE WRONG LAYER.** `crate::artifact::canonical_checksum` normalises the `sha256:` PREFIX dialect and stops there, so every consumer that is not the gate — an agent driving `reconcile_artifacts` over MCP, CI in another project, the coding agent the tool's own description tells to *"compute the hashes yourself"* — hits the bug the gate is immune to. **This is [BL-125] in a second form and it deserves the same verdict.** BL-125 was *"the canonicaliser is applied when writing and never when comparing, so the first honest reconcile reports 100% false drift"*; that was fixed for the prefix and the identical failure survives for the length. Its own argument applies unchanged: *a false red on a gate whose whole job is to be believed is worse than no gate*. **Fix shape:** compare on the shorter of the two digests when one is a prefix of the other, in `canonical_checksum`'s caller in `drift.rs`, so the core answers correctly for everyone and `reflow2_check.py`'s Python workaround can be deleted rather than duplicated by the next consumer. **The counterweight that keeps it honest:** a prefix match must require a real prefix relationship, not merely equal-length-after-truncation, or two genuinely different digests sharing 16 characters would read as agreement — and the existing `ver:checksum-dialect` suite is where that case belongs, beside the counterweight it already pins (a genuinely different digest still reports `checksum_change`). Size **S** — **BUILT 2026-08-01 as the row specified, plus one half the row did not see.** `artifact::checksums_agree` sits beside `canonical_checksum` and both call sites go through it. **THE HALF THE ROW MISSED IS NOT A GATE PROBLEM AT ALL:** `set_artifact_checksum`'s *would this MOVE the baseline?* guard — the one [BL-157] added to stop `baseline_established` laundering a real drift — was the same string comparison, so a [BL-157] bulk sweep hashing with `sha256sum` would have been **refused on every short-registered artifact for a change that never happened**. The row named `drift.rs`; the accept path is where it would have bitten a user hardest, and it was found by asking who else compares two checksums rather than by following the reproduction. **One judgement the row left open, decided and pinned:** when the two dialects agree the LONGER digest stays on the record — it contradicts nothing the shorter one said, and it makes the accept idempotent ACROSS dialects, so re-stating a baseline coarsely mints one claim rather than two. **NO MINIMUM PREFIX LENGTH IS IMPOSED, deliberately.** A 16-char baseline is a weak baseline, but its strength is decided when it is REGISTERED — the write side takes it without complaint — and a read side refusing to honour what the write side accepted is the same write/read disagreement [BL-125] and this row are both about. **The counterweight the row asked for is pinned, and four more beside it:** two FULL digests sharing sixteen characters are still drift (the truncate-both-to-N trap the row named); a truncated observation disagreeing within its own length is still drift; a different digest is still refused as a first baseline; prefix tolerance never escapes the `sha256:` dialect, so `blake3:zz` and `blake3:zzzz` stay different; and an empty digest agrees with nothing, because every string starts with the empty string. 9 new cases in `checksum_dialect.rs` (14 total), **mutation-checked seven ways**, each failing exactly the tests that name it — including *make agreement always-true*, which fails all seven counterweights while every bug case passes, and *fix `drift.rs` alone*, which fails exactly the two accept-path cases. **THE TEST WAS CORRECTED BY MEASUREMENT:** the still-refused counterweight was first written against a SHORT baseline, where `NEAR` is a legitimate extension rather than a different digest — a 16-char baseline only ever pinned 16 characters, established by getting it wrong rather than by assuming. **PROVEN ON THE REAL DESIGN, the row's own scenario replayed:** 109 artifacts swept off the working tree at the full 64 chars, **51 truncated baselines, all 51 unchanged**, and the only 4 findings are the files this change edited. `reflow2_check.py`'s Python workaround is **deleted**, with two new gate cases pinning the pair end to end — short baseline + untouched file gates GREEN, short baseline + edited file still gates RED. **A SMALL CORRECTION TO THE DESIGN'S OWN MEMORY:** `chg:accept-e52b527aba651ac4` (2026-07-29) records a full-length registration as *"harmless, since reconcile truncates the observation to the registered dialect"*. Reconcile never did; the gate did. The belief was written down as a property of the core eleven days before the core acquired it. | S |
| **BL-159** | **The documented gate list disagrees with the gates CI runs — 8 commands against ~24, and one of the 8 carries the wrong flags — DONE 2026-08-01** | *Found 2026-08-01 the only way it can be — by following the list exactly and watching CI fail (PR #27, run `30711278862`), then MEASURING the divergence instead of fixing only the line that bit.* **The trigger:** two `redundant_closure` lints in `service.rs`, warnings locally and **errors in CI**, because AGENTS.md's *"A change is done when all of these are clean"* block did not list `cargo clippy -p reflow2-mcp --all-targets -- -D warnings`. Every gate the block DID list was green. **AGENTS.md already predicts this exact experience — *"Green locally but red in CI means your local run skipped a gate — believe CI"*** — so the file anticipates the failure mode and then omits the command that causes it. **THE MEASUREMENT IS THE ITEM, and it is much worse than the one missing line.** Diffing the two lists: `ci.yml` runs **~24** gate commands, the block listed **8**, and **a ninth divergence was latent and identical in kind** — the block's `-p reflow2-core` clippy lacked `-D warnings` while CI passes it, so a core warning would have failed CI exactly the same way, one line above the line that just did. Absent entirely: `reflow2_check`, `phase_trial`, `coherent_erosion_trial`, `model_the_loop`, `stateless_seat_probe`, `check_doc_versions`, and nine `test_*.py` suites. **Fixed for now** by correcting the flags, adding `reflow2_check`, and stating in the block that it is a SUBSET with `ci.yml` authoritative — *green here is not green there*. **The item is the CLASS: two records of one contract, kept by hand, drifting.** [BL-152]'s shape one layer out — that found a served skill instructing a call the served tool had made unnecessary; [BL-154] found skills nobody reads; this is the build gates. **Fix shape:** a `skill_lint`-style check asserting every `cargo`/`python3` gate command in `ci.yml` either appears in AGENTS.md's block or is explicitly listed as deliberately-omitted — `skill_lint.py` already does exactly this cross-check for served tool names and byte-identical command mirrors, so the machinery exists and this is a small addition to it. **Cheap, and it converts a class of silent divergence into a build failure — which is the only thing that has ever worked on this class in this repo.** Size **S** — **BUILT 2026-08-01, with FOUR checks rather than the row's one, and the extra one is the load-bearing one.** The row proposed **coverage** — every gate in `ci.yml` is listed or declared omitted. But **the defect that filed this row was a FLAGS difference on a gate that WAS listed**, so coverage alone would have passed it and left the exact failure unfixed. Identity and fidelity are therefore checked **separately**: identity deliberately drops flags to answer *"is this documented at all"*, fidelity compares the full command to answer *"is it documented correctly"*. Both rot directions are closed too (a documented gate CI does not run; an omitted name CI has stopped running). **THE SPLIT THAT KEEPS IT HONEST is [BL-160]'s threshold lesson one day later: the lint OBSERVES, the document JUDGES.** *"Is this `ci.yml` line a gate at all?"* is mechanical and lives in code (`NOT_A_GATE` holds exactly two entries — `pip install` and `cargo build`); *"does this gate belong in the everyday local subset?"* is judgement and lives in AGENTS.md, where a human reads it. The omissions are read from **the prose a person already reads**, not a parallel machine-readable list — a lint keeping its own copy of the answer would be this very defect, one turn further in. **IT FOUND TWO REAL HOLES ON DAY ONE**, which is the difference between a guard and a ceremony: `cargo test -p reflow2-core --no-default-features` (the in-memory backend on its own — a class `--workspace` does not catch) and `test_check_doc_versions`, both in `ci.yml` and in neither list. **The second is the trap the design anticipated**: `check_doc_versions` is a *substring* of `test_check_doc_versions`, so a substring match would have called it covered — matching is exact on the script stem, and that pair is pinned by a test. **THE CHECK CAUGHT A DEFECT IN ITSELF, on its own commit**: scanning the whole blockquote read the backticked words `cargo` and `python3` out of the new paragraph *describing the lint* and reported them as gates CI had stopped running. Fixed by scoping omissions to the note's first paragraph. **AND ONE REDUNDANCY IS REPORTED RATHER THAN HIDDEN** — reverting that scoping *alone* leaves every real-file check green, because the runner-word exclusion happens to cover the only two words today's prose backticks; the two are not redundant in general, so the scoping is pinned on synthetic text where the redundancy cannot mask it. New hermetic suite `tools/test_skill_lint.py` (14 cases), wired into `ci.yml` — **and its own name had to be declared in AGENTS.md before the lint would go green, which is the mechanism working on the commit that introduced it.** Mutation-checked seven ways, each failing exactly the check that names it. | S |
| **BL-157** | **Establishing a FIRST drift baseline has no disposition, so it has to be recorded as an accepted change that never happened — DONE 2026-08-01** | *Found 2026-08-01 while closing the `art:detect` hole (BL-153's session).* `set_artifact_checksum` requires a `disposition`, and both answers assume there **was** a baseline: `design_holds` means *the code moved and the change carries no design meaning*, `design_updated` means *behaviour moved and the design moved with it*. Neither is *"nothing moved — this artifact was registered with no checksum and is getting its first one"*. `design_holds` was used because it is the least wrong, and its `change_type` enum (`test_failure_fix` / `refactor` / `performance_optimization`) made it worse: the record now says `refactor` about a file this session did not touch. **The disposition vocabulary is right for what it was built for** — `dec:two-sided-accept` exists because silent accept is how a design erodes — **and a first baseline is simply not an accept**, so forcing it through the accept path writes a small fiction into exactly the ledger that exists to prevent fiction. **Fix shapes:** (a) a third disposition, `baseline_established`, recorded as a dated claim with no change_type; (b) let `link_artifact`/`add_artifact` refuse a code artifact with no checksum, so the state cannot arise — heavier, and wrong for artifacts that genuinely cannot be hashed; (c) leave it and document the convention. **(a) is cheap and is the honest one.** Note the shape is the same as [BL-115]'s: two states that are genuinely different rendering identically because the vocabulary has no word for one of them. Size **S** — **BUILT 2026-08-01 as (a), and it cost one schema enum value and no stamp movement.** `DriftDisposition::BaselineEstablished` takes no `change_type` and writes `ChangeEvent.change_type = baseline_established`. **FIX SHAPE (b) WAS FOLDED IN WITHOUT ITS COST:** the row rejected (b) — refusing to register a code artifact without a checksum — as "heavier, and wrong for artifacts that genuinely cannot be hashed". Refusing the wrong *disposition* rather than the registration gets (b)'s guarantee with none of that: an accept against no baseline is refused naming `baseline_established`, and a `baseline_established` that would MOVE an existing baseline is refused naming the other two. **THAT SECOND GUARD IS WHAT MAKES THIS A FIX AND NOT AN OFF SWITCH** — without it the new word would launder real drift past `dec:two-sided-accept`. Re-establishing the SAME baseline stays idempotent so a re-run is safe. Also refused by `add_change_event`/`record_change`, so the label cannot be hand-applied to an ordinary change and the ledger's count still measures something. **AND THE LEDGER ARITHMETIC NEEDED THE SAME CARE AS THE VOCABULARY:** `baseline_claims` is counted apart from `design_holds_claims` (folding it in would report a judgement nobody made), and a first baseline is deliberately kept OUT of `last_claim_at`, because that field is read as "the newest accepted change to the code this check covers" — letting it in would mark every passing check stale the moment someone registered a checksum that had been missing all along. **FOUND BY THE GUARD, worth keeping:** two `changelog.rs` fixtures had passed `"sha256:aaaa"` as `add_artifact`'s fourth argument, which is the LOCATION — they had no checksum at all and their accepts had been accepting nothing for as long as they existed. Nothing had ever checked that an accept had something to accept. 13 cases in `confirmation_vocabulary.rs`, mutation-checked five ways. | S |
| **BL-158** | **A clean `reconcile_artifacts` cannot clear its own loop debt — [BL-134] reproduced on reflow2's own design — DONE 2026-08-01, and it SUPERSEDES [BL-134]** | *Reproduced 2026-08-01, first-hand, which [BL-134] did not have.* An **exhaustive** sweep of all 107 registered artifacts reported **106 unchanged, zero drift** — and `loop_status` afterwards still said *"1 built capability never checked against reality (reconcile_artifacts)"*, naming the same capability as before. The sweep was the most complete one this design has ever had and it moved the number by zero. **The mechanism is now visible:** `record_events` only records on *divergence*, so a clean pass writes nothing, and the ledger's `unexamined` state is computed from recorded claims — which a clean reconcile never produces. So the operator who checks everything and the operator who checks nothing see byte-identical output, and the loop keeps asking for a pass that has just been run. **This is [BL-134] with evidence rather than inference; fold it in or supersede it.** **Fix shape:** a clean reconcile records a dated *claim* (not a DriftEvent) against the artifacts observed — the confirmation ledger already has the vocabulary for it (`design_holds_claims`, `last_claim_at`), and `set_artifact_checksum` already writes exactly that shape when a checksum is re-accepted unchanged. The honest counterweight: it must record what was ACTUALLY observed, so a partial sweep cannot read as a full one — which is what `exhaustive` already distinguishes. Size **S** — **BUILT 2026-08-01, and the fix shape held except in ONE respect worth recording.** The row proposed recording the claim the way `set_artifact_checksum` does; that would have been a `ChangeEvent`, and **a ChangeEvent for a pass where nothing changed is the same fiction [BL-157] is about, one layer over.** So the confirmation is a PROPERTY — `Artifact.last_confirmed_at` — stamped by a recording reconcile on every clean match, returned in `DriftReport.confirmed`, and read by the ledger as `confirmations` / `last_confirmed_at`. **The property is the right shape for a second reason the row did not name: a confirmation is high-frequency and says nothing changed, so a node per artifact per pass would bury axis Z — the log of what actually MOVED — under non-events.** `Verification.last_run_at` is the existing precedent for exactly this. **THE COUNTERWEIGHT THE ROW ASKED FOR, HONOURED THREE WAYS:** a partial sweep confirms only what it looked at, a drifted artifact is never confirmed, and an undated pass writes NOTHING and returns those artifacts in `unconfirmed_undated` rather than dropping them silently — a skipped write the caller has to infer from a count that did not move is the exact shape of the bug being fixed. **THE PAYOFF IS PINNED BY A TEST THAT REPRODUCES THE ORIGINAL STATE:** an artifact registered with a checksum and never touched since is `unexamined` — which is where `cap:skill-triggers` sat while `loop_status` asked, pass after clean pass, for a check that had already happened — and one clean reconcile now moves it to `confirmed`. 13 cases in `confirmation_vocabulary.rs`, mutation-checked five ways. | S |
| **BL-114** | **A detector's message claims more than the detector checked** | *Found 2026-07-31 by an adopt pass over a real design-review document in another organisation — a pre-construction design with no code — and independently reproduced in reflow2's own graph the same day. Redacted of all source specifics.* **Two detectors report a narrow check in wide words.** `disconnected_component` said *"component is not connected to anything"* about a component that had a `CONTAINS` parent and siblings in the same subsystem; it meant *no runtime path*. `orphan_node` said a document Artifact *"realizes nothing"* — true and irrelevant: it is a specification, it `DOCUMENTS` the project, and `REALIZES` is the only edge the detector counts. A 63-node cluster was likewise reported *"disconnected from the rest of the design"* while its parent subsystems carried `DEPENDS_ON` edges outward and everything shared one Project. **In every case the machinery was right and the sentence was wrong.** **Reproduced here the same day:** `disconnected_community` fired on a Requirement that had just gained an acknowledgement Decision, reporting an island for two nodes sharing the Project — the [orphan-requirements-hide-from-heal] shape from the other side. **This is not a detector bug; it is the class `req:schema-prose-is-checked` already names** — prose that promises breadth the code does not implement, which is exactly why `Project.mode`'s description was fixed at the source rather than annotated. Same defect, different surface: there it was schema prose, here it is finding prose. **Fix shape:** every finding states the edge kinds it considered — *"no runtime (PROVIDES/CONSUMES/DEPENDS_ON) path; containment edges exist but do not count"*. Cheap, changes no computation, and it is the difference between a finding a user acts on and one they learn to distrust. **The `orphan_node` half may additionally be a real rule gap:** `DOCUMENTS` arguably *is* ownership for `artifact_type` of `document`/`spec`/`diagram`, and if so the detector should say so rather than every design document reading as an orphan forever. Decide that separately from the wording. **THE CONTROL EXPERIMENT THIS ITEM LACKED, supplied 2026-07-31 by a case study designed end to end through reflow2:** `propagate_change` returned `impacted: []` for a change that materially activated a regulatory Constraint sitting three nodes away — correct given the edges, badly wrong about the world, because the CONSTRAINS edge had never been drawn. Later in the SAME session, same tool, ~16 nodes both times, with edges present, it produced a genuinely valuable answer with correct upstream hop chains that caught two requirements about to be orphaned. **Edge density, not graph size, decided whether the tool was useful — and the output is identical for "nothing depends on this" and "you never drew the edges", which is the overwhelmingly common case early in a design's life, i.e. exactly when users decide whether to trust the feature.** Same disease as this item: the machinery was right and the sentence claimed more than it checked. **Fix, one line:** when few or no nodes are impacted, report the seeds' connectivity — *"0 impacted; note these seeds carry 2 edges total, so this design may be under-linked rather than genuinely isolated"*. **The data already existed:** `detect_defects` had ALREADY flagged that exact node set as a disconnected community in the same session. Two detectors knew and neither told the tool that mattered. **INDEPENDENT CORROBORATION OF THIS ITEM'S PROPOSED FIX, arrived at from the other end (2026-07-31, same case study):** `disconnected_community`'s SUGGESTION does not know about the detector's own SELECTIVITY. Connectivity is measured over traceability edges; `MITIGATES`/`RISKS`/`CONTRADICTS` do not participate. So a user told to bridge the island draws a TRUE edge of a type that does not count, re-runs, and gets **nothing back** — not "closer", not "that edge does not count", nothing. The only edge types that would clear the finding are ones that would be FALSE for that cluster. **The advice's only correct implementations are excluded from the measurement.** The underlying position is right — an unselective topology detector fires everywhere and means nothing, and `structure.rs` says so — but the message never mentions it. This is exactly the fix already proposed above, reached independently: *state the edge kinds considered*. **Plus a refinement worth taking:** if the island already carries non-traceability edges outward, say so — *"semantically connected to the design but structurally isolated"* is a completely different finding from *"isolated"*, and in that project it was the true one. Size **S**  **INDEPENDENTLY REPRODUCED 2026-08-01 by an /adopt pass over `music_graph`, a different project in a different domain, and it is the `orphan_node` half almost word for word.** Four artifacts reported *"realizes nothing"*: a doc that DOCUMENTS the project, a doc that DOCUMENTS a capability, one that is PRODUCES-d by a Verification *and* DOCUMENTS, and a MIDI corpus that SATISFIES a requirement. **All four were correctly linked — just not with `REALIZES`**, which the Artifact hint itself says is only one of four valid link types (REALIZES / SPECIFIES / DOCUMENTS / PRODUCES). **Verified in source the same day: `heal.rs` checks outgoing `REALIZES` alone.** Their consequence is the one that raises this item's priority, and it is the BL-23 lesson arriving through a third door: *"acknowledging four defects that are the schema working as designed trains you to skim the defect list."* **Two unrelated repos have now hit this, so the `orphan_node` half is no longer a may-also-be — it is a rule gap with two witnesses, and counting any of the four edge types is the fix.** | S |
...
| **BL-117** | **The import/export document shape is undiscoverable without reverse-engineering it — DONE 2026-08-02** | *Found 2026-07-31 by the same external adopt pass, which had to create a throwaway node and edge and re-export the graph purely to learn the encoding.* An export of an EMPTY graph shows the envelope and nothing about per-node or per-edge shape, so an agent building a document by hand — which the `adopt` skill explicitly recommends, *"build one export document and `import_graph` it once"* — cannot learn `{node_id, node_type, properties}` / `{edge_type, from_id, to_id, properties}` from any served surface. **The skill recommends a path the tools do not document.** Two wasted round trips, and worse for anyone without a scratch graph to burn. **Fix shape:** a document-format section in `describe_schema`, or in `import_graph`'s own description — which is also `cap:tool-carries-convention`'s exact argument, a convention an agent would never guess arriving with the call. Size **XS** — **BUILT 2026-08-02.** The document shape now rides `import_graph`'s OWN description, which is `cap:tool-carries-convention`'s exact argument and the row's own recommendation: the envelope in full, that `graph_id`/`stamp`/`content_hash`/`prev_content_hash` are ALL optional on the way in and `edges` may be omitted, that endpoint types are recovered from the document or the graph rather than stored on the edge, and a pointer to `describe_schema` for per-type properties. **It also carries [BL-139]'s honest half** — each node must be COMPLETE, unlike `create_node` where a partial props object edits — so the one thing most likely to bite on the corrective re-import is stated at the door whichever way the merge question goes. | XS |
| **BL-118** | **`import_graph` validation is fail-fast, one error per attempt — DONE 2026-08-02** | *Found 2026-07-31 by the same external adopt pass: FOUR consecutive imports each failed on the first error found — a missing stamp field, then three different enum violations — over a hand-authored 9,000-line document.* Every attempt is a full edit-retry cycle for one error, so a document with N faults costs N round trips. **Atomicity is not the problem and must not be touched** — the same session named it as one of the things that worked notably well, nothing half-loaded across four failures. The problem is that validation stops at the first fault when it could collect them all. **Fix shape:** validate the whole document and report every violation in one response, keeping the all-or-nothing write. Four round trips become one. Size **S** — **BUILT 2026-08-02, and it needed no new decision.** `dec:bulk-is-all-or-nothing-with-per-item-findings` ([BL-153]) already IS this shape — every item attempted so you learn every failure at once, and if any failed nothing is written — and BL-153's own row noted that `import_graph` files the opposite defect. The shape was reused rather than reinvented. Faults carry their POSITION (`nodes[1]`, `edges[0]`) so a 9,000-line document can be navigated, and bad edges are collected beside bad nodes. **IT STILL RETURNS AN ERROR rather than an ok-with-failures report, deliberately:** a caller treating a rejected import as success would be exactly the silent failure AGENTS.md rule 4 forbids, so the whole list rides the error. **THE ATOMICITY THE ROW SAID MUST NOT BE TOUCHED IS PINNED INDEPENDENTLY** — mutating the reject path to commit the valid items fails THREE tests including the pre-existing `a_bad_document_imports_nothing`, which is the guarantee the reporting session named as working notably well. | S |
| **BL-119** | **The stamp requirement is wrong for a hand-authored import document — CLOSED 2026-08-02: ALREADY FIXED, by `chg:bl87`** | *Found 2026-07-31 by the same external adopt pass: rejected with "missing field `node_types` at line 9001".* For a round-tripped export, demanding the full type-name inventory is right — it is what `dec:stamp` exists for, and BL-19/BL-94's lockout depends on it. For a document an AGENT builds, it means copying boilerplate the server already knows, and the `adopt` skill recommends exactly that path. **The two cases are genuinely different and the code does not distinguish them.** **Fix shape:** make the stamp optional on import, or accept a minimal one (`schema_version` alone), and validate the content against the LIVE schema — which is the stronger check anyway, since a hand-copied inventory proves nothing about the document beneath it. **Constraint that must hold:** a document that DOES carry a full stamp must still be refused when it disagrees, or the lockout guarantee is gone. Size **S** — **CLOSED WITHOUT A BUILD, because the fix shipped before the row was triaged.** `chg:bl87` (*"import tolerates an unstamped document — stamp optional + reported, like content_hash"*) did exactly what this row asks: `GraphExport.stamp` is `Option<GraphStamp>`, absence is a first-class state reported through `ImportReport.provenance_note`, and `ver:export` pins it (`a_stampless_document_imports_and_is_reported_unstamped`). **The row's stated constraint also still holds** — a document carrying a FULL stamp is still checked against the live vocabulary, so BL-19/BL-94's lockout is intact. **Flagged by Anthony 2026-08-02 and CONFIRMED IN SOURCE rather than taken on his word**, which is how the same check then found [BL-138] half stale for the identical reason. **The transferable lesson is about the BOARD, not the code: a row can go stale between filing and triage, and nothing on this project notices.** Two of five rows in one cluster had been overtaken. Reading the row is not the same as reading the code. | S |
...
| **BL-129** | **`Interface.medium` is unreachable from BOTH tools that create and specify Interfaces, and its default silently manufactures false SPOF warnings — DONE 2026-08-01** | *Found 2026-07-31 by following the obvious path.* `add_interface` accepts only `id` and `name` (`graph.rs:720`). `set_interface_spec` fills paradigm, payload format, schema, endpoint, operations, auth, transport security and error model (`service.rs:4784-4800`) — **and not `medium`.** So the one property AGENTS.md explicitly warns about — *"mark a shared package `library`, because a library linked into its callers cannot fail on its own, and the structural detectors need to know that to avoid calling it a single point of failure"* — is reachable only through `create_node`. **A user who follows the obvious path leaves every interface at `unspecified` and collects false SPOF warnings later, having done nothing wrong**, which is the punishing-correct-work shape of [BL-23]. Note the seam work already had to fix `medium` defaulting to a flattering value (`ver:seam-incompatibility`: *"two silent boundaries agreed on a value neither had chosen"*) — the honest default is in place and the field is simply unwritable from where people stand. **BUILT 2026-08-01 on `set_interface_spec`**, not on `add_interface`: `medium` is part of what a consumer must AGREE with — that tool's stated subject — and every other contract property already lives there, so `add_interface` stays minimal and there is one way to do this rather than two. Omitting it leaves the stored value alone, like every other field on that tool. **The behaviour was never broken and is not what was built:** the property, its honest `unspecified` default, the seam comparison and the foundation exemption all already worked — the existing SPOF test proves it and builds its fixture with `create_node`, which was the workaround. **So the new test is a REACHABILITY test:** the same two-subsystem bridge built the way a user actually builds it (`add_interface` then `set_interface_spec`) now earns the same exemption for `library` and `data`, still fires when the medium is never stated, and still fires for `REST` — the counterweight that stops a stated medium buying an exemption it has not earned. Minor by the versioning table (the surface gained a parameter), no schema change, no stamp move, toolsnap regenerated deliberately. Size **XS** | XS |
...
| **BL-136** | **A value FITTED to a piece of evidence can still be cited AS VALIDATED BY that evidence — circular validation is invisible, and it is [BL-106]'s third axis — DONE 2026-08-01** | *Found 2026-07-31 by a project that hit it, invented its own fix, and then had that fix catch the same author committing the error again one entry later.* **The shape:** a design node whose value was DERIVED from an anchor should not be able to count as VERIFIED by that anchor. reflow2 has `Verification` nodes and provenance values for how a node ENTERED the graph, and nothing that expresses *"this check is circular with respect to that parameter"*. **Concretely, in that project:** a layout coefficient was fitted to one published anchor; agreement with that anchor immediately stopped being a test and became a fit; the framework's only footprint check was consumed and every status still read green. They added a `CALIBRATED` provenance carrying `calibrated_against`, and made the report print the anchor as *[CONSUMED — a fit, not a test]*. **Their own diagnosis of why it matters, which generalises past quantum computing:** in ANY empirically-calibrated design — control systems, simulation models, anything with fitted constants — this is the standard way a design becomes fiction while nothing reports a gap. **This completes a family, and the framing should match the other two:** [BL-106] is a check gone stale along the TIME axis; [BL-126] is a check narrow along the INPUT axis; this is a check that is not INDEPENDENT of what it validates. All three are the same underlying hole — **the quality of the evidence is not a fact the graph holds**, only its existence and its status. **The closest existing thing is `unvalidated_capability`** ("built right, but the right thing?"), which that project named the single most valuable prompt of its session — and which cannot see this, because a circular check is a check. **Fix shapes:** a `calibrated_against` relation from the fitted node to the evidence it consumed, and a detector that reports a `VERIFIES` edge whose target was calibrated against the same evidence. Adjacent to [BL-120](d) — both are provenance being unable to say where a number's AUTHORITY comes from. **SHARPENED 2026-07-31 by that project's closing entry, and it escalates the item.** When a verified source finally arrived, the fitted coefficient turned out not to be merely mis-valued — **the FUNCTIONAL FORM was wrong.** The real relation was a tile count for a BLOCK with a constant term that does not scale at all; the single fitted per-unit coefficient had been silently absorbing that structural error and agreeing with the data anyway. Their conclusion, which is the finding: ***a single-point fit does not merely get a number wrong, it can silently hold a wrong EQUATION in agreement with the data.*** **And the part that decides what reflow2 can even aspire to here:** they had built four independent internal diagnostics — a back-solve, a coefficient locus, a space-time frontier, and a physical-floor proof — *"every one narrowed the question correctly and none could have found this. Only the source could."* So a circular check is not a weak check to be strengthened from inside; **no internal evidence can detect it, by construction.** That rules out a whole class of fix and points at the only one that works: make the circularity STRUCTURALLY VISIBLE — record what a value was calibrated against, and refuse to let that same evidence count as its validation — rather than trying to detect it analytically. It also raises the stakes: the failure is not a number you cannot trust, it is a model that agrees with reality for the wrong reason while every status reads green. Size **M**  **DONE 2026-08-01, as the family's INDEPENDENCE axis.** `CALIBRATED_AGAINST` (new edge type, 57→58, moves the stamp) records what a value was FITTED to; `evidence_report` marks any passing check that IS, or PRODUCED, that evidence as `[CONSUMED — a fit, not a test]` and excludes it from independent evidence, so a claim whose every check is consumed reports as not independently verified. **The closing evidence decided the shape and ruled out the alternatives**: four independent internal diagnostics, every one narrowing the question correctly, none able to find it — so this is recorded and never detected, because no check inside a design can establish its own independence. An edge rather than the `calibrated` provenance value because the detector is then a graph query and the relation carries WHAT was consumed; the provenance half (BL-120(d)) stays open and is not foreclosed — it answers HOW the value entered, a different question. **And the impact-table question was asked BEFORE the code was written** (`dec:calibration-propagates`): `CALIBRATED_AGAINST` is a traceability edge, so correcting an anchor reaches every value fitted to it, which is the one moment a wrong functional form hiding in a fitted coefficient becomes findable. `INCLUDES` and `SCHEDULED_FOR` each reached that table only after a detector complained; this is the third such edge and the first asked in advance. **On reflow2's own design: 0 consumed — the design carries no fitted values, which is a measurement, not an assumption.** | M |
| **BL-137** | **`changelog_view` silently DROPS a ChangeEvent that is not pinned to an epoch, breaking its own no-silent-drop promise** | *Found 2026-08-01 while preparing the v0.20.0 cut — by the release the tool exists to serve.* The tool's own description promises that *"anything no rule covers comes back in `unmapped` rather than being guessed or dropped."* Run on the `rel:v0190 → now` window it returned **6 entries, `unmapped: []`, and none of the three fixes about to ship** (BL-124, BL-125, BL-129). The cause is exact and was confirmed against the export: the four ChangeEvents it surfaced all carry `AT_EPOCH`; the three it dropped carry none. **A ChangeEvent with no epoch has no moment, so it cannot be placed in a window — that half is defensible. Vanishing is not.** Those three belonged in `unmapped` with the reason, and instead the draft read as complete. **Had the cut been taken from that draft, three fixes would have been absent from the changelog — which is precisely the failure `cap:changelog-view` was built to end, arriving through its own door.** (The same shape is already on the record twice: PR #7 shipped four tools with no `[Unreleased]` entry, and #9 and #11 are *still* unlogged, which is what prompted running the tool in the first place.) **Compounding cause, and the more useful half:** `add_change_event` never creates the `AT_EPOCH` edge, and nothing asks for one — so the default path produces exactly the unpinnable event this drops. [The release procedure names `AT_EPOCH` as the step that was already got wrong on the v0.19.0 cut and is still missing on v0.17.0]; [BL-122] built a detector for a *Release* with no epoch, and this is the same hole one node type over. **Fix shapes:** (1) report unpinned ChangeEvents in `unmapped` with the reason — cheapest, and it restores the stated contract; (2) let `add_change_event` take an epoch, or default it to the current one, so the honest path is the easy one; (3) a detector for a ChangeEvent with no epoch, mirroring `release_without_epoch`. **(1) is owed regardless** — the promise is already written down. Size **S** | S |
| **BL-156** | **"Export exactly once per COMMIT" is insufficient under the squash-merge the same document mandates, and it broke the chain on `main`** | *Found 2026-08-01 by the gate, immediately after merging PR #21 — the first PR in this project's history to carry two exporting commits.* AGENTS.md said **"Export the design EXACTLY ONCE per commit, straight onto the committed file"**, and that rule was followed exactly. It is still not enough. **PRs merge by SQUASH, so N commits become one on `main`, and only the LAST export's `prev_content_hash` survives.** PR #21 had three commits: a COORD claim (no export), BL-149..152 (export, linking correctly to v0.21.0's `799b06d9`), and BL-153..155 (export, linking to the *branch* intermediate `da60a633`). The squash kept the second export's link — pointing at a document `main` never saw. `main`'s chain now reads `cd6e6c8: 799b06d9` → `56bc698: prev da60a633`, and the gate caught it on the very next run with the right message. **WHY IT HAD NEVER FIRED BEFORE, and this is the uncomfortable part:** PRs #18 and #19 ALSO carried two commits each and were fine — because their first commit was the COORD claim, which touches no export. **They complied by accident, not by rule.** The rule as written has been insufficient for every multi-commit PR this project has ever merged; it simply had not yet met one where two commits both exported. **THIS IS THE `req:schema-prose-is-checked` FAMILY AGAIN, in the process layer** — a written instruction that is correct as stated and wrong in the workflow it is embedded in, exactly like [BL-114]'s finding prose and [BL-152]'s skill prose. Three surfaces now: schema, findings, skills, and process. **FIXED IN THE SAME CHANGE:** AGENTS.md now says once per PULL REQUEST, explains the squash mechanism, and states the corollary that the claim commit goes first. **THE CHAIN IS REPAIRED FORWARD, NOT BACKWARD:** re-exporting onto the committed file makes every future link sound, but the hole at `56bc698` is history and stays on the record rather than being rewritten — `main` is pushed and shared, and rewriting it to hide a hole in a history-integrity chain would be its own kind of lie. **What is NOT built, and is the real fix:** nothing enforces this. The gate catches it *after* the merge, on `main`, where it is expensive; a pre-merge check that a branch touches `docs/design/reflow2.json` in at most one commit would catch it while it is still free. That check is cheap (`git log --name-only origin/main..HEAD`) and belongs in CI on the pull-request trigger. Size **S**  **COROLLARY FOUND THE SAME DAY, while building BL-141(b) on a branch stacked on this fix:** a STACKED branch inherits its parent PR's exporting commit, so "one export per PR" is measured against **the `main` it will actually merge into**, not against the branch's own base. Building on an unmerged branch gives you two exporting commits relative to `main` and the same squash break. **The proposed check catches this unchanged** — `git log --name-only origin/main..HEAD -- docs/design/reflow2.json | grep -c` returns 2, which is exactly the signal — and that is a point in its favour: it measures the thing that matters rather than the thing that is easy to count. **The practical rule: land the parent before stacking, or rebase and re-export once before opening.** | S |
| **BL-153** | **53% of every tool transition is the same tool called again — half the surface's usage is a missing bulk form — DONE 2026-08-01, ALL THREE FIX SHAPES** | *Measured 2026-08-01 by `tools/surface_usage.py`, the sixth instrument and the first to measure the SURFACE rather than the engine — 46 retained Claude Code transcripts, 2026-07-18 to 2026-08-01, **6,095 reflow2 tool calls across 10 projects**. Anthony's idea.* Of **5,005** tool-to-tool transitions, **2,650 (52.9%) are self-loops**. The offenders, as self-loops · total calls · sessions: `release_includes` **988 · 1008 · 7** (~144 per session), `set_artifact_checksum` 244 · 408 · 22 (~18), `create_node` 112 · 209 · 19, `contains` 109 · 176 · 11, `acknowledge_gap` 90 · 157 · 12, `gap_to_prompt` 83 · 118 · 12, `contain_component` 77 · 87 · 5, `satisfies` 74 · 164 · 20. **NOBODY CALLS ONE TOOL 144 TIMES IN A SESSION BECAUSE THEY WANT TO.** **AND IT IS NOT AN ARTIFACT OF REFLOW2'S OWN RELEASE BOOKKEEPING — it reproduces in every consumer project independently**: QBI's top two calls are `contains(76)` and `set_artifact_checksum(68)`; brainmaker's are `create_node(75)` and `contain_component(66)`; 3dtictactoe's are `contains(24)`, `allocate(21)`, `satisfies(15)`. Five projects, same shape, none of them cutting releases. **The release-manifest case is already documented as a known cost** — [the release-cut procedure] says the manifest "is 151 edges and nothing computes it for you" and budgets *"about ten batched messages"* for it — but it was recorded as a chore rather than as a defect in the surface, and the measurement shows it is the largest single line item in reflow2's entire recorded usage. **Fix shapes, and they are not all the same fix:** (1) a genuine BULK form for the worst four (`release_includes`, `set_artifact_checksum`, `contains`, `create_node`) — a list parameter, one call, one refusal semantics to define (all-or-nothing, or per-item findings?); (2) for `release_includes` specifically, the manifest is DERIVABLE — every artifact and component of the project, with each artifact's current checksum frozen — so the right fix is `release_includes_all` or a manifest-from-graph call, not a faster loop; (3) `gap_to_prompt` and `acknowledge_gap` batching suggests the detect→ask→acknowledge round trip wants a multi-gap form. **The counterweight to state before building:** `dec:two-sided-accept` and `dec:ask-not-repair` mean a bulk accept must not become a silent one — a batch of 144 acknowledgements with one reason is exactly the erosion those decisions exist to prevent, so a bulk form needs per-item dispositions or it is worse than the loop. **FIX SHAPE (2) BUILT 2026-08-01** — `release_includes_all` (`cap:derived-release-manifest`, `dec:manifest-derived-is-not-manifest-accepted`, `ver:derived-release-manifest`, 8 cases, mutation-checked three ways). **Measured on reflow2's own 1200-node design over the real MCP binary: 160 `INCLUDES` edges in ONE call**, re-run adds 0, and a typo'd exclusion is refused by name. **The counterweight was examined and found not to bite:** those decisions bound bulk *dispositions* — a judgement taken about each item — and a derivation takes none, so per-item dispositions would be ceremony; what does apply is the no-silent-write bar, hence `apply` defaults to false. **FIX SHAPES (1) AND (3) BUILT 2026-08-01** — `create_nodes`, `create_edges`, `set_artifact_checksums`, `acknowledge_gaps`, `gaps_to_prompts` (`cap:bulk-forms`, `ver:bulk-forms`, 14 cases, mutation-checked three ways). **Every self-loop this row named is now answered.** **`create_edges` is ONE tool, not six** — `contains`, `contain_component`, `satisfies`, `allocate` and `realizes` are thin wrappers that only fill in the endpoint types, so a bulk `create_edge` covers all of them, and [BL-155]'s 40-never-called count makes six near-identical tools a cost rather than a convenience. **THE REFUSAL-SEMANTICS QUESTION THIS ROW POSED WAS A FALSE CHOICE:** the answer is all-or-nothing **with** per-item findings — every item is attempted so the caller learns every failure in one round trip, and if anything failed the batch is discarded. The store already had the atomic batch HEAL's apply step and `import_graph` use, so all-or-nothing cost nothing; and collecting every failure is exactly the defect [BL-118] files against `import_graph` (*"fail-fast, one error per attempt"*), which a bulk form must not inherit — one error per round trip would replace N writes with N retries and save nobody anything. **THE COUNTERWEIGHT WAS HONOURED WHERE IT BITES:** `set_artifact_checksums` carries a disposition PER ARTIFACT and `acknowledge_gaps` a reason PER GAP, never hoisted to the call — 244 accepts now cost one call and still 244 decisions. `gaps_to_prompts` groups answers per gap for the same reason plus a mechanical one: it is what stops two gaps' prompt ids colliding, so no gap is replayed against another's answers, and a half-answered batch is refused rather than half-served. A rejected bulk write returns an ERROR carrying every failure in its `data`, never a payload with `applied: false` — a tool result reads as success. Size **M** | M |
| **BL-154** | **Skills are read once per 380 tool calls, and the skill for the tool used 92 times has never been read at all** | *Measured 2026-08-01 by `tools/surface_usage.py`, the sixth instrument and the first to measure the SURFACE rather than the engine — 46 retained Claude Code transcripts, 2026-07-18 to 2026-08-01, **6,095 reflow2 tool calls across 10 projects**. Anthony's idea.* **16 `get_skill` calls in 6,095 tool calls.** Eleven of fifteen skills fetched at all, and none more than **twice** across 46 sessions. **NEVER FETCHED: `link-artifacts`, `report-friction`, `ci-gate`, `kpp-proposal`.** **The `link-artifacts` case is the sharpest, because it is not a case of an unused feature:** `link_artifact` was called **92 times** and `set_artifact_checksum` **408 times** across 22 sessions — the WORK the skill covers is among the most-performed in the whole dataset, and its skill has never once been read. Whatever that skill knows is not reaching the agent doing the thing. `report-friction` is the same shape from the other side: friction gets filed constantly (BL-114..BL-152 are largely friction reports) and the skill telling you how to file it has never been opened. **This is `req:skill-use-survives-a-long-session` with a number attached at last** — that requirement exists because skill use was believed to decay, and the decay is now measured rather than suspected. **It is also the counterpart to [BL-152]:** that item found a served skill whose text had rotted; this one finds that almost nobody would have read it either way. **Fix shapes:** (1) the honest cheap one — the served tool descriptions already carry one-sentence conventions for a few tools ([the tool-carries-convention verification] is planned for exactly this), so extend that to the tools whose skills are never read, since the tool description IS read every session and the skill is not; (2) situation-shaped triggers, which is `cap:skill-triggers`, already planned and unbuilt — and this measurement is the evidence for prioritising it; (3) accept that skills are documentation for humans rather than instructions for agents, and stop counting on them — which would be a real decision and should be made deliberately if the numbers do not move. **Do NOT respond by nagging**: `ver:skill-triggers`'s own counterweight says a trigger that fires on correct work is the BL-23/BL-42 failure, and one read per 380 calls will not be fixed by more reminders. Size **M**  **CORROBORATED THE SAME DAY, and it is the sharpest evidence this row could have had.** Hours after `cap:gap-carries-a-reading` was BUILT — its six obligations written into `detect-and-ask` and enforced by `skill_lint` — **the agent that wrote them went on putting decisions to Anthony as prose**, until he pointed it out: *"we joked that this should be a reflow2 skill and I even think we recorded this, but I'm still having to remind you."* **The skill existed, was correct, was enforced, and did not reach the session that authored it.** That is this row's claim at its strongest: not that skills go unread by the inattentive, but that WRITING one does not make it operative even for its own author. **It is also the argument for [BL-152]'s sibling fix** — the convention that rides the TOOL arrives with every call, and a rule that lives only in a skill depends on someone choosing to load it. **When the format was finally used, both questions were answered in one reply with the recommendation taken** — versus the prose versions, which had produced several rounds of clarification. One observation, not a measurement, and recorded as such. | M |
| **BL-155** | **40 of 132 served tools were never called in the retained sample, and reflow2 cannot tell "unused" from "unreachable"** | *Measured 2026-08-01 by `tools/surface_usage.py`, the sixth instrument and the first to measure the SURFACE rather than the engine — 46 retained Claude Code transcripts, 2026-07-18 to 2026-08-01, **6,095 reflow2 tool calls across 10 projects**. Anthony's idea.* Never called: `add_environment`, `add_flow`, `add_resource`, `alternatives_for`, `analyze_alternatives`, `apply_merge`, `arrival_delta`, `budget_report`, `calibrated_against`, `claim_region`, `claim_report`, `collapse_decision`, `compose_and_analyse`, the four `content_*` tools, `decomposes`, `dimension_drift`, `evidence_report`, `ingest_step`, `merge_designs`, `mint_seat`, `mirror_surface`, `mirrors`, `pair_designs`, `part_of_flow`, `performed_in`, `plan_epoch`, `recall_resolutions`, `reconcile_dependencies`, `reconcile_deployment`, `register_alternative`, `release_claim`, `seam_report`, `set_epoch_status`, `set_evidence_scope`, `set_project_mode`, `set_requirement_designation`, `withdraw_defect_acknowledgement`. **THE CAVEAT IS LOAD-BEARING AND MUST TRAVEL WITH THE NUMBER: absence here is WEAK evidence.** Calibrated against a known case — `set_project_mode` appears as TEXT in ten transcripts and has ZERO recorded calls, although reflow2's own project was set to `rigid` with it. So a tool can be discussed far more than used, a tool used before the retention window is indistinguishable from one never used, and this session's own transcript was still being written. **What the list is good for is the QUESTION, not the verdict:** three distinct explanations sit in it and they want different answers. (a) **Genuinely young** — `calibrated_against` and `set_evidence_scope` shipped hours before the measurement and are not yet reachable from a running server, so of course they are zero. (b) **Whole subsystems nobody has had cause to drive** — the content store (4 tools), the claim/seat/merge family (7), composition and pairing (5). Each is built, tested and shipped; none has met its use case yet, which is a finding about the ROADMAP, not the tools. (c) **The uncomfortable middle** — `evidence_report`, `seam_report`, `budget_report`, `dimension_drift`, `analyze_alternatives`, `arrival_delta` are READS that answer real questions on designs that exist today, and nobody reaches for them. That is the same disease as [BL-154] one layer over: a capability nobody can find is worth what an absent one is worth. **Fix shape, and it is measurement before code:** re-run the instrument after retention improves, and for group (c) specifically ask whether the tool is unreachable from any skill or nudge that would name it — `evidence_report` gained three new axes this very release and appears in no skill. Size **S** | S |
| **BL-152** | **A served skill instructs a call the served tool made unnecessary a week earlier — the two halves of the served surface disagree** | *Found 2026-08-01 by FOLLOWING the `brainstorm` skill to record ten ideas, which is the only way this surfaces.* The skill's step 3 says: *"`set_decision_status` to `proposed`. **This call is not optional**: `add_decision` defaults to `accepted`, which would assert a settlement that never happened."* **`add_decision` has not defaulted to `accepted` since 2026-07-25** — its own served description says so in capitals: *"BEHAVIOUR CHANGED 2026-07-25: this used to default to `accepted`."* So an agent that follows the skill makes a redundant write on every brainstormed idea, and — worse — an agent that reads both is told two different things by the same server in the same session. **This is the [BL-114] / [BL-135] family arriving in the skills layer**: prose that describes behaviour the code no longer has. It is also precisely what `req:schema-prose-is-checked` names, one surface over — there it was schema prose, in BL-114 it was finding prose, here it is SKILL prose. **Why nothing caught it:** `skill_lint.py` checks the skills' *contract* with the served surface — that tool names resolve, that mirrors are byte-identical, frontmatter, the standing rule — but nothing checks that a skill's CLAIM ABOUT a tool's behaviour still holds, because that is semantic and the project deliberately keeps semantic quality in trials rather than CI. **Fix shapes:** (1) correct the sentence — owed regardless, and it is one line; (2) the general question, which is the interesting one: when a tool's default changes, what tells the skills? A grep for skill text quoting a tool default would be cheap and narrow, and would have caught this. **Worth noting the skill is otherwise correct and its reasoning still stands** — recording an idea as `accepted` would be a forgery; it is only the claim about the default that has rotted. Size **S** | S |
| **BL-149** | **A blast radius ranks by GLOBAL importance while claiming to rank by relevance to the change** | *Found 2026-08-01 by a review of what `dynograph-foundation` v0.12.0 offers that reflow2 does not use, at Anthony's request. The headline number: reflow2 consumes **8 of 17** `dynograph-graph` exports, **2 of ~35** `dynograph-vector` functions, and **none** of `dynograph-game` — there is more unused foundation than used. The full option set, including the ideas NOT filed, is recorded as `proposed` Decisions (`dec:idea-*`) so the roads not taken stay in the design's memory.* `propagate.rs:389` ranks impacted nodes by **betweenness centrality over the whole design network** — `design_network()?.betweenness()`. Betweenness answers *"how much of the golden thread routes through this node?"*, which is a claim about the node's standing in the design as a whole. **IP-9's question is different**: given the change I just made, what should I look at first? **`dynograph_graph::personalized_pagerank` answers exactly that and is already a pinned dependency, unused.** It is random-walk-with-restart: the teleport mass returns to a SEED SET rather than spreading uniformly, so every score is relevance *to those seeds*. Seed it on the change's own seeds and the ranking becomes what the field's doc comment says it is for. **Small**: the crate is pinned, the function is exact at this graph size, and `propagate` already builds the network it needs. **THE OPEN PART, and it is why `dec:idea-relevance-ranking` stays proposed:** betweenness is not WRONG, it answers a second real question — the existing doc comment defends it as predicting a wider *secondary* blast radius, i.e. onward propagation rather than relevance. So this is plausibly an ADDITION, and two ranking columns may serve a reader worse than one. Decide whether the output is both, one, or a blend before building. Size **S** | S |
| **BL-150** | **Nothing asks what order an increment can be built in, and `budget.rs` hand-rolls the algorithm that would answer it** | *Found 2026-08-01 by a review of what `dynograph-foundation` v0.12.0 offers that reflow2 does not use, at Anthony's request. The headline number: reflow2 consumes **8 of 17** `dynograph-graph` exports, **2 of ~35** `dynograph-vector` functions, and **none** of `dynograph-game` — there is more unused foundation than used. The full option set, including the ideas NOT filed, is recorded as `proposed` Decisions (`dec:idea-*`) so the roads not taken stay in the design's memory.* **Two findings, one function.** (1) **A simplification available today:** `budget.rs:312` implements Kahn's algorithm in about forty lines under a comment that says *"Kahn's toposort; leftovers mean a cycle"* — while `dynograph_graph::topological_sort` is exported, is Kahn's, is deterministic (the lexicographically-smallest valid order, so it is stable run to run) and returns `None` on a cycle. It was reimplemented because nobody knew it was there, which is itself the finding. (2) **A capability that does not exist:** capabilities form a `DEPENDS_ON` DAG that `detect_defects` insists stays acyclic, and **nothing ever asks what order that DAG implies.** reflow2 can say what is scheduled for an increment (`SCHEDULED_FOR`) and what has been delivered (the golden thread), but not *what can be started now*. **Lands on [BL-68]** (TRL/MRL readiness gating): that item asks whether a thing is ready, this asks whether its DEPENDENCIES are — the other half of the same question — and on epoch planning generally. **THE SHAPE IS OPEN (`dec:idea-build-order`), and the objection is real:** a topological order is ONE valid order among many, and presenting it as THE order would assert a schedule reflow2 has no business asserting — the overreach `dec:report-dont-judge` exists to prevent. The honest output is probably the **READY FRONTIER** (what currently has no unbuilt dependency) rather than a full ordering. Note also that (1) is not free: the local version works on string-id arcs while the foundation one wants a `Graph` built through `GraphBuilder`, so it trades forty tested lines for an adapter. Size **S** for (1), **M** for (2) | S–M |
| **BL-151** | **Severability is claimed as computed, and only the single-node case actually is** | *Found 2026-08-01 by a review of what `dynograph-foundation` v0.12.0 offers that reflow2 does not use, at Anthony's request. The headline number: reflow2 consumes **8 of 17** `dynograph-graph` exports, **2 of ~35** `dynograph-vector` functions, and **none** of `dynograph-game` — there is more unused foundation than used. The full option set, including the ideas NOT filed, is recorded as `proposed` Decisions (`dec:idea-*`) so the roads not taken stay in the design's memory.* `req:modularity-computed` says severability is computed rather than asserted, and `cut_structure` — the only thing computing it — reports **articulation points**: single nodes whose removal splits the network. That answers *"what is a single point of failure"*. It does not answer *"how separable are these two subsystems"*, which is the question modularity actually turns on. **`dynograph_graph::max_flow_min_cut` (Edmonds-Karp, exact, unused) returns the minimum-capacity set of EDGES** whose removal disconnects a source from a target — read here as *"which contracts would you have to sever to split A from B, and how many?"* Edge weights are already coupling weights, so the capacity means something rather than being invented for the occasion. **Lands squarely on [dec:mosa-conformance], which is OPEN and whose own text says reflow2 "already computes most of the criteria"** — this names one of the ones it does not. **TWO OBJECTIONS, both on the record at `dec:idea-severability-cut`:** min-cut needs a SOURCE and a TARGET and nothing in the design says which two subsystems a user wants separated, so either the user names the pair (a new question) or it runs over `leiden` community pairs (defensible, but the communities are themselves computed rather than declared); and **low cut capacity is not automatically good** — subsystems SHOULD be coupled where the design says so, and a detector rewarding separation everywhere would push toward a design nobody wants, which is the over-selectivity trap `is_single_point_of_failure` already had to be tuned out of. Size **M** | M |
| **BL-138** | **`import_graph` needs envelope fields the adopt skill never mentions, so the skill's central instruction fails if followed literally — DONE 2026-08-02** | *Found 2026-08-01 by an /adopt pass over `music_graph`, a ~11k-LOC Python research repo taken from an empty graph to 134 nodes / 379 edges. Reported in that project's own `reflow2_feedback.txt`.* **Blocking, and it blocks the one path the skill tells you to take.** The adopt skill's core instruction is *"build one export document and `import_graph` it once"*. Doing exactly that fails with `MCP error -32602: ... is not a reflow2 export document: missing field 'graph_id'`. A hand-built document must also carry `stamp` — 57 edge-type names, 28 node-type names, version, schema_version — and **none of that is returned by `describe_schema`, named by the error, or mentioned in the skill.** The only way the reporter found the envelope was to write a throwaway node with `add_project` and `export_graph` to see the shape. **`graph_id` and `stamp` are things the SERVER knows**, which is what makes requiring them from a hand author strange: the caller is being asked to restate the receiver's own identity. **Fix shapes, cheapest first:** (1) the error names the required fields — owed regardless, since [BL-132] already says error-message quality is the difference between a finding acted on and one worked around; (2) accept a minimal `{nodes, edges}` document and have the server fill its own `graph_id`/`stamp`; (3) the adopt skill carries a two-line skeleton. **(2) is the real fix** — a document that omits the receiver's identity is not ambiguous, and stamping it on import is what an import already means. Size **S** — **BUILT 2026-08-02, and HALF THE ROW WAS ALREADY STALE — verified in source, not assumed.** The row says a hand-built document *"must also carry `stamp` — 57 edge-type names, 28 node-type names, version, schema_version"*. **That requirement was already gone**, removed by `chg:bl87` (see [BL-119]); `stamp`, `content_hash` and `prev_content_hash` are all `#[serde(default)]`. Only `graph_id`, `nodes` and `edges` were still mandatory, so the row's quoted error `missing field 'graph_id'` was still exactly right and the remaining work was smaller than it read. **THE FINDING THAT MADE THE FIX OBVIOUS: `import_graph` NEVER READS `graph_id`.** It was demanded from the author and then ignored by the very operation demanding it — an import loads into the receiving graph, whose id the server already knows. `nodes` and `edges` also default now, so the skill's literal instruction (a document of nodes and edges, or even nodes alone) works. **THE COUNTERWEIGHT IS WHY THIS IS NOT SIMPLY DELETING A REQUIREMENT:** `mirror_surface` DOES read it, and still refuses an unidentified document BY NAME, because mirroring records where a surface came from (`mirror_of`) and guards against mirroring a design into itself — neither answerable without the id. **That is [BL-119]'s shape exactly: one requirement right for a round-tripped export and wrong for a hand-authored one, and the code now distinguishes them instead of applying one rule to both.** | S |
| **BL-139** | **`import_graph` validates whole nodes, so the adopt loop's corrective re-import is exactly where it breaks** | *Found 2026-08-01 by an /adopt pass over `music_graph`, a ~11k-LOC Python research repo taken from an empty graph to 134 nodes / 379 edges. Reported in that project's own `reflow2_feedback.txt`.* Re-importing an existing Requirement carrying only `{lineage: "derived"}` fails with *`Requirement.statement: required property is missing`*. `create_node` merges — its own description says *"a partial props object edits"* — and `import_graph` does not. **The adopt workflow is import → detect → correct → re-import**, so the mismatch lands precisely on the corrective step the skill prescribes. Adjacent to [BL-133], where two independent readings concluded statements were write-once: the edit path exists and keeps being invisible, and here it is invisible on the *bulk* door specifically. **Fix shapes:** make `import_graph` merge like `create_node`, or say plainly in the tool description that import documents must carry complete nodes. The second is honest and cheap; the first is what the workflow actually wants. Size **S** — **HALF DELIVERED 2026-08-02; the other half is ANTHONY'S and is recorded as `dec:import-merges-or-demands-whole-nodes` (proposed).** The row's own judgement was *"the second is honest and cheap; the first is what the workflow actually wants"*, so the cheap half shipped: `import_graph`'s description now says plainly that each node must be COMPLETE, and that this differs from `create_node` where a partial props object edits. **The merge half was NOT built on my signature, and the measurement says why it should not be.** It crosses THREE published boundaries — `ifc:core-api`, `ifc:graph-export`, `ifc:mcp-tools` — reported by `propagate_from("art:export")` at 705 impacted nodes and independently by this session's `propagate_change` at 723, both naming exactly those three. **And the cost no boundary count shows: merging makes import NON-IDEMPOTENT FOR DELETION.** Today a document is a complete statement of the nodes it names, so re-importing without a property removes it and an export→import round trip reproduces the design exactly; under merge, a property could be added or changed but never dropped, and restoring a backup would silently retain whatever the live graph had accumulated. That trades the migration and backup jobs this module exists for against a smoother adopt loop. **A middle path is on the record rather than skipped:** keep whole-node semantics as the default and let the DOCUMENT ask for merge explicitly. | S |
| **BL-140** | **`designation: required` means "supplied from outside" and `unprovided_interface` does not read it, so declaring the truth does not stop the alarm** | *Found 2026-08-01 by an /adopt pass over `music_graph`, a ~11k-LOC Python research repo taken from an empty graph to 134 nodes / 379 edges. Reported in that project's own `reflow2_feedback.txt`.* **Verified in source 2026-08-01:** `detect.rs:1165` counts incoming `PROVIDES` and incoming `CONSUMES` and never consults `Interface.designation`. **The schema already carries the answer the detector is asking for.** `designation: required` is documented in `structure.yaml` as *"this design NEEDS one of these FROM OUTSIDE; it is the half that was missing until 2026-07-30, and without it a design could say what it offered but not what it needed"*. Two interfaces marked `required` (`ifc:wav-audio`, `ifc:fluidsynth-cli`) still fired *"Nothing supplies X, but N parts rely on it"* at severity 0.72 — **the second-highest gaps in the run**. **The workaround is the finding:** the reporter invented a boundary Component (`cmp:external-fluidsynth`) purely to satisfy the detector, and named it correctly — *"that is modelling for the tool, not for the truth"*, which is the failure mode [BL-42] and [BL-23] both exist to prevent. **Fix shape:** an Interface designated `required` or `both` has already answered the question; it should not raise `unprovided_interface`. **The counterweight that must survive:** an `internal` interface with no provider is still a real break, and `internal` is the DEFAULT — so this must key on the designation actually being `required`/`both`, never on silence. Size **S** | S |
| **BL-141** | **A shared Interface manufactures phantom dependency cycles reported `critical`, four of them and zero real ones in a single adopt pass — DONE 2026-08-01, both halves** | *Found 2026-08-01 by an /adopt pass over `music_graph`, a ~11k-LOC Python research repo taken from an empty graph to 134 nodes / 379 edges. Reported in that project's own `reflow2_feedback.txt`.* **The detector the adopt skill sells as the payoff of a brownfield pass produced four findings, all false.** Two real modules with no import cycle between them produced `critical: cmp:instrument-detector → cmp:template-cache → cmp:instrument-detector`, because both PROVIDE and CONSUME one Interface modelled as a single `.npz` template format. Splitting that interface produced a SECOND phantom cycle through a shared WAV/MIDI interface; the renderer pass produced a third and fourth. **Every one had the same cause: ONE Interface node standing for what are really two contracts** — `ifc:midi-file` meaning both *"source MIDI we read"* and *"MIDI we emit"*. The renderer reads MIDI and writes WAV; the transcriber reads WAV and writes MIDI. **They are inverses sharing two file formats — a round trip, which is genuinely interesting — and reflow2 reported it identically to a real dependency cycle.** **VERIFIED IN SOURCE 2026-08-01, and the fix for (a) is nearly free:** `structure.rs:293` `dependency_pairs()` collapses `c CONSUMES i` + `p PROVIDES i` into the pair `(c, p)` — its own comment says *"the Interface is the medium, so it collapses into a direct dependency rather than appearing as a hop"* — **so the interface id is discarded at exactly the point the cycle edge is created**, and `heal.rs:868` prints only `A → B → A`. The datum needed to diagnose all four in seconds is present and thrown away. **(a) name the shared Interface(s) the cycle runs through**, which tells *"your model is coarse"* apart from *"your code is tangled"*. This is the same class as [BL-114]: a finding whose message claims more than the detector checked. **(b) the harder question, and a genuine design call:** should `PROVIDES`+`CONSUMES` through a `data`-medium Interface count as a dependency cycle at all? *"A file format is not a call graph. Two programs that read and write the same format have no runtime dependency in either direction."* **There IS a precedent and it is NOT automatically transferable:** `structure.rs:78` `is_foundation_medium()` already exempts `library` and `data` from `single_point_of_failure` — but its stated reasoning is SPOF-specific (*"you cannot run a second copy of it, so `add_redundancy` is incoherent"*), which is a different argument from the cycle one. Reachable now that [BL-129] made `medium` settable through `set_interface_spec`. **(a) is owed regardless of how (b) is answered.** Size **M**  **(a) DONE 2026-08-01** (`cap:cycle-names-its-basis`, `ver:cycle-basis`, 6 new cases of 15, mutation-checked three ways). Every `circular_dependency` finding now states its basis: the Interfaces any hop was collapsed out of, whether ANY hop is backed by a real `DEPENDS_ON` edge, and whether every Interface involved is a library/data medium. **Detection semantics are UNCHANGED** — the same cycles, the same severity; only the sentence grew, which is the BL-114 fix shape applied to this detector. **THE ITEM'S OWN DIAGNOSIS WAS HALF WRONG, AND ONLY MEASURING IT SAID WHICH HALF — keep this as method, it is the second time (see [BL-124]).** The reporter wrote that every phantom was *"ONE Interface node standing for two contracts"*, so the first build keyed the discriminator on the INTERFACE COUNT: one shared contract for the whole loop = coarse model. **Then their actual cycle was reproduced on their real design and it has TWO interfaces, not one.** `cmp:fundamental-detection → cmp:midi-renderer → cmp:fundamental-detection` runs through `ifc:midi-file` AND `ifc:wav-audio` — a renderer that reads MIDI and writes WAV against a transcriber doing the reverse. **That is structurally IDENTICAL to a genuine two-contract service cycle**: same nodes, same edges, same interface count. The interface-count discriminator misses the very case the item was filed about. **What separates them is `medium`, and nothing else** — both of theirs are `data`. So the finding now reports the medium, with a counterweight test pinning that the same shape over `REST` makes no such claim. **Verified end to end on their real data**, by reconstructing their pre-workaround model from a read-only `--export-snapshot` of their live graph (their committed design has 0 cycles — they had already worked around all four by splitting interfaces, which is why the fix arrives after the cost was paid). **(b) REMAINS OPEN AND IS NOW THE LOAD-BEARING HALF.** Since only the medium distinguishes a file-format round trip from a service cycle, the question *"should `PROVIDES`+`CONSUMES` through a foundation medium count as a dependency cycle at all?"* is no longer a nicety — it is the difference between a detector that is right and one that merely explains itself well. Wants Anthony's word; `is_foundation_medium`'s existing exemption is argued on SPOF-specific grounds (*"you cannot run a second copy of it"*) that do not transfer unexamined.  **(b) DONE 2026-08-01, on Anthony's call from three options** (`dec:foundation-cycle-is-a-warning`, 4 new cases of 19, both mutations caught). **DOWNGRADED, NOT SILENCED — that distinction WAS the decision.** `circular_dependency` drops Critical→Warning when BOTH hold: no hop is a real `DEPENDS_ON`, and every Interface in the loop carries a foundation medium. **Option (c), suppressing the case, was REJECTED** because shared-data coupling is sometimes genuine — two services over one table are truly entangled, since a schema change in one breaks the other — so the finding keeps its place, affected set, suggested fix and explanation, and loses only the claim that it is an emergency. **Option (a), keeping Critical and relying on (a)'s improved message, was REJECTED** because a severity is a claim about what a reader should DO: prose cannot repair a wrong word, and a severity that is wrong four times out of four cannot be filtered on. **TWO GUARDS, both pinned:** `contracts_only` is load-bearing — one genuine `DEPENDS_ON` anywhere keeps the whole loop Critical, because a real dependency does not stop being one by sharing a loop with a data contract; and `Interface.medium` defaults to `unspecified`, which is NOT a foundation medium, so **silence about the medium keeps the LOUDER answer** and a design that never classified its boundaries is not quietly excused. **MUTATION-CHECKED BOTH WAYS:** drop the `contracts_only` guard and exactly the mixed-cycle case fails; downgrade every cycle and **FOUR** fail, including the pre-existing `a_cycle_is_critical_and_never_auto_applied` that has asserted Critical since the detector was written — which is the proof this is not a way of turning the detector off. **THE PRECEDENT WAS DELIBERATELY NOT LEANED ON:** `is_foundation_medium`'s SPOF exemption argues *'you cannot run a second copy of it, so add_redundancy is incoherent'*, which is about whether a FIX is possible rather than whether a DEPENDENCY exists; the decision stands on its own reasoning. **VERIFIED ON THE ORIGINATING PROJECT'S REAL DATA:** their reported loop `cmp:fundamental-detection → cmp:midi-renderer → cmp:fundamental-detection` through two `data` contracts now reports WARNING with its explanation intact. | M |
| **BL-142** | **`describe_schema` on one node type returns ~30 KB, and the compact escape hatch drops exactly what the caller needed** | *Found 2026-08-01 by an /adopt pass over `music_graph`, a ~11k-LOC Python research repo taken from an empty graph to 134 nodes / 379 edges. Reported in that project's own `reflow2_feedback.txt`.* `describe_schema {node_type: "Fragment"}` inlines every wildcard edge (`from: ["*"]`) in both directions — about 25 edge types repeated with full property lists — **for a node type with 9 properties of its own**. `required_only: true` is the compact form and **drops the edges entirely**, so there is no *"properties plus the edges that specifically model this type"*. The no-argument schema is 67 KB and overflowed into a persisted file. **This is the cost side of the answer to [BL-1]**: discovery was the right thing to build and the bill is now paid on every adopt session, in a context window that has other work to do. **Fix shape:** a middle setting that returns the type's own properties plus edges whose endpoints NAME it — the `exact`/`half_exact` distinction `vocabulary.rs` already computes for `describe_schema {from, to}`. The wildcard edges are the bulk and are the least informative, which is precisely the judgement that module was written to make. Size **S** | S |
| **BL-143** | **No vocabulary for "it runs but the answers are wrong" — so an opinion gets recorded as a measurement, which is the evidence-quality family's FOURTH axis** | *Found 2026-08-01 by an /adopt pass over `music_graph`, a ~11k-LOC Python research repo taken from an empty graph to 134 nodes / 379 edges. Reported in that project's own `reflow2_feedback.txt`.* **This repo's central fact is that everything executes and almost nothing is correct**, and `Verification.status` offers only `planned`/`passing`/`failing`/`skipped`/`blocked`. The reporter recorded a smoke run as `passing` (it truly does run) and an accuracy measurement as `failing` — **but nothing in the repo states an acceptance threshold, so `failing` is a judgement smuggled in as data.** Their words. The caveat went into the description, *"where no detector reads it"*. **THIS COMPLETES THE [BL-106]/[BL-126]/[BL-136] FAMILY WITH A FOURTH FACE.** Those three say a check can be stale in TIME, narrow in INPUT SPACE, or not INDEPENDENT of what it validates — all answering *how good is the evidence*. This one is one step earlier: **there is no criterion, so there is nothing for the evidence to be evidence OF.** A `failing` with no threshold and a `failing` against a stated one are indistinguishable, which is the same flattering-collapse the family exists to break. **Corroborated from the other side by [W7] in the same document:** having to write *"marked failing because the owner says so, NOT because a threshold exists"* into a description felt wrong enough that it became the question put to the owner — **which produced that project's first acceptance criterion in seven months.** The pressure worked; it just came from an agent feeling awkward rather than from the schema. **Fix shapes:** a `status: inconclusive`; or a `criterion` field required alongside `failing`; or the same fact as a relation, matching how the family recorded the other three axes rather than adding a property. **Do not let this be answered by prose in a description** — that is where this instance already went to die. Size **M**  **SHAPE DECIDED 2026-08-01, on Anthony's call from three options** (`dec:a-failing-check-says-against-what`, accepted). **`Verification` gains an OPTIONAL `criterion` — what the check was measured against.** A `failing` carrying one is reported as a MEASUREMENT; a `failing` without one is reported as a bar never stated, and is never presented as though a threshold existed. **Requiring it was REJECTED** on two counts: it breaks every existing failing check until someone supplies a bar retroactively, and it blocks the honest case where something is plainly broken and no threshold was ever written down — but mainly because it is OUT OF STEP WITH THE FAMILY. [BL-106], [BL-126] and [BL-136] all REPORT the gap rather than refusing the work: an unscoped check is counted unscoped, an undated one unknown, a consumed one consumed, and none is forbidden. This is the fourth axis and it behaves like the other three. **A new `inconclusive` status was also REJECTED** — cheapest, and it gives *it runs but correctness is unknown* a home, but someone can still write `failing` as a pure opinion and nothing notices, which is the exact defect. Not foreclosed; it simply answers a different question. **THE FAMILY IS NOW FOUR FACES OF ONE HOLE:** stale in TIME, narrow in INPUT SPACE, not INDEPENDENT of what it validates, and — this one — no CRITERION, so there is nothing for the evidence to be evidence OF. The first three ask how good the evidence is; this asks what it was evidence FOR. **NOT YET BUILT: the shape is settled, the build is this row.** | M |
| **BL-144** | **`export_graph` will not create its own parent directory, and the path it rejects is the one AGENTS.md tells you to use** | *Found 2026-08-01 by an /adopt pass over `music_graph`, a ~11k-LOC Python research repo taken from an empty graph to 134 nodes / 379 edges. Reported in that project's own `reflow2_feedback.txt`.* AGENTS.md documents restoring with `reflow2-mcp --import .reflow2/backups/design-<utc>.json`, but `.reflow2/backups/` **does not exist in a fresh design**, and exporting to that documented path fails with a bare `No such file or directory (os error 2)`. One `mkdir` fixes it; the error does not say so. **A papercut, and it is on the documented recovery path** — the one somebody reads when something has already gone wrong. Adjacent to [BL-132] (error-message quality) and an instance of [BL-135] (state the failure mode): the message names the symptom and neither the path it tried nor the remedy. **Fix shapes:** create the parent directory, or name the missing directory and the `mkdir` in the error. Creating it is defensible here because the caller has already named the path explicitly — this is not a directory reflow2 invented (`dec:where-content-lives`'s concern), it is one the caller asked for. Size **S** | S |
| **BL-145** | **One detector produced 27 of 49 gaps saying the same thing, and buried every distinct finding below the fold** | *Found 2026-08-01 by an /adopt pass over `music_graph`, a ~11k-LOC Python research repo taken from an empty graph to 134 nodes / 379 edges. Reported in that project's own `reflow2_feedback.txt`.* Of 49 initial gaps, **27 were `unverified_capability` — one per capability, every one meaning "this repo has no test suite"**. They outnumbered every other finding combined. Compounded on the read side: after the owner answered, `graph_report` reported `gaps_truncated` at **32 of 37**, so the rollup showed five items while hiding most of the list. **The first adopt report is the one that decides whether somebody trusts the tool**, and this is the shape [BL-23] and [BL-42] keep naming — a detector that is right every time and therefore worth skimming. **reflow2 already knows how to do this:** `component_granularity_verification` rolls exactly this class up, and `dec:aggregate-gap-keyed-on-rule` ([BL-*], 2026-07-30) established that an aggregate gap keys on its RULE rather than its affected set, which is the machinery a rollup needs to stay acknowledgeable. **Fix shape:** when a project has zero Verifications at all, raise ONE aggregate gap about that rather than one per capability — the same call `is_aggregate()` already makes for `unvalidated_capability`. **The counterweight:** once *some* checks exist, per-capability is the right granularity again, so this must key on the zero case rather than on a count threshold. Size **S** | S |
| **BL-146** | **`coverage_report` marks a location `unobserved` when a parent directory also claims it, so the field that answers "did I forget to sweep this?" cannot be trusted** | *Found 2026-08-01 by an /adopt pass over `music_graph`, a ~11k-LOC Python research repo taken from an empty graph to 134 nodes / 379 edges. Reported in that project's own `reflow2_feedback.txt`.* `archive/reco.py` and `archive/reco_clean.py` were passed **explicitly** in the sweep AND are covered by an artifact claiming `archive/` as a whole. Both came back in `unobserved_locations` **despite appearing in `observed`**. Harmless in this pass, and it is the one field whose entire job is to say *you forgot to sweep this* — a false entry there makes the honest completeness signal untrustworthy. **Worth noting what is NOT broken:** the opaque-parent case itself works, and is deliberately pinned by `ver:coverage` (*"ONE OPAQUE ARTIFACT LEGITIMATELY CLAIMS THE MASS BENEATH IT"* — 900 vendored files under one registered directory report zero unclaimed). So the claiming logic is right and the `unobserved` bookkeeping disagrees with it. **Fix shape:** a location observed under a parent's claim is observed; reconcile the two paths so `observed` and `unobserved_locations` cannot both contain the same entry. **A test that they are disjoint is owed either way** — that invariant is checkable and was not checked. Size **S** | S |
| **BL-147** | **The shared server exited during continuous use, and the restart failed naming the symptom rather than the cause** | *Found 2026-08-01 by an /adopt pass over `music_graph`, a ~11k-LOC Python research repo taken from an empty graph to 134 nodes / 379 edges. Reported in that project's own `reflow2_feedback.txt`.* After about two hours of **active** use the shared server exited on its idle timer, and the next tool call returned `could not spawn the shared reflow2 server: No such file or directory (os error 2)`. **Two distinct problems, and the reporter separated them correctly.** (1) **The message's reassurance is well judged and should be kept** — *"The design is not lost — this session cannot currently talk to it"* was true, and having just exported 184 nodes meant they could say so with certainty. That framing is right. (2) **`No such file or directory` names the symptom, not the cause.** The cause is that `reflow2-mcp` is not on `PATH`: that install runs a dev build at `target/debug/reflow2-mcp` via a wrapper at `~/.local/bin/reflow2`, and the restart path looks the binary up by name. **The error should name the path it tried.** Diagnosing it took reading the server log, the wrapper script, and `pgrep`. Straight [BL-135]. **AND THE SEPARATE, POSSIBLY WORSE BUG:** the log says *"idle for 120 minutes"* while the session was continuously active for that whole window, **doing graph writes throughout**. Either the idle timer is not counting tool calls as activity, or it reports wall-clock since start rather than since last use. **A server that exits under active use is a different bug from one that exits when genuinely idle**, and only the second is the intended behaviour. Check that first. Size **S** | S |
| **BL-148** | **A session's tool surface changed under it mid-conversation and nothing said so — the BL-32 hazard running in reverse** | *Found 2026-08-01 by an /adopt pass over `music_graph`, a ~11k-LOC Python research repo taken from an empty graph to 134 nodes / 379 edges. Reported in that project's own `reflow2_feedback.txt`.* When the shared server restarted it came back on a **newer binary**: two tools that had not existed appeared (`calibrated_against`, `set_evidence_scope` — the [BL-106]/[BL-126]/[BL-136] family, merged that morning) and `served_by.binary_mtime_unix` jumped from 1785556181 to 1785561997. **This is the [BL-32] hazard inverted** — not a stale server serving an old surface, but a live session whose surface changed underneath it. **Nothing broke, and the version string stayed `0.20.0`, so version alone would not have revealed it.** `served_by.binary_mtime_unix` is what made it visible, which is an argument for keeping that field. **Fix shape, and the reporter's own:** a one-line note in the tool result when the surface changes mid-session — *"server restarted on a newer build; N tools added"* — turning something noticed by accident into something a session is told. **Related to [BL-113] and the launch-wrapper trap from the other direction:** that one is a session talking to the wrong design for hours, this is a session talking to a different *surface* than the one it started with. Both are silent, and both are only findable by inspecting the process rather than the protocol. Size **S** | S |
...
| **BL-111** | **`loop_nudge` promises "this nudge fires once" and nothing records that it fired — DONE 2026-08-01** | *Found by me, 2026-07-29, only because the nudge fired twice in one session and the second firing contradicted its own text.* The message ends *"(This nudge fires once; stopping again proceeds.)"* — and the guarantee rests entirely on `event.get("stop_hook_active")` at `loop_nudge.py:207`, a flag the **harness** sets when it re-invokes within a single stop cycle. It is never persisted. The session state file carries `{"writes": 0, "edits": 4, "touched": false}` and **has no `nudged` key at all**, so once the cycle ends the hook has no memory of having spoken. The rule it actually implements is *fires once per stop cycle*; the rule it advertises is *fires once per session*. Observed directly: it blocked at `edits: 3`, the session continued and did more work, and a fresh Stop arrived with `stop_hook_active` false again and blocked a second time at `edits: 4`. **The case where this bites hardest is the one where the nudge cannot be satisfied.** `state["touched"]` only ever clears on a successful `mcp__reflow2__*` call, so a session whose server is unreachable — bound to the wrong cwd, or serving the latent surface only — gets nudged at *every* stop for the rest of its life with no action available that would stop it. That is precisely the session in which the user is least able to comply and most likely to disable the hook. **Fix shape:** set `state["nudged"] = True` beside the `print` and test it alongside `stop_hook_active`; both blocking branches need it, since the graph-writes branch at `:214` makes the same promise on the same footing. This does not change the design intent — the once-only rule is deliberate noise-bounding from BL-90 — it makes the stated intent true, which is the same class of fix as [BL-107]: a designed guarantee with no computation behind it. Size **XS** — **BUILT 2026-08-01 exactly as the row specifies, and the row's fix shape was right down to "both blocking branches need it".** `claim_nudge()` is an atomic test-and-set: the first caller gets True and prints, everyone after gets False and stays silent. **IT HAD TO BE A CLAIM RATHER THAN A FLAG SET AFTER PRINTING, and that is [BL-161]'s lock earning its keep**: the hook can legitimately be registered MORE THAN ONCE — reflow2 installs machine-wide and a project can carry its own registration, and the two command spellings do not dedupe — so two processes run the Stop hook concurrently and a plain read-check-write would let both read `nudged: false` and both print. That doubled message is what this row was filed from and what the user saw three times in one session. **THE COUNTERWEIGHT: the claim is spent by NUDGING, not by STOPPING.** A session with nothing owed stops silently and keeps its one nudge, or stopping early would become a way to burn the trigger; mutating that fails eleven tests. **MUTATION-CHECKED BOTH WAYS:** make the claim never persist — the exact original defect — and precisely the three once-only cases fail; claim on every stop earned or not, and the counterweight plus ten others fail. 4 new cases. **The CONFIG half is deliberately NOT fixed here and is not the repo's to fix:** the duplicate registration lives in `.claude/settings.local.json` and `~/.claude/settings.json`, both personal config, and after this fix a duplicate registration is HARMLESS rather than doubled — which is the better outcome, since installing machine-wide *and* per-project is a supported combination rather than a mistake. | XS |
...
**BUILT 2026-08-02 — BL-68 IS DONE, and with it the board's most ambitious item.** The eleventh
schema domain, `readiness`, carries `ReadinessAssessment` (28 → 29 node types) plus `GATED_ON` and
`HAS_READINESS` (58 → 60) — **the first change since v0.4.0 to move both halves of the stamp**, so
v0.22.0 owes an upgrade doc and it is written. `TemporalFact` gains `basis` and `confidence`,
additively: `basis` defaults to `measured`, which **states what every pre-existing fact already
meant** rather than inventing a claim about it (`req:defaults-do-not-assert`, the `DesignEpoch.status`
argument), and the foundation applies defaults on create rather than retroactively, so there is no
migration step.

**TWO CALLS THE DECISIONS LEFT OPEN, PUT TO ANTHONY AND ANSWERED 2026-08-02.** *Where the threshold
lives:* on a **`GATED_ON` edge**, over a property on either endpoint or a `ReadinessGate` node.
Decisive argument: one increment must be able to demand TRL 7 of one technology and TRL 4 of
another, and the *same* technology is legitimately demanded at different levels by a demonstrator
and a fielded increment — the row's own worked example — which no single endpoint property can say.
The edge is also what lets the answer NAME what decided the date. *Who produces forecast
confidence:* **the author, per fact.** reflow2 deriving a decay curve from horizon would assert a
risk model nobody chose — the threshold refusal one layer along.

**THE REFUSALS ARE THE FEATURE, and both are pinned by tests that fail under mutation.** An
increment with no stated threshold reports `ungated`, **never ready** — mutating that verdict to
`achievable_now` fails exactly one test, and it is the one the whole item rests on. A gate whose
technology has no level and no clearing forecast makes the whole answer `indeterminate`; skipping
it instead — computing a date from the gates that happen to have evidence — fails **five**. That
second one is the subtler half: an optimistic date built by ignoring the inconvenient record is
exactly the undefendable slide-roadmap BL-68 exists to replace.

**`GATED_ON` WENT INTO THE STRUCTURAL TABLE BEFORE A DETECTOR COMPLAINED, which is the second time
of four.** `INCLUDES` and `SCHEDULED_FOR` both reached `structural_rule` only after
`disconnected_community` fired on an island they had failed to join, and the table's own comment
says nothing checks the question is ever asked. Here the failure would have been worse than an
island: a technology could slip TRL 7 → 3 and `propagate_change` would report nothing downstream,
leaving the roadmap silently stale. `HAS_READINESS` is deliberately **out**, on the same argument
`HAS_OBSERVATION` already carries — an assessment is a reading *about* a node, not a coupling
between two design nodes.

**THE MUTATION CHECK EARNED ITS KEEP BY FINDING A HOLE IN MY OWN SUITE.** Dropping the ladder
filter from the FORECAST path passed every test: `trl_and_mrl_are_not_interchangeable` only ever
exercised the MEASURED path, so an **MRL projection silently clearing a TRL gate** would have
shipped a date built on the wrong evidence entirely. Fixed with the sibling case the suite was
missing; nine mutations now each fail exactly the tests that name them.

**AND THE SURFACE INCONSISTENCY ONLY THE REAL BINARY SHOWED.** The Rust tests compare typed enums
and were green, while the served JSON carried `Ungated` and `{"GatedUntil": {...}}` — Rust's
default tagging, in an otherwise snake_case surface. Caught by driving seven checks over real
stdio in `smoke_mcp.py`, which is AGENTS.md's *"every other layer is a client we wrote"* landing
on new code the same day it was written.

15 cases in `crates/reflow2-core/tests/readiness.rs`, mutation-checked nine ways; 4 MCP tools
(toolsnaps 138 → 142); workspace green, clippy 0 both crates, smoke OK, gate OK. **What remains of
the wider thread is [BL-150]'s `dec:idea-build-order` — BL-68 asks whether a *thing* is ready, that
asks whether its *dependencies* are.**
