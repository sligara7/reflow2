...
| ID | Item | Why | Size |
|---|---|---|---|
| **BL-183** | **`add_decision` on an existing id silently DOWNGRADES an accepted Decision to `proposed` — BL-166's shape at a third call site** | *Walked into 2026-08-02 while correcting `dec:bl83a-functional-decomposition`'s stale enumeration. Calling `add_decision` with an existing id to revise its prose — which is exactly what the **revise-design** skill prescribes ("call the node's `create_node` with the SAME `id` and only the properties you are changing… an existing id **merges**") — silently reset `status: accepted` to `status: proposed`.* **THE RECORD LOOKED FINE:** the write returned success, the decision text was correct, and nothing reported a downgrade. Only reading the result back caught it — [BL-166]'s discipline paying off two rows after [BL-166] itself. **THIS IS THE THIRD KNOWN INSTANCE OF ONE DEFECT** ([BL-46] first, `link_artifact` second in [BL-166], `add_decision` now): a create-or-REPLACE helper that names a subset of a node type's properties, so every property it does not name is silently reset to its schema default on re-write. `upsert_node` was written for exactly this and is still not used at every call site. **THE COLLATERAL DAMAGE IS WORSE THAN THE FIELD:** a `proposed` Decision with ≥2 registered alternatives is an open decision point that `detect_gaps` reports as `undecided_decision_point`, so a silent downgrade can *reopen a settled fork* and put a question back to the user that they already answered — the exact failure `dec:reopen-supersedes` exists to prevent, arriving through a code path rather than a choice. **AND THE SKILL IS COMPLICIT:** revise-design tells you to re-call the constructor, which is the operation that does this. **Fix shape:** audit every `add_*` helper for subset-naming and route them all through `upsert_node`; the audit is the row, not the single fix. **Also worth pairing with [BL-178]**, which recorded that the brainstorm skill's claim about `add_decision`'s default status was already wrong — the same constructor, misdescribed twice. Size **M** | M |
