...
| ID | Item | Why | Size |
|---|---|---|---|
| **BL-184** | **`DimensionAssessment.score` is only ever ASSERTED, while reflow2 computes a pile of evidence bearing on the same axes and connects none of it** | *Anthony, 2026-08-03, correcting my claim that the maturity bands were "judgement rather than computation": **"I'm not sure a blanket yes or no can be applied here. I feel like there are some areas where the graph may inform or give a computed signal that indicates one of the 'ilities' are not sufficiently being met by the design."** He is right and the code is blunter than the argument.* **MEASURED:** `Verification`-style computation over the `dimension` enum does not exist. The ONLY thing reading a dimension is `dimension_drifts`, which fits a least-squares slope over **asserted** scores — so **reflow2 computes the trend of an opinion, never the ility.** Meanwhile it already computes Leiden **modularity**, articulation points, circular dependencies, god-components, coupling weights, surprising connections, granularity mismatch, verification coverage and (as of [BL-179]) the seams band — **none of it connected to the axis it informs.** That is arguably `dec:edge-orthogonality` already violated inside the schema: nine distinctions only an LLM writes into. **THE ANSWER IS NOT BLANKET IN EITHER DIRECTION, which is his actual point.** `coupling` and `maturity` are strongly informed by the graph; `maintainability` and `testability` partially; `reliability` weakly (SPOFs, but real reliability needs runtime data); and **`performance`, `security`, `scalability` and `observability` not at all** — nothing structural measures latency, threat, load or instrumentation, and computing them would be fiction. **SHAPE CHOSEN BY ANTHONY, 2026-08-03, of three offered: a SEPARATE reading that names the computed evidence per axis and, where an asserted `DimensionAssessment` exists, says whether the two AGREE.** It never writes a score. **The precedent that forbids a derived number is his own:** `dec:readiness-is-an-observation-the-threshold-is-the-judgement` refused to put TRL into `DimensionAssessment.score` because *"TRL 7 as 0.78 asserts a precision the ladder does not have"* — collapsing three cycles and a god-component into `maintainability: 0.62` is the identical lossy move. **The valuable output is the disagreement**: a component scored 0.9 for maintainability while holding three dependency cycles is the same record-vs-record shape as `compare_designs` and [BL-182]. **Rejected:** writing derived `DimensionObservation` rows (would invent the score and put reflow2's opinion in the human's field), and routing it through `detect_gaps` only (silent wherever nobody has asserted anything, which is most of the graph). Size **M** | M |
