...
| `quality_target_unstated` ✅ | the design has **never said what it is FOR** on the quality axis — no ACCEPTED Decision carries a `quality_target`. It matters because the attribute decides *which grouping is right* and the four disagree: performance wants least chatter across boundaries; reliability wants no articulation point and may deliberately DUPLICATE a function across parts; maintainability wants what-changes-together-lives-together (a co-change graph, a different edge set); security wants boundaries following trust and data classification, which is constrained partitioning rather than clustering. **Allocating without the answer silently picks performance** (`dec:idea-the-ility-chooses-the-allocation-graph`, whose co-change experiment measured 71% of reflow2's own strongest maintainability signal crossing its functional boundaries). ⭐ **THREE STATES, NOT TWO, and the middle one is the point.** `Decision.quality_target` has **no default**, so absence means *nobody was asked*; an ACCEPTED target settles it and this goes quiet; a PROPOSED target is somebody LEANING — weighed, not committed — and the finding then reads *"still being weighed"* at severity 0.40 instead of *"unstated"* at 0.55. Collapsing a deferral into silence is the failure `Decision.no_relation_note` exists to prevent on this same node type: prose cannot be told apart from silence. 🛑 **A deferral must never be `acknowledge_gap`-ed** — the finding is aggregate, so one acceptance silences it permanently and for every capability added afterwards, which is the opposite of deferring (`dec:idea-an-aggregate-acknowledgement-never-expires`). Asked at GENESIS rather than at allocation: asking is cheap and shapes everything downstream, while allocating still waits for the last responsible moment — different questions. Silent on a design that has not yet said what it DOES (0 capabilities and 0 requirements), which is `design_without_intent`'s ground |
| `artifact_standard_undeclared` ✅ | a Project has **never said what standard its design artifacts must be in** — no `EnvironmentRule` of `rule_type: standard` that the Project `COMPLIES_WITH`. Asked FROM THE VERY BEGINNING: a Project with nothing else in it is asked, because the answer decides the pipeline before anyone draws (`req:a-project-names-its-domains-artifact-standard-from-the-beginning-and-its-absence-is-reported`). The measurement is bhome, 2026-08-31: an agent drew a house plan sheet in HTML because nothing in the design asked what a plan sheet must be, and the person who would have caught it was the city planner at $250/hour. ⭐ **"NONE KNOWN" IS A REAL ANSWER**, recorded by acknowledging the finding with that reason — and the finding is keyed on the project AND on whether any `drawing`/`model` artifact exists, so a software project that said "the code is the artifact" stays quiet until the day a drawing appears, and is then asked ONCE MORE (a new gap id) rather than never. Severity 0.35 with no drawing or model yet, 0.65 once some exist unasked. Silent on a mirrored Project (somebody else's design as received). reflow2 opens no file: the standard's `checker` is named on the rule and run by the agent or CI, and its output is the evidence on the compliance edge |
| `artifact_not_under_declared_standard` ✅ | the Project declared a standard and **a `drawing` or `model` artifact does not say whether it is in it** — no `COMPLIES_WITH` and no `VIOLATES_RULE` from the artifact to any declared standard rule. One rollup per project keyed on the SET of artifacts it names, so a new drawing re-asks and an acknowledged set stays acknowledged. Only drawings and models are asked: a parametric script that GENERATES the standard artifact is `code` and is exactly the reviewable source the who-authors reframing says the agent may write. Silent when no standard is declared — that is the row above's ground. Answered with `complies_with` (`verified: true` with the checker's output only when it actually ran) or `violates_rule` naming the format it is in instead |
| `unit_system_undeclared` ✅ | the design carries unit-bearing quantities — a Constraint with a `limit`, a CONSTRAINS edge with a `contribution`, an Interface with `units` — and **no live DesignRule of `category: unit_system` declares what units it is done in**. The Mars Climate Orbiter board found the SE function "not robust enough" to catch a unit mismatch; this is that review made mechanical (`req:a-project-declares-its-unit-system-as-a-governed-rule-and-undeclared-or-off-system-units-are-reported`, Anthony 2026-09-13). One aggregate finding, severity 0.45. **Silent when the design carries no quantities at all** — there is nothing to check and the finding says nothing rather than reading clean; a design with no numbers is unchecked, not compliant |
| `quantity_without_unit` ✅ | a Constraint carries a `limit` and no `unit`, or a CONSTRAINS edge carries a `contribution` and no `unit`. One rollup keyed on the set of offenders, so a new one re-asks and an acknowledged set stays acknowledged. Severity 0.5. The unit is asked for explicitly rather than read off a `quantity` suffix like `mass_kg`, because a suffix convention is exactly the thing nobody compares |
| `unit_outside_declared_system` ✅ | a stated unit — on a Constraint, a CONSTRAINS edge or an Interface's `units` — is **not among the units the `unit_system` rule declares**. Keyed on the set of offenders. Severity 0.6, and 0.8 when the rule is `enforced`. Compared by exact spelling on purpose: `N·s` and `N s` are two answers until somebody says they are one. Silent when no unit system is declared — that is the row above's ground |
| `quantity_without_source` ✅ | a Constraint carries a `limit` and no `limit_source`, or a CONSTRAINS edge carries a `contribution` and no `source` — **a number nobody owns**. The founding fear made mechanical (`req:a-quantity-carries-how-it-was-obtained-and-an-unsourced-one-is-reported`, Anthony 2026-09-16: "something that rules out that the AI agent just didn't make up a measurement"): reflow2 cannot rule out a made-up number; it can make one with no source LOOK DIFFERENT from one with a source, and count them. A source that is the agent (`who:claude-code`) is allowed and visible — a proposal reads as a proposal. Keyed on the set of offenders, severity 0.5. Silent when the design carries no numeric limit |
| `quantity_check_without_executable_form` ✅ | a Verification VERIFIES a Constraint that carries a `limit` and has no `IMPLEMENTS` from any Artifact — a check on a number that nothing can re-run, which is a claim wearing a status. Keyed on the set of checks, severity 0.45. The remedy is the file that IS the check (a test, a script, the measuring tool's call), registered with link_artifact and joined by IMPLEMENTS |
| `closure_criterion_undeclared` ✅ | the design carries live requirements and **no Project has declared what closure means** — `closure_legs` (which of traceability, budgets, seams, decisions, provenance count) and `closure_threshold` (the share of each that must close). `closure_report` reads "no closure criterion stated" until the owner says (`req:a-design-closes-against-a-declared-threshold-and-the-report-names-the-first-hole`, Anthony 2026-09-16: "need to ensure that designs close and are verified"). One aggregate finding, severity 0.4, silent on a design with nothing to close. Never defaulted: 100% is a declaration and so is "traceability and budgets only"; closure is a report, never a gate |
| `encoding_undecided` ✅ | several live nodes of one type are **encoded differently** — some carry the type's discriminator (Component.kind, Interface.medium, Requirement.kind, Verification.method), some carry nothing — and the ones carrying nothing are governed by no accepted Decision. The fifth service that looks like the first four and was recorded differently (`req:a-taxonomy-is-decided-once-before-bulk-capture-and-every-instance-cites-it`, Alex 2026-09-17: "the taxonomy lives as Decisions… nothing helps you write them"). One finding per type and discriminator, keyed on the pair; silent under three instances; a materialised `unspecified` reads as nothing said; a type where nobody has said fires only at five. Artifact.artifact_type and Flow.flow_type are not swept — they default to a real value and would read as stated on every node. Asks whether an encoding decision is owed (the establish-taxonomy skill); never picks one |
| `convention_delivered_nowhere` ✅ | a live DesignRule of `category: convention` names no `steps`. get_skill and the tool list carry a rule only where its steps name the served skill or tool it bears on, so an undelivered convention is findable and reaches nobody at the moment it matters — the per-repo know-how Alex measured living in thin project adapters (`req:a-repos-procedural-know-how-is-delivered-at-the-step-it-bears-on-and-the-design-knows-the-adapter-exists`, 2026-09-17). Keyed on the set, severity 0.35; silent with no conventions. Measured on reflow2's own design the day it shipped: 8 of 9 |
| `agent_instructions_unregistered` ✅ | the design holds five or more artifacts and no Artifact DOCUMENTS a Project with `doc_kind: agent_instructions` — the file every agent reads before it knows reflow2 exists (AGENTS.md, CLAUDE.md, a project-local skill) is invisible to the design and never checked for drift. One per design, severity 0.3, silent under five artifacts (a design with nothing registered has no adapter to miss). Measured on reflow2's own design the day it shipped: its root AGENTS.md was not registered |
...
| `fix_without_recorded_cause` ✅ | a `ChangeEvent` of `change_type` `defect_fix` or `test_failure_fix` is joined to NO cause: nothing `CAUSES` it, it `CAUSES` nothing, and it `INVALIDATES` no finding. **Since 2026-09-14 a checksum ACCEPTANCE wearing a fix label (its `CHANGED` edge carries `accepted_baseline`) is told apart from a repair** — counted and named separately as a label to correct, never a cause to invent — after a field project found twenty of its twenty-one "fixes" were reconcile-pass acceptances wearing the tool's old default; and the gap's text states the discipline with a triage recipe rather than offering to decline it. **The third leg of the root-cause vocabulary** (`dec:idea-how-does-root-cause-become-automatic-rule-hook-detector-or-composition`, ruled 2026-09-06): the skill says how to find a cause, the rules say to write it down, and nothing noticed when a fix landed with neither — so `req:a-fix-says-whether-it-corrected-the-cause` was accepted and unenforceable. Measured on reflow2's own design the day it shipped: 209 fixes recorded, the large majority joined to no cause. **A `CAUSES` edge on an artifact the fix touched is deliberately NOT counted**: it says something about that file's past, not about this repair, and one old cause on a hub file would launder every later fix of it. **AGGREGATE** (`N of M`), for the `change_axis_unstated` reason — fixes are among the fastest-growing records in an active design. **Forward-only by the graph's own word, not by a date in the detector**: a backlog that predates the rule is PARKED per fix (`GOVERNED_BY` `ruling=parks` under one accepted Decision) and COUNTED in the evidence, so the finding keeps moving as new fixes land; acknowledging the aggregate is the other act — a standing judgement that this design does not record causes — and silences it. Severity 0.35, just above the missing axis: a missing cause is the condition under which the same class comes back wearing different clothes. Reports absence of a record, never the quality of one |
| `defect_overtaken_by_change` ✅ | an OPEN `TemporalFact` of `fact_type: defect` (no `valid_to`, no incoming `INVALIDATES`) whose `subject_id` — or an `Artifact` that `REALIZES` it — was `CHANGED` by a REPAIR — a `ChangeEvent` of `change_type` `defect_fix` or `test_failure_fix` — whose `detected_at` is strictly after the fact's `valid_from`, and nothing says whether that repair was the fix. **Repairs only, measured rather than assumed**: over later changes of any kind reflow2's own design raised 65 findings and one hub component carried 39 members, a question nobody can answer; over later repairs, 45 with a median of 3 members. A feature or refactor that touched the subject is counted in the evidence and never offered as the fix. **The mirror of the row above, written from `fact:a-defect-fixed-but-never-closed-on-the-record-was-re-fixed-wrongly-a-day-later`**: a fix drew no `INVALIDATES`, the fact went on reading as open, and the next session trusted it and re-fixed the symptom a different way, breaking two pins. A stale open defect is a live instruction to do the wrong thing. **Per fact, keyed on the fact AND the later repairs**, so a further repair on the same subject asks again — the world moved. **Ordering needs two dates and the detector says so**: a change with no `detected_at` is counted in the evidence and never treated as later; a fact with no `valid_from` is skipped. **What it cannot see, stated**: only the subject and its direct realizers are watched, so a defect recorded against the document that *reported* it rather than the part that was wrong is out of reach — that was the shape of the motivating case, whose fixer was also undated, so as specified this detector would NOT have caught it (recorded on the fact). Severity 0.5: above the absence gaps, below a failing check and unresolved drift |
| `decision_overtaken_by_promotion` ✅ | a `Decision` still at `status: proposed` with an outgoing `EVOLVES_INTO` edge to a live node the design has ALREADY TAKEN UP — a Requirement `accepted`/`met`, a Capability `realized`/`verified`, a Decision `accepted`, or a `ChangeEvent` or `DesignRule`, which carry no status and are counted on their existence (a change happened; a rule is in force). **The same shape as the row above, one type over, and simpler: one edge and one status, with no dates to order and no artifact walk.** Written from the 2026-09-14 sweep of reflow2's own design, where seven ideas whose answers had already shipped were still being offered as open work. **`EVOLVES_INTO` is the whole record that an idea became something, and until this nothing anywhere read it** — the project's own three-leg rule (typed tool + instruction + detector that notices absence) missing its third leg, which predicts exactly what was measured: the instruction was followed 5 times of 7 and silently skipped twice, and nothing ever noticed either way. **Deliberate exclusions, each pinned by a test**: a `dropped`/`deferred`/`rejected`/`superseded` target does NOT settle the question — what the idea became was itself put down, which reopens it; a target still `proposed`/`planned` is intent, not a settlement; and a `TemporalFact` does NOT count though it too is status-free, because a measurement about an idea is evidence toward an answer rather than the answer (which is why the status-free rule is an enumeration and not "anything without a status"). **What it cannot see, stated in its own evidence line**: only promotions that DREW the edge — an idea answered by work nobody linked back is unreachable by any query, and that was 2 of the 7. The only fix for that half is the instruction, now repeated in **capture-intent**, which is the skill that actually runs at the moment of promotion (brainstorm step 5 names the edge and then hands straight over). **Per decision, keyed on the decision AND what it became**, so a second promotion asks again. Parked decisions skipped. Severity 0.4: below a stale open defect (0.5), which is a live instruction to do the wrong work, and above `unreviewed_ideas` (0.3), where nothing is actually wrong — here the design says two contradictory things at once. Measured on reflow2's own export the day it shipped: **11 of 277** proposed Decisions |
| `prohibition_in_prose` ✅ | a live Requirement, Capability, Component, Interface or ACCEPTED Decision whose name, statement, description or decision text carries a prohibition phrase — `must never`, `must not ever`, `is/are not allowed to`, `never allowed`, `is forbidden` — and which no Constraint `CONSTRAINS`. **The routing table's own row, enforced**: capture-intent sends "a prohibition with no number in it" to a Constraint, *not* a Requirement, because a Constraint can `constrains` what it binds and be read by the checks, while a sentence inside prose can be read by nobody. Measured on reflow2's own design 2026-09-14 (`fact:capture-is-blind-to-three-families-…`): ≈61 prohibitions in prose, 9 of 10 sampled genuine, against 8 Constraint nodes — and the row had been added precisely because eleven prohibitions were left as Requirements. Instruction present, detector absent: the three-leg rule missing the leg that notices. **ONE aggregate finding, severity 0.35** — beside `unreviewed_ideas`, because nothing is wrong: the intent is recorded and findable, just in a home no computation reads. Per node it would be the BL-73 wallpaper. **Deliberate exclusions, each pinned by a test**: `proposed` Decisions (brainstormed ideas — the brainstorm skill's own rule is that detectors do not run over them); DesignRules (a "we never…" there is home, by the table's other row); Constraints (the home); ChangeEvents, TemporalFacts and Verifications (records and checks, not intent — "the one direction this must never fail in" on a test was in the sample); the `rationale` field (the why, not the what); parked nodes. **`shall not` is deliberately NOT matched**: it is the standard negative form of an obligation in requirement writing, and an SE reader files it as a Requirement — a false neighbour is worse than a missing one. The closed-set row ("may only be A, B or C") is not matched either: "exactly one of" is ordinary API prose here and would flood. Silent on a design with nothing of the scanned types. Reports, never judges: filing the sentence as a Constraint, or acknowledging the phrase as incidental, is the reader's call |
