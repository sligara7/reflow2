...
| **Surprising connections** — an edge bridging two otherwise-distant communities | ✅ **built** | `surprising_connections()` (`surprises` module): a coupling edge whose endpoints sit in different `leiden` communities, ranked by surprise (rarity = sole bridge; peripheral→hub) with explained `reasons`. Reported as a **signal** by `graph_report` (its own section) and the `surprising_connections` tool — deliberately *not* a gap, since it fires on correct architecture: an Interface bridges two clusters by construction, so every properly modelled contract read as surprising (BL-6b). Still feeds HEAL's creative-bridge · `tests/surprises.rs`, `tests/report.rs` |
