...
| GS-6 | Traceability gaps | 🟡 | unsatisfied_requirement, unallocated_capability, unrealized_capability, unverified_capability ✅ (capabilities only; the artifact rule was retired in BL-23 after the self-host probe made it 22 of 25 gaps — per-file coverage is now a `graph_report` statistic. The capability key is frozen because gap ids hash it and acknowledgements hang off the id); **interfaceless_dependency deferred** |
