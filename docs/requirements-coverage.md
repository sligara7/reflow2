...
| HEAL-3 | Defect catalog (13 categories) | 🟡 | orphan_node, contradiction, duplicate, unresolved_setup, unthreaded_cluster, single_point_of_failure, dead_end, circular_dependency ✅ (8); **unreachable, weak_connection, missing_link, missing_entity, missing_embedding deferred**. `circular_dependency` walks a *directed* dependency view (DEPENDS_ON + contracts collapsed through their Interface), deliberately excluding the other traceability edges so the golden thread closing on itself isn't reported as a loop · `tests/cycles.rs`. **`unthreaded_cluster` SAYS WHAT IT WALKED, since 2026-08-17.** It read "disconnected from the rest of the design" while computing components over `design_network()` — which drops nine node types and every review record, and does not count `CONTAINS` — so the nodes it named were genuinely reachable by an undirected walk of the graph and the sentence claimed otherwise (measured: the walk covers **1133 of 2413 nodes**). The message now names what it computed, how much of the graph it held, the exclusion that most often explains a false island, and the singleton count it does not report. **AND THE KEY WAS RENAMED TOO, hours later on the same day.** The first fix left the identifier saying the wrong thing, on the ground that consumers match on it and it is an ility source; `rule:fix-it-properly-while-it-is-still-cheap` (Anthony) reversed that — before a stability commitment, a break is cheapest now and dearer with every user. `unthreaded` names what is actually missing, a traceability edge, and cannot be misread as "unreachable in the graph". The rename cost no data migration: the key was never persisted in any graph, only in code and prose · `the_finding_says_what_it_swept_and_stops_claiming_unreachability`, `the_swept_count_tracks_the_graph_rather_than_being_boilerplate` |
| HEAL-3b | A sweep says what it examined | ✅ | Unscoped `detect_defects` returns `{swept, defects}` rather than a bare list (2026-08-17): `swept.nodes` examined, `swept.rules` naming the checks that ran, `swept.design_network_nodes` reporting the narrower topology walk instead of hiding it, and `swept.note` present ONLY when the sweep could not have found anything. So an empty result says which empty it is — the distinction a fleet needed before running `apply_heal`, which deletes nodes. The scoped sibling had answered this way since 2026-08-09, so one tool was answering one question two ways and the honest half was the half fewer people call · `an_empty_graph_says_its_zero_is_vacuous_and_a_real_one_does_not`, `the_sweep_reports_the_topology_walk_as_narrower_than_the_graph`, `the_sweep_names_the_rules_that_ran`, `every_category_is_listed_in_all` |
...
| HEAL-6b | HEAL does not double-report what DETECT asks | ✅ | `orphan_node` covers only the Artifact **attached to nothing** — every edge it carries, either direction, is bookkeeping (`INCLUDES`/`CHANGED`/`YIELDED`/`AT_EPOCH`). It counted outgoing `REALIZES` alone until BL-176, which made every correctly-filed `DOCUMENTS`/`SPECIFIES` document a defect (field measurement: 13→39 defects on 26 documents, 46%→82% false positives). The list is the EXCLUSIONS so a new design edge counts as attachment without an edit · `orphan_attachment.rs` (6 cases: 2 defect, 4 counterweight incl. `an_artifact_attached_by_nothing_is_still_an_orphan` and `a_release_include_does_not_silence_the_orphan_rule`) — DETECT owns the Requirement and Capability questions (`unsatisfied_requirement`, `unallocated_capability`), which were never repairable here anyway (a `generate_owner` stub `apply_heal` cannot apply). Four trials reported the double-count; storyflow made it 20 of 31 defects (BL-42) · `detect_finds_duplicate_and_orphans`, `a_dropped_requirement_is_ignored_by_detect_and_heal_alike`, `a_flow_member_has_a_home_and_a_loose_capability_is_asked_about_once`. **WIDENED 2026-08-16 and the division above is what bounds it:** the *degree-zero* rule beside the Artifact one ran on `Decision` alone, so `detect_defects` answered clean over a DesignEpoch with no edges at all — in two packages, through every health call of a session — and it now runs on every type nothing else asks about (7 → 21 zero-degree nodes reported on reflow2's own graph). Requirement/Capability/Interface stay excluded by name, so this row's claim still holds; Project and advisory DesignRule are excluded because unattached is their *resting* state (a lone Project is what genesis produces on day one); and a **pointer property counts as attachment** — `TemporalFact.subject_id`, `Snapshot.target_id`, `Question.gap_id` are required indexed properties, so only a pointer resolving to nothing is reported, which is what stopped 48 of reflow2's own 212 facts being filed as defects · `an_epoch_with_no_edges_is_not_clean`, `a_verification_that_verifies_nothing_is_reported`, `a_pointer_property_is_attachment_and_only_a_dangling_one_is_reported`, `detect_still_owns_the_types_it_asks_about_by_name`, `an_empty_design_and_a_process_rule_are_resting_states_not_defects` |
...
| 3AX-12 | **The QUALITY of evidence is a fact the graph holds, not just its existence** (BL-106/126/136) | ✅ | The three axes on which a passing check can fail to support its claim, delivered as one increment 2026-08-01. **TIME**: `confirmation_ledger` reports `last_verified_at` + `verification_freshness` (current/stale/**unknown**) · `an_accept_dated_after_the_check_reports_stale`, `a_check_re_run_after_the_accept_is_current`, `an_undated_check_reports_unknown_not_current`, `a_failing_check_does_not_refresh_the_claim`. **INPUT**: `set_evidence_scope` on the `VERIFIES` edge, reported by `evidence_report` · `a_parameter_every_check_pins_is_reported_as_narrow`, `one_check_that_sweeps_it_ends_the_narrowness`, `one_suite_can_be_broad_about_one_claim_and_narrow_about_another`, `a_check_that_states_no_scope_is_unscoped_not_broad`. **INDEPENDENCE**: `calibrated_against` + consumed-check exclusion · `a_check_the_target_was_calibrated_against_is_consumed`, `a_check_that_produced_the_calibration_anchor_is_consumed`, `an_independent_check_beside_the_fit_restores_the_claim`, `a_calibration_against_unrelated_evidence_is_not_circular`, `correcting_the_anchor_reaches_what_was_fitted_to_it`. **Mutation-checked on all three axes.** Reading this row against 3AX-3's lesson: the ✅ covers what the graph can RECORD and REPORT about evidence scope — it does not claim reflow2 can tell whether a stated scope is true, which is unknowable from the graph and belongs to BL-95 |
...
| SP-3 | Surface layer: MCP server / CLI exposing core ops as tools | ✅ | `reflow2-mcp` crate — rmcp 3.x stdio server (`ReflowService`) exposing ~78 fine-grained tools over a RocksDB graph (`--graph-path`): DETECT/analyze (detect_gaps, propagate_*, graph_report, heal detect/propose, allocate, hierarchy, surprises, dimensions), build (add_*/create_*/CRUD/apply_heal, incl. `add_interface`/`provides`/`consumes` so a contract's two sides are modelled from the surface, not only from `ingest`), CHANGE (epoch/change_event/record_change), and the `gap_to_prompt` collect-then-serve handshake (SP-2). No-envelope, no-silent-fallback results · `tests/tools.rs`. **`ingest` handshake deferred → SP-3b.** Since 2026-07-30 also `mint_seat` + the `claim_region` seat guard: identity is a handle the caller carries, and on the sessionless transport (MCP 2026-07-28+, where rmcp builds a handler per REQUEST) a claim with no `seat` is REFUSED rather than given one that would change per call — `req:seat-identity-survives-stateless-mcp`, `ver:stateless-seat`, `tools/stateless_seat_probe.py` (three transports, both halves, exits zero) |
...
| SP-6b | As-built drift detection / reconcile | 🟡 | `drift::DesignGraph::reconcile_artifacts` — compares caller-supplied observations against the `Artifact.checksum` baseline (new schema property, recorded by `link_artifact`), emitting `missing_artifact` / `checksum_change` / `undocumented_addition` findings, plus `no_baseline` when a comparison is impossible (surfaced, never silently passed). Optionally records `DriftEvent` nodes (deterministic ids, `DEPENDS_ON` their Artifact). `propagation_seeds` carry the realized design nodes, so drift walks **upstream** via `REALIZES` to the Capability/Requirement — the code→design feedback path. `reconcile_artifacts`/`set_artifact_checksum` MCP tools + `link-artifacts` skill · `tests/drift.rs` (12), `reflow2-mcp/tests/tools.rs::reconcile_surfaces_a_code_change_back_to_the_design`. **The core performs no I/O by design** — the caller (agent/CI/CLI) supplies observations, same seam pattern as `LlmBackend`. **Deferred: `spec_mismatch`/`status_mismatch` drift types (need content comparison, not just hashing), and discovery of unregistered files (the caller must name what it observed).** **2026-08-04 (BL-191): a content change on an Artifact declaring `volatility` `append_only`/`living` reports as the new `expected_change` kind and is NOT recorded — a log that grew is not a divergence, and five coordination buses produced five correct-but-meaningless `checksum_change` findings within minutes of being modelled per the adopt skill. Reported rather than suppressed, because silence would let a wholesale replacement pass unmentioned; ABSENCE still fires at full severity whatever the volatility. SHRINK detection deliberately excluded — a checksum cannot express it, it needs a size baseline nothing records, and deliberate compaction shrinks such files on purpose, so the rule needs the design's consent. `tests/artifact_intent.rs` (9, mutation-checked ×3).** *Corrected 2026-08-02 (BL-170): this used to read "reflow2 cannot enumerate a filesystem it never reads", which is over-stated and was being used as the argument for deferring the work — `identity.rs` and `provenance.rs` in the core read and write files, as do `nudge.rs`, `latent.rs` and `main.rs` in the surface. The honest and still-sound statement is narrower: **the analysis never reads the subject system**, because the caller is the one that knows what is relevant, holds the permissions, and can honour ignore rules.* |
...
| SP-6g | Analysis of alternatives + the decision point (BL-70 v1 + rung 2) | 🟡 | `alternatives::analyze_alternatives` (branch-by-file, `dec:parallel-alternatives`) loads N alternative exports into throwaway in-memory graphs and lays their measures side by side (graph_report snapshot: design nodes, open gaps, structural defects, allocation modularity, capabilities verified) plus each non-baseline branch's `compare_designs` divergence — comparable on measures, not advocacy. **Rung 2 (the decision point):** a *proposed* Decision holds forkable alternatives — `register_alternative` (Artifact pointer, `GOVERNED_BY` + `CONTRADICTS` siblings, refused unless proposed), `alternatives_for`, `set_decision_status`, and `collapse_decision` (winner → Decision accepted, losers `OBSOLETES`-retired on the record, outcome written into the ADR `alternatives` field). Alternatives are design *space* (CONTRADICTS), distinct from epochs (*time*); winner's content merges via `apply_merge`. **The teeth:** the `undecided_decision_point` DETECT gap surfaces a proposed Decision with ≥2 alternatives as an open fork, anchored so it clears on collapse. MCP tools + `tests/alternatives.rs` (9). **Deferred:** the cross-branch-absence gap ("satisfied in only one alternative"), per-branch readiness (BL-68), and world-scope. |
...
| SP-6h | Readiness gating + the derived roadmap (BL-68 part 1, the keystone item's last unbuilt part) | ✅ | `readiness::DesignGraph::readiness_report` derives an increment's delivery epoch FORWARD from the maturity of what it waits on — the earliest epoch by which every `GATED_ON` technology clears the level demanded of it — and NAMES the gate that decided it. Three write ops keep observation and judgement apart: `add_readiness` (a `ReadinessAssessment`, TRL/MRL 1–9, an observation), `gate_on` (the threshold, on the EDGE so one increment can demand TRL 7 of one technology and 4 of another, and a demonstrator and a fielded increment can differ on the same one), `forecast_readiness` (a `TemporalFact` with `basis: forecast`, because `observed_at` says *observed* and nobody observed anything in 2035). **The two refusals are the feature:** no stated threshold reports `ungated`, **never ready**; a gate with no level and no clearing forecast makes the whole answer `indeterminate` rather than a date computed from the evidence that happens to exist. Forecast confidence is author-stated, never derived from horizon. `GATED_ON` is a traceability edge, so slipping readiness reaches every increment gated on it. Schema domain 11 (`readiness.yaml`): 28→29 node types, 58→60 edge types — both halves of the stamp, so v0.22.0 owes an upgrade doc. 4 MCP tools · `tests/readiness.rs` (15), mutation-checked nine ways, `smoke_mcp.py` §7b2 (7 checks over real stdio). **Deferred:** the READY FRONTIER — whether an increment's *dependencies* are buildable, as against whether a *thing* is ready ([BL-150], `dec:idea-build-order`); and per-branch readiness in AoA (SP-6g). |
| SP-6f | Design-vs-design compare + three-way merge, propose & apply (BL-71 rung c, BL-80) | ✅ | `compare::compare_designs` reports directional divergence between two as-designed records (BL-71 rung c). `merge::merge_designs` (BL-80) is its write-side sibling: the three-way case table (`dec:merge-three-way`/`dec:merge-conflict-semantics`) over base/ours/theirs — one-sided taken, agreed taken, both-sides conflicted as deterministic-id Questions, delete/modify retained-and-asked, edges symmetric. **`resolve_merge`** materialises the merged design from the human's per-conflict decisions and **`apply_merge`** commits it into the live graph atomically — **refusing, writing nothing, until every conflict is decided** (`dec:merge-three-way`, no silent guess). `merge_designs` (read-only) + `apply_merge` (write) MCP tools + `--merge` CLI · `tests/merge.rs` (25), `tests/compare.rs`. Base supplied by git `merge-base` today; the reflow2-native DAG/ref layer (reflow2 finding its own merge-base) is BL-70. **rerere (BL-80 #5) done**: each conflict carries a content-fingerprint `resolution_key` (values, node-independent); `apply_merge` records applied property/edge-property resolutions as answered Questions keyed by it (travels in the export, no schema change); `recall_resolutions` returns them and `apply_merge use_recorded` reuses them advisory-only (`dec:merge-rerere`) — resolve one conflict shape, apply across all N identical ones. **Deferred: rerere keys for node-type/delete-modify conflicts** (no clean value triple), memory aging, and a document-producing `--merge-apply` CLI. |
| SP-6i | Function-preservation certificate for a maturity restructuring (BL-180, `req:restructuring-is-certified`) | 🟡 | `preserve::certify_preservation` reads a `compare_designs` diff and returns a **verdict**, not a listing: every divergence is classified `function` / `structure` / `supporting` / `unclassified`, and the answer is `preserved`, `not_preserved` or `indeterminate`. Function is the Capability/Requirement/Flow/Actor/Constraint/Project population plus links **wholly inside** that layer; structure is allocation, containment, interfaces and artifacts. **Edges are classified from their ENDPOINTS, not their type** — `DEPENDS_ON` is the functional DAG between Capabilities and packaging between Components, and reading the type alone would file all 51 of this design's cross-system dependencies as function changes. **`unclassified` never passes** (`dec:readiness-is-an-observation-…` applied to structure): an unknown node type, an unresolvable endpoint, or a semantic property edit on a function node forces `indeterminate` rather than a silent green — the [BL-170] fourth-quadrant guard, held by `every_schema_node_type_has_a_class`, which fails the build when a schema type lands with no class. A known function change **outranks** an unknown, since more information cannot un-move a capability. `certify_preservation` MCP tool (live graph or two files) + `DesignGraph::certify_preservation_against` · `tests/preserve.rs` (15), mutation-checked against four seeded regressions. **Silent about, and says so on every certificate:** the CODE (this reads two design records), function never captured, whether the new structure is *better*, a rename vs a scope change, and what a changed Interface now carries. **🟡 because the human-facing half is not built:** no skill asks *"is this the structure I would choose now?"*, and the maturity ladder that would place a design on the trajectory is still an open question under `dec:maturity-restructuring-delta` ([BL-179]). |
| SP-6j | Build-vs-design granularity, reported never judged (BL-182, `req:build-granularity-is-visible`) | ✅ | `granularity::granularity_report` counts the Capabilities each registered Artifact realizes and names the distributional outliers within **this design**, with the arithmetic and both cutoffs attached. **It detects no monoliths and makes no size judgement** — "avoid monoliths" is a subjective principle and reflow2 reads no code. What it reports is a **record-vs-record disagreement**: the design asserts N distinct Capabilities, the build asserts one Artifact — the same family as `compare_designs` (design-vs-design) and `reconcile_artifacts` (design-vs-disk), and like both it rules on neither side (`dec:report-dont-judge`). **The safety property is that the comparison is distributional**: a uniformly coarse early-phase design has no outlier and is told nothing, pinned by `a_uniformly_coarse_design_is_not_punished` — [BL-179]'s trap as an executable guarantee. `MIN_DISTINCTIONS` exists because counts pile up at 1 and the standard deviation collapses, so the z cutoff alone fires on noise; it is a floor on the **design's** distinctions, never on size, and it is echoed in the report. z-masking is a recorded property (`several_coarse_artifacts_mask_each_other`), so the reading is a prompt to look, not a count. `granularity_report` MCP tool + a section in `graph_report_markdown` (a reading nobody calls is invisible, which was the original complaint) · `tests/granularity.rs` (9), most of them silences; `the_observation_carries_no_verdict` asserts the serialized report contains no severity / suggested_fix / "monolith" / "split". **Validated against reality twice:** the committed design yields exactly one observation (`art:service`, 10 capabilities, z=6.46, alone), and the **v0.21.0 export — before the observation was ever made — yields the same one** at 9 capabilities, z=5.75. **Silent about:** unregistered artifacts, size of any kind, and which side is wrong. || SP-6k | Constructors merge on revise; only import replaces (BL-183, `req:no-silent-fallback`) | ✅ | All 18 public `add_*` helpers route through `upsert_node`, so re-calling one with an existing id revises rather than replaces: what you pass overwrites, what you omit keeps its current value. **The audit is the item, not the instance** — 16 of 18 named a subset of their type's schema properties, so `add_interface` was losing 13 (including `designation`, the published-boundary distinction `req:key-interfaces` depends on), `add_requirement` was losing `status` (which *is* certainty, `dec:certainty-derived`), `add_capability` was un-building verified capabilities, and `add_artifact` was BL-166 still reachable because that fix landed on `link_artifact` alone. Invisible until a property moves off its default, which is why it survived BL-46 and BL-166. **`create_node` and `import_graph` deliberately still REPLACE** — an import document is a complete statement of a node, so merging would resurrect what it meant to omit; `dec:import-merges-or-demands-whole-nodes` stays open and untouched. **The served revise-design skill was itself the cause** and is corrected in all three copies: it prescribed the generic `create_node` and asserted it merged. `tests/constructors_preserve.rs` (9) — written before the fix, 6 defect cases failing and 3 counterweights passing; the counterweights (creation still writes defaults, a named property is still overwritten, an explicit optional still wins) are what keep "merge" from becoming "first write wins". || SP-6l | The tool surface is split along the design's own decomposition (BL-181) | ✅ | 141 items moved verbatim from `service.rs` into eleven per-domain modules under `crates/reflow2-mcp/src/tools/`, each with `#[tool_router(router = X_router, vis = "pub")]`, summed in `ReflowService::new`. **6,356 → 2,658 lines.** Carved by `dec:bl83a-functional-decomposition` rather than by crate layout. **The loop closed on itself and every step is evidenced, not asserted:** `granularity_report` found it (z=6.46, alone, and would have at v0.21.0) → `toolsnap` proves the served surface byte-identical (**144/144**) → `certify_preservation` returns **`preserved`** (0 function, 0 unclassified, 36 structural, 30 supporting) → `granularity_report` re-run shows `art:service` absent. **`skill_lint` refused to lint mid-split** — it parses `#[tool]` from `service.rs`, found zero, and stopped rather than passing quietly; it now reads `tools/*.rs` too, with the implausible-parse guard kept. **Deliberately deferred:** the 126 request structs stay in `service.rs` (a coherent set of wire types; moving them is a second increment), and the three artifacts unmasked by the split (`art:temporal` 4 caps, `art:detect` and `art:graph` 3 each) are the next lap, not this row. || SP-6m | A design is located on the function-to-structure trajectory (BL-179, `req:trajectory-position-is-computed`) | ✅ | `maturity::maturity_report` measures seven bands — intent, function, allocation, seams, realization, assurance, operation — each as a count over a population carrying the question it answers, and names the lowest-scoring measurable band as the **frontier**. **The frontier is RELATIVE, so the reading holds no threshold at all**, which is stronger than `granularity`'s stated cutoffs: there is nothing to default. It reports where a design **is** and refuses to say where it **should** be — `dec:readiness-is-an-observation-the-threshold-is-the-judgement` applied to the design's own structure rather than to enabling technologies. **reflow2's own profile is the thesis as a measurement:** allocation 100%, **seams 0.0% (frontier)**, assurance 84.5%, operation 95.5% — three bands ahead of the frontier, reported as *normal*, on a design that has shipped 22 releases with no contract declared between any two of its eight systems. **No stage label** (`dec:edge-orthogonality`: a distinction no computation reads does not earn its keep) and **no schema change — the stamp does not move**. `maturity_report` MCP tool + a section in `graph_report_markdown` · `tests/maturity.rs` (10), mutation-checked against five seeded regressions, all caught. **Silent about:** where the design should be, whether these are the right seven questions, anything unrecorded, and quality — every band counts whether something exists, not whether it is any good. || SP-6n | Computed evidence reaches the quality axis it informs (BL-184, `req:computed-evidence-reaches-its-axis`) | ✅ | `ility::ility_report` attaches what the existing detectors and reports already found — cycles, articulation points, misplaced capabilities, hierarchy mismatches, modularity, surprising couplings, granularity, the trajectory bands — to the quality axis each bears on, naming the computation it came from. **It computes nothing new**, which is the point: the finding was never that reflow2 cannot measure these but that it measured them and said nothing about what they were evidence of. **The measurement that motivated it is stark: `DimensionAssessment` count in this design is ZERO** — nine axes, no instances, and `dimension_drifts` (a slope over asserted scores) therefore reading nothing. **Never derives a score** — collapsing findings into `maintainability: 0.62` is the lossy move `dec:readiness-is-an-observation-the-threshold-is-the-judgement` refused for TRL. **Adverse is inherited, never re-judged:** a finding counts against an axis only where its producing computation already calls it a defect, so ratios, trajectory positions and granularity observations are reported as **context** — relabelling them would overrule modules that deliberately refused to grade and reintroduce the thresholds they were built without. **Not blanket** (Anthony's point): `performance`, `security`, `scalability`, `observability` report *not informed* with the reason. **The asymmetry is deliberate:** an asserted good score over adverse evidence is flagged; an asserted low score over a quiet axis is not, because absence of findings is not evidence. `ility_report` MCP tool · `tests/ility.rs` (11), mutation-checked — four of five caught, **the fifth inert and exposing a mis-named test**, since removing the `Info` guard fails nothing when all four mapped categories are Warning-or-Critical by construction. **No schema change; the stamp does not move.** |
