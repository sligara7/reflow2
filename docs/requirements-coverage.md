...
| SP-6f | Design-vs-design compare + three-way merge (BL-71 rung c, BL-80) | 🟡 | `compare::compare_designs` reports directional divergence between two as-designed records (BL-71 rung c). `merge::merge_designs` (BL-80) is its write-side sibling: the three-way case table (`dec:merge-three-way`/`dec:merge-conflict-semantics`) over base/ours/theirs — one-sided taken, agreed taken, both-sides conflicted as deterministic-id Questions, delete/modify retained-and-asked, edges symmetric. Pure/deterministic, **proposal only — writes nothing**. `merge_designs` MCP tool + `--merge` CLI · `tests/merge.rs` (14), `tests/compare.rs`. **Deferred: the APPLY rung** — resolve the conflict Questions and commit the merged design (the write side; base supplied by git `merge-base` today, the reflow2-native DAG/ref layer is BL-70). |
