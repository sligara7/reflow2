...
| HEAL-3 | Defect catalog (13 categories) | 🟡 | orphan_node, contradiction, duplicate, unresolved_setup, disconnected_community, single_point_of_failure, dead_end, circular_dependency ✅ (8); **unreachable, weak_connection, missing_link, missing_entity, missing_embedding deferred**. `circular_dependency` walks a *directed* dependency view (DEPENDS_ON + contracts collapsed through their Interface), deliberately excluding the other traceability edges so the golden thread closing on itself isn't reported as a loop · `tests/cycles.rs`. **`disconnected_community` SAYS WHAT IT WALKED, since 2026-08-17.** It read "disconnected from the rest of the design" while computing components over `design_network()` — which drops nine node types and every review record, and does not count `CONTAINS` — so the nodes it named were genuinely reachable by an undirected walk of the graph and the sentence claimed otherwise (measured: the walk covers **1133 of 2413 nodes**). The message now names what it computed, how much of the graph it held, the exclusion that most often explains a false island, and the singleton count it does not report. The category key is deliberately NOT renamed: consumers match on it and it is an ility source · `the_finding_says_what_it_swept_and_stops_claiming_unreachability`, `the_swept_count_tracks_the_graph_rather_than_being_boilerplate` |
