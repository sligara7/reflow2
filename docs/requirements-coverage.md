...
| SP-6g | Analysis of alternatives — compare parallel branches (BL-70 v1) | 🟡 | `alternatives::analyze_alternatives` (branch-by-file, `dec:parallel-alternatives`) loads N alternative exports into throwaway in-memory graphs and lays their measures side by side (graph_report snapshot: design nodes, open gaps, structural defects, allocation modularity, capabilities verified) plus each non-baseline branch's `compare_designs` divergence from the baseline — comparable on measures, not advocacy. Alternatives are design *space* (CONTRADICTS), distinct from epochs (*time*). `analyze_alternatives` MCP tool · `tests/alternatives.rs` (4). Collapse reuses `merge_designs`/`apply_merge` + retire-from-design. **Deferred (rung 2): the decision-point wiring** (a proposed Decision GOVERNS alternative pointers that CONTRADICT; collapse accepts + merges winner + retires losers), the cross-branch-absence gap, per-branch readiness (BL-68), and world-scope. |
