...
| `tools/erosion_trial.py` | After N fix cycles, does the design describe what shipped? | **3/7** (was 2/7; coverage now distinguishes passing from present, and that probe went from hardcoded-fail to genuine), and `detect_gaps` still returns `[]` on a design that is fiction | BL-33, BL-34 |
