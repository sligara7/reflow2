...
| `tools/erosion_trial.py` | After N fix cycles, does the design describe what shipped? | **3/7** (was 2/7; coverage now distinguishes passing from present, and two probes have been tightened from weak to genuine — the drift-record probe now demands one event per drift and gets it), and `detect_gaps` still returns `[]` on a design that is fiction | BL-33, BL-34 |
