...
| `tools/build_design_graph.py` | What does reflow2 say about reflow2's own design? | 173 nodes, 3 gaps, **5 defects** — one **critical circular_dependency (cmp:propagate ↔ cmp:structure)**, real and previously invisible. 2026-07-20: the self-adopt pass found 15 of the prior 16 gaps pointed at the *model* (five shipped capabilities still `planned`; 15 of 33 source files unmodelled; zero DEPENDS_ON edges), ruled per §2 and corrected — gaps fell because the model was fixed, **not because a probe was loosened**; the builder now derives DEPENDS_ON from source and reconciles against the filesystem, so both staleness classes fire on their own next time. (Was 96 nodes, 16 gaps, 9 defects; before that 33/36 with 13 false gaps and 24 false defects — BL-38/BL-5/BL-42 cleared the noise.) | — |
