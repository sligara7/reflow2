...
| `skills/…` | The 20 workflows: genesis, adopt, brainstorm, capture-intent, capture-session, kpp-proposal, parallel-work, where-am-i, detect-and-ask, impact-check, check-health, link-artifacts, link-projects, revise-design, retire-from-design, ci-gate, report-friction, governance-proposal, plan-increments, ingest-corpus. `adopt` is the one for a system that already exists; [SKILLS.md](../SKILLS.md) says which to reach for when. **Served by the server, not installed** — `list_skills` names them, `get_skill` fetches one, and they always match the running binary |
